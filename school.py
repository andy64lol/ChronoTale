...
# Initialize colorama with different settings based on platform
import sys
import platform

# Fix for colorama in different environments
...
def settings_menu():
    ...
    while True:
        ...
        print("{0}2. Game Difficulty:{1} {2}".format(Fore.GREEN, Style.RESET_ALL, game_settings['difficulty'].capitalize()))
        print("{0}3. Text Speed:{1} {2}".format(Fore.BLUE, Style.RESET_ALL, game_settings['text_speed'].capitalize()))
        print("{0}4. Enable Cheats:{1} {2}".format(Fore.MAGENTA, Style.RESET_ALL, 'Yes' if game_settings['enable_cheats'] else 'No'))
        print("{0}5. Show Tutorial:{1} {2}".format(Fore.CYAN, Style.RESET_ALL, 'Yes' if game_settings.get('show_tutorial', True) else 'No'))
        ...
        if choice == "1":
            ...
            print("Cheats {0}.".format('enabled' if game_settings['enable_cheats'] else 'disabled'))
        elif choice == "5":
            game_settings["show_tutorial"] = not game_settings.get("show_tutorial", True)
            print("Tutorial hints {0}.".format('enabled' if game_settings['show_tutorial'] else 'disabled'))
        elif choice == "6":
            save_settings()
...
def content_settings_menu():
    ...
    while True:
        ...
        print("1. Allow Bullying Events: {0}".format('Yes' if game_settings['allow_bullying'] else 'No'))
        print("2. Allow Cuss Words: {0}".format('Yes' if game_settings['allow_cuss_words'] else 'No'))
        print("3. {0}Allow Anime-Style Family Relationships:{1} {2}".format(
            Fore.MAGENTA, Style.RESET_ALL, 
            f'{Fore.GREEN}Yes{Style.RESET_ALL}' if game_settings['allow_cross_family_love'] else f'{Fore.RED}No{Style.RESET_ALL}'))
        ...
        if choice == "1":
            ...
            print("Bullying events {0}.".format('enabled' if game_settings['allow_bullying'] else 'disabled'))
        elif choice == "2":
            game_settings["allow_cuss_words"] = not game_settings["allow_cuss_words"]
            print("Cuss words {0}.".format('enabled' if game_settings['allow_cuss_words'] else 'disabled'))
        elif choice == "3":
            game_settings["allow_cross_family_love"] = not game_settings["allow_cross_family_love"]
            if game_settings["allow_cross_family_love"]:
                print("{0}Anime-style family relationships enabled!{1}".format(Fore.MAGENTA, Style.RESET_ALL))
                print("This allows special relationships with siblings similar to anime storylines.")
            else:
...
def toggle_setting(setting_name, options):
    ...
    print("{0} set to {1}".format(setting_name.capitalize(), game_settings[setting_name].capitalize()))
...
def reset_settings():
    global game_settings
    game_settings = {
        "allow_nsfw": False,  # Always kept false, not visible in settings menu
...
def is_content_allowed(content_type):
    ...
    if content_type == "nsfw":
        return game_settings["allow_nsfw"]
    elif content_type == "bullying":
        return game_settings["allow_bullying"]
    elif content_type == "cuss_words":
        return game_settings["allow_cuss_words"]
    elif content_type == "cross_family_love":
        # Force this to be always allowed for the anime-style game (default to True even if not in settings)
        return game_settings.get("allow_cross_family_love", True)
    return False
...
def filter_text(text, content_types=None):
    ...
    # Remove cross_family_love from content types - it's always allowed in this anime game
    ...
    for content_type in content_types:
        if not is_content_allowed(content_type):
            if content_type == "cuss_words":
                # Replace cuss words with asterisks
                cuss_words = ["damn", "hell", "crap", "shit", "fuck", "ass", "bitch"]
                censored_text = text
                for word in cuss_words:
                    censored_text = censored_text.replace(word, "*" * len(word))
                return censored_text
            # For other content types, provide alternative text
            if "nsfw" in content_types:
                return "[Content not available with current settings]"
            elif "bullying" in content_types:
                return "Someone threw a paper ball at you from across the room."
                
    # If we reach here, all content types are allowed
    return text
...
def check_relationship_compatibility(person1, person2):
    ...
    """
    # Check if they are related (siblings, etc.)
    if "family" in person1 and person2["name"] in [member["name"] for member in person1["family"]["siblings"]]:
        # Cross-family love is now always allowed in this anime-themed game
        relation = "sibling"  # Default value
        for member in person1["family"]["siblings"]:
            if member["name"] == person2["name"]:
                relation = member["relation"]  # Get if they're older or younger sibling
                break
        
        # Anime-themed messages
        special_messages = [
            "{0}\"It's not like I think of you as just a sibling or anything, b-baka!\"{1}",
            "{0}\"W-what if someone finds out about us? Not that I care!\"{1}",
            "{0}\"We may be related, but my heart knows what it wants...\"{1}",
            "{0}\"I've always felt this special connection with you...\"{1}",
            "{0}\"Maybe this was destined to happen all along...\"{1}"
        ]
        
        slow_print("{0}A special bond forms between you and your {1}...{2}".format(
            Fore.MAGENTA, relation, Style.RESET_ALL), delay=0.04)
        
        # Random anime-style line
        slow_print(random.choice(special_messages).format(
            Fore.MAGENTA, Style.RESET_ALL), delay=0.05)
        
        # Add special achievement for sibling romance
        if "Forbidden Love" not in player["achievements"]:
            player["achievements"].append("Forbidden Love")
            slow_print("{0}Achievement unlocked: Forbidden Love{1}".format(
                Fore.YELLOW, Style.RESET_ALL))
            
            # Bonus charisma for being daring
            player["charisma"]["social"] += 3
            slow_print("Your social charisma has increased due to your unique relationship!")
...
# Ensure colorama is properly rendering colors in ASCII art
ASCII_ART = f"{Fore.CYAN}    ____________________________\n   |                            |\n   |    My First Day Here:      |\n   |    Campus Life Edition     |\n   |____________________________| {Style.RESET_ALL}"
...
clubs = {
    "Student Council": {
        ...
        "activities": ["Organize school festivals", "Hold meetings", "Vote on school policies"],
        "requirements": {"charisma": {"social": 5, "academic": 6}, "reputation": {"teachers": 20}},
        "members": [],
        "president": "Yuki Kato"
    },
    "Drama Club": {
    ...
    "Science Club": {
        ...
        "benefits": {
            "charisma": {"academic": 2},
            "grades_boost": ["Science"]
        },
        "activities": ["Conduct experiments", "Prepare for science fair", "Discuss scientific topics"],
    ...
    "Gaming Club": {
        ...
        "activities": ["Play video games", "Organize tournaments", "Discuss game development"],
        "requirements": {},
        "members": [],
...
def is_clothing_appropriate(clothing_name, location, is_school_day=True, is_festival=False):
    """
    ...
    # School rules
    school_locations = ["Classroom", "School Hallway", "Library", "Cafeteria", "Science Lab", "Gym"]
    ...
    if location in ["Student Room 364", "Your Bedroom"] and player.get("sleeping", False):
        if clothing["type"] != "sleep":
            return False, "You need to wear pajamas to sleep properly!"
    
    # Pajamas restrictions
    if clothing["type"] == "sleep" and location not in ["Student Room 364", "Your Bedroom"]:
...
def apply_clothing_effects(clothing_name, apply=True):
    ...
    if "charisma" in clothing:
        ...
        if clothing["type"] == "sleep" and player["current_location"] not in ["Student Room 364", "Your Bedroom"]:
            charisma_boost = -10 * modifier
        
        player["charisma"]["social"] = max(0, min(100, player["charisma"]["social"] + charisma_boost))
    ...
    if "reputation_boost" in clothing:
        ...
        player["reputation"]["students"] = max(0, min(100, player["reputation"]["students"] + rep_boost))
...
FESTIVALS = {
    "Spring Cherry Blossom Festival": {
        ...
        "activities": ["Cherry blossom viewing", "Traditional dance", "Food stalls", "Poetry contest"],
        "special_rewards": {
    ...
    "Summer Cultural Festival": {
        ...
        "activities": ["Firework display", "Stage performances", "Game booths", "Food competition"],
        "special_rewards": {
            "Cultural Festival Star": "Charisma (social) +2"
        },
    ...
    "Winter Holiday Festival": {
        ...
        "activities": ["Gift exchange", "Holiday decorating", "Hot food stalls", "Caroling"],
        "special_rewards": {
            "Winter Festival Enthusiast": "Money +1000"
        },
    ...
    "Tanabata Festival": {
        ...
        "activities": ["Writing wishes on tanzaku papers", "Decorating bamboo with ornaments", "Star gazing", "Traditional storytelling"],
        "special_rewards": {
    ...
    "Obon Festival": {
        ...
        "activities": ["Bon Odori dance", "Floating lanterns", "Ancestor offerings", "Traditional food"],
        "special_rewards": {
    ...
    "Setsubun Festival": {
        ...
        "activities": ["Bean throwing", "Demon mask making", "Fortune direction rituals", "Special sushi rolls"],
        "special_rewards": {
            "Fortune Bearer": "Luck +20% for the next month"
        },
    ...
    "Hinamatsuri": {
        ...
        "activities": ["Displaying ornamental dolls", "Making peach blossom decorations", "Traditional tea ceremony", "Special rice cakes"],
        "special_rewards": {
    ...
    "Kodomo no Hi": {
        ...
        "activities": ["Koinobori (carp streamers) decoration", "Kabuto (samurai helmet) crafting", "Traditional sweets", "Martial arts demonstrations"],
        "special_rewards": {
            "Youthful Warrior": "PE stats +2 and energy +15"
        },
    ...
    "Gion Matsuri": {
        ...
        "activities": ["Float procession viewing", "Traditional music performances", "Yukata wearing", "Street food tasting"],
        "special_rewards": {
    ...
    "Awa Odori": {
        ...
        "activities": ["Group dancing", "Costume wearing", "Traditional music", "Dance competitions"],
        "special_rewards": {
            "Dance Master": "Charisma (social) +2 and PE stats +1"
        },
    ...
    "Shichi-Go-San": {
        ...
        "activities": ["Traditional photo session", "Shrine visit", "Chitose candy distribution", "Special kimono wearing"],
        "special_rewards": {
            "Child at Heart": "Stress -30 and energy +20"
        },
    ...
    "Momijigari": {
        ...
        "activities": ["Leaf viewing hikes", "Nature photography", "Outdoor tea ceremony", "Leaf crafts"],
        "special_rewards": {
            "Nature Enthusiast": "Stress -25 and creativity +15"
        },
...
achievements = {
    ...
    "Club Leader": {
        ...
        "condition": lambda: any(club_info["president"] == player["name"] for club_name, club_info in clubs.items())
    },
    "Romance Expert": {
    ...
    "Master Networker": {
        ...
        "condition": lambda: len([name for name, points in relationship.items() if points >= 60]) >= 10
    },
    "Career Focused": {
    ...
    "Cultural Connoisseur": {
        ...
        "reward": {"charisma": {"social": 3, "academic": 2}, "reputation": {"students": 10, "teachers": 5}},
...
# Cafeteria menu
cafeteria_menu = {
    "Rice Bowl": {"price": 300, "energy_gain": 20, "hunger_gain": 30, "stress_reduction": 5},
    "Curry": {"price": 500, "energy_gain": 35, "hunger_gain": 50, "stress_reduction": 10},
    "Sandwich": {"price": 250, "energy_gain": 15, "hunger_gain": 20, "stress_reduction": 3},
    "Salad": {"price": 200, "energy_gain": 10, "hunger_gain": 15, "stress_reduction": 2},
...
def get_health_indicator():
    ...
    if health >= 90:
        ...
        return f"{Fore.YELLOW}Poor{Style.RESET_ALL}"
    else:
...
def get_stress_indicator():
    ...
    if stress < 20:
        return f"{Fore.GREEN}Relaxed{Style.RESET_ALL}"
    elif stress < 40:
        return f"{Fore.GREEN}Normal{Style.RESET_ALL}"
    elif stress < 60:
        return f"{Fore.YELLOW}Stressed{Style.RESET_ALL}"
    elif stress < 80:
        return f"{Fore.YELLOW}Very Stressed{Style.RESET_ALL}"
    else:
        return f"{Fore.RED}Burnout Risk{Style.RESET_ALL}"

def handle_rumor_interaction(choice, name, personality, relationship_gain_mult=1.0, relationship_loss_mult=1.0):
    """
    ...
    if choice == "12":  # Share gossip
        if "rumors" not in player or not player["rumors"]:
            slow_print(f"{name}: I haven't heard any interesting gossip lately. Do you know something I don't?")
            return 0
        ...
        try:
            ...
            if 1 <= choice_num <= len(sorted_rumors):
                ...
                if personality == "gossip":
                    slow_print(f"{name}: Ohhh, that's juicy! I'm definitely telling everyone about this!")
                    points_gain = int(random.randint(5, 8) * relationship_gain_mult)
                    # Gossip personalities spread rumors faster
                    selected_rumor["spread_level"] = min(10, selected_rumor["spread_level"] + 2)
                elif personality == "serious" or personality == "kuudere":
                    slow_print(f"{name}: I see. That's not particularly interesting to me, but I appreciate you sharing.")
                    points_gain = int(random.randint(1, 3) * relationship_gain_mult)
                    # Serious personalities rarely spread gossip
                    if random.random() < 0.3:
                        selected_rumor["spread_level"] = min(10, selected_rumor["spread_level"] + 1)
                else:
                    slow_print(f"{name}: Oh wow, I hadn't heard that! Thanks for telling me.")
                ...
                if "target" in selected_rumor:
                    ...
                    if name == target:
                        ...
                        slow_print(f"{Fore.RED}{name} suddenly looks upset.{Style.RESET_ALL}")
                        slow_print(f"{name}: Wait... people are saying that about ME? And you're spreading it?")
                        points_gain = -int(random.randint(10, 20) * relationship_loss_mult)
                    elif target in relationship:
                        # Target is someone this person knows
                        if relationship.get(target, 0) > 50:  # They like the target
                            slow_print(f"{name} frowns. \"That's not very nice to say about {target}.\"")
        ...
        if random.random() < gossip_chance:
            ...
            if personality in ["serious", "kuudere"]:
                slow_print(f"{name}: I don't participate in idle gossip. It's beneath me.")
            elif personality == "shy" or personality == "dandere":
                slow_print(f"{name}: I... um... don't really know any rumors. Sorry...")
            else:
                slow_print(f"{name}: I don't have any interesting gossip right now. Maybe ask me later?")
            points_gain = int(random.randint(1, 2) * relationship_gain_mult)  # Small gain for interaction
        ...
        try:
            ...
            if 1 <= type_num <= 4:
                ...
                rumor_type_map = {1: "romantic", 2: "academic", 3: "social", 4: "scandal"}
                ...
                if target_choice == "1":
                    ...
                    try:
                        ...
                        if 1 <= student_num <= len(students):
                            ...
                            slow_print("Invalid choice. Starting a general rumor instead.")
                    except ValueError:
                ...
                # Create the rumor
                created_rumor = create_custom_rumor(rumor_content, rumor_type=selected_type, target=target_name)
                
                # Relationship effects of starting a rumor
                if personality == "gossip":
                    slow_print(f"{name}: Ohhh, that's spicy! I'll definitely help spread that around!")
                    created_rumor["spread_level"] += 1  # They help spread it
                    points_gain = int(random.randint(4, 8) * relationship_gain_mult)
                elif personality in ["serious", "kuudere"]:
                    slow_print(f"{name}: I don't think it's appropriate to spread unverified information.")
                    ...
                    created_rumor["spread_level"] += random.choice([0, 1])  # May or may not help spread
...
def update_mental_health():
    ...
    if mental_health["bullying_incidents"] > 5:
        mental_health["depression"] = min(100, mental_health["depression"] + (mental_health["bullying_incidents"] // 5))
        mental_health["self_esteem"] = max(0, mental_health["self_esteem"] - (mental_health["bullying_incidents"] // 7))
    
    # Breakups affect depression and self-esteem
    if mental_health["breakup_count"] > 0:
        mental_health["depression"] = min(100, mental_health["depression"] + (mental_health["breakup_count"] * 5))
        mental_health["self_esteem"] = max(0, mental_health["self_esteem"] - (mental_health["breakup_count"] * 7))
    ...
    mental_health["depression"] = max(0, mental_health["depression"] - depression_reduction)
    
    # Happiness is affected by various factors
    mental_health["happiness"] = max(0, min(100, 
        80 - mental_health["depression"] // 2 - mental_health["anxiety"] // 3 + mental_health["self_esteem"] // 4))
    
    # Apply mental health effects to gameplay
    if mental_health["depression"] > 70:
        slow_print(f"{Fore.RED}You're feeling very depressed. Consider seeking therapy.{Style.RESET_ALL}")
    ...
    if mental_health["anxiety"] > 60:
        slow_print(f"{Fore.RED}Your anxiety is severe today, making it hard to focus.{Style.RESET_ALL}")
...
def attend_therapy():
    ...
    if player["money"] < therapy_cost:
        slow_print(f"{Fore.RED}You don't have enough money for therapy (costs {therapy_cost} yen).{Style.RESET_ALL}")
        return False
    ...
    slow_print(f"{Fore.GREEN}Your therapy session was helpful. You feel a bit better.{Style.RESET_ALL}")
    ...
    if mental_health["therapy_sessions"] >= 5:
        slow_print(f"{Fore.CYAN}You've developed better coping mechanisms from regular therapy.{Style.RESET_ALL}")
        mental_health["support_network"] = min(100, mental_health["support_network"] + 10)
...
def build_support_network():
    ...
    if player["family_relationship"]:
        family_support = sum(player["family_relationship"].values()) / len(player["family_relationship"])
    ...
    mental_health["support_network"] = min(100, 10 + (close_friends * 10) + (family_support * 5) + (club_support * 5))
...
def process_bullying_event(severity):
    ...
    mental_health["depression"] = min(100, mental_health["depression"] + depression_increase)
    mental_health["anxiety"] = min(100, mental_health["anxiety"] + anxiety_increase)
    mental_health["self_esteem"] = max(0, mental_health["self_esteem"] - self_esteem_decrease)
    
    # Compound effect for repeated bullying
    if mental_health["bullying_incidents"] > 3:
        slow_print(f"{Fore.RED}The repeated bullying is starting to affect your mental health.{Style.RESET_ALL}")
...
def update_health():
    ...
    player["health"] = min(100, max(0, int(stress_factor + hunger_factor + energy_factor + mental_health_factor)))
    
    # Give player feedback if health is critically low
    if player["health"] < 30 and player["health"] > 0:
        slow_print(f"{Fore.RED}Warning: Your health is getting very low. Try to eat, rest, and reduce stress.{Style.RESET_ALL}")
    elif player["health"] <= 0:
        slow_print(f"{Fore.RED}Your health has reached a critical level. You've been taken to the school nurse.{Style.RESET_ALL}")
        ...
        if player["mental_health"]["depression"] > 50 or player["mental_health"]["anxiety"] > 50:
            slow_print(f"\n{Fore.YELLOW}The school nurse seems concerned about your mental state.{Style.RESET_ALL}")
            slow_print("\"I'm noticing some signs of distress. Have you considered talking to someone about it?\"")
            slow_print("She gives you some information about the school's counseling services.")
            
            # Slight reduction in mental health symptoms from nurse intervention
            player["mental_health"]["depression"] = max(0, player["mental_health"]["depression"] - 5)
            player["mental_health"]["anxiety"] = max(0, player["mental_health"]["anxiety"] - 5)
            player["mental_health"]["support_network"] += 2  # Small boost from knowing help is available
...
def improve_relationship(name, points):
    ...
    if player["romantic_interest"] and player["romantic_interest"] == name:
        ...
        if next_stage in ROMANCE_STAGES and player["romance_points"] >= ROMANCE_STAGES[next_stage]["req"]:
            ...
            if next_stage == 5 and "Romance Expert" not in player["achievements"]:
                ...
                slow_print(f"{Fore.YELLOW}Achievement unlocked: Romance Expert{Style.RESET_ALL}")
...
def random_friendship_event():
    ...
    name = student["name"]
    if name not in relationship or relationship[name] < 20:  # Only for students you don't know well
        chance = min((player["charisma"]["social"] + player["charisma"]["academic"]) / 100, 0.8)
        if random.random() < chance:
            slow_print(f"\n{Fore.CYAN}Event: {name} wants to be friends with you!{Style.RESET_ALL}")
            choice = input("Accept? (y/n): ").lower()
            if choice == 'y':
                relationship[name] = max(relationship.get(name, 0) + random.randint(15, 25), 40)
                student_status[name] = "Friend"
                slow_print(f"{Fore.GREEN}You are now friends with {name}!{Style.RESET_ALL}")
...
def eat_food(food_name):
    ...
    slow_print(f"You ate {food_name}. Energy +{food['energy_gain']}, Hunger +{food['hunger_gain']}, Stress -{food['stress_reduction']}.")
...
all_subjects = {
    ...
    "Computer Basics": {"difficulty": 2, "homework_freq": 0.5, "year": 1, "core": False},
    "Economics Basics": {"difficulty": 3, "homework_freq": 0.6, "year": 1, "core": False},
    "Psychology Intro": {"difficulty": 3, "homework_freq": 0.5, "year": 1, "core": False},
    ...
    "Computer Science": {"difficulty": 4, "homework_freq": 0.7, "year": 2, "core": False},
    "Business Studies": {"difficulty": 3, "homework_freq": 0.6, "year": 2, "core": False},
    ...
    "Advanced Physics": {"difficulty": 6, "homework_freq": 0.9, "year": 3, "core": False},
    "Advanced Chemistry": {"difficulty": 6, "homework_freq": 0.9, "year": 3, "core": False},
    "Advanced Biology": {"difficulty": 5, "homework_freq": 0.8, "year": 3, "core": False},
    "Political Science": {"difficulty": 4, "homework_freq": 0.7, "year": 3, "core": False},
    ...
    "Advanced Research": {"difficulty": 6, "homework_freq": 0.9, "year": 4, "core": False},
    "International Relations": {"difficulty": 5, "homework_freq": 0.7, "year": 4, "core": False},
    "Literature IV": {"difficulty": 5, "homework_freq": 0.7, "year": 4, "core": True},
    "PE IV": {"difficulty": 2, "homework_freq": 0.3, "year": 4, "core": False},
    "Software Development": {"difficulty": 6, "homework_freq": 0.8, "year": 4, "core": False},
    "Business Management": {"difficulty": 5, "homework_freq": 0.7, "year": 4, "core": False},
...
# Student Council
student_council = [
    {"name": "Yuki Tanaka", "role": "President", "gender": "F", "personality": "strict"},
    {"name": "Akira Suzuki", "role": "Vice President", "gender": "M", "personality": "serious"},
    {"name": "Sakura Yamamoto", "role": "Secretary", "gender": "F", "personality": "kind"},
    {"name": "Ryo Nakamura", "role": "Treasurer", "gender": "M", "personality": "serious"}
]
...
special_events = {
    "Spring Festival": {
        ...
        "activities": ["Food Stalls", "Student Performances", "Sports Tournament", "Art Exhibition"],
        "rewards": {
            "reputation": 10,
            "money": 1000,
            "stress_reduction": 20
    ...
    "Cultural Festival": {
        ...
        "activities": ["Class Exhibitions", "Club Performances", "Cosplay Contest", "Food Court"],
        "rewards": {
            "reputation": 15,
            "money": 1500,
            "stress_reduction": 15
    ...
    "Winter Celebration": {
        ...
        "activities": ["Gift Exchange", "Winter Concert", "Special Dinner", "Year-in-Review"],
        "rewards": {
            "reputation": 5,
            "money": 500,
            "stress_reduction": 25
    ...
    "Sports Day": {
        ...
        "activities": ["Relay Race", "Tug of War", "Ball Games", "Swimming Competition"],
        "rewards": {
            "reputation": 8,
            "money": 800,
            "stress_reduction": 10
...
FESTIVALS = {
    "Cherry Blossom Festival": {
        ...
        "activities": ["Hanami Picnic", "Poetry Reading", "Photography Contest", "Outdoor Tea Ceremony"],
        "special_rewards": {
    ...
    "Summer Beach Day": {
        ...
        "activities": ["Swimming", "Beach Volleyball", "Sand Castle Contest", "Barbecue Party"],
        "special_rewards": {
            "Beach MVP": "Improved PE stats and social reputation"
        }
    },
    "Cultural Exchange Week": {
        ...
        "activities": ["International Food Fair", "Language Exchange", "Cultural Performances", "Traditional Costume Display"],
        "special_rewards": {
    ...
    "Winter Formal Dance": {
        ...
        "activities": ["Ballroom Dancing", "Photo Booth", "Formal Dinner", "Awards Ceremony"],
        "special_rewards": {
    ...
    "New Year Festival": {
        ...
        "activities": ["Bell Ringing Ceremony", "Fortune Telling", "New Year's Games", "Resolution Board"],
        "special_rewards": {
            "Fresh Start": "Reset stress levels and gain bonus money"
        }
    },
    "Spring Sports Tournament": {
        "date": (5, 20),  # May 20
        "description": "Inter-school sports competition showcasing athletic talents.",
        "activities": ["Soccer Match", "Basketball Tournament", "Track and Field Events", "Swimming Competition"],
        "special_rewards": {
...
# Locations
school_locations = [
    "Classroom", "Courtyard", "Library", "Cafeteria", "Club Room", "Gym", 
    "School Garden", "Rooftop", "Student Council Room",
    "Music Room", "Science Lab", "Art Studio", "Teachers' Office"
]
...
job_locations = ["Convenience Store", "Library (Work)", "Cafe", "Bookstore", "Restaurant"]

# Dating & Recreation Locations
dating_locations = [
    "Restaurant District", "Movie Theater", "Shopping Mall", 
    "City Park", "Beach", "Amusement Park", "Karaoke Bar", "Arcade Center"
]

# Combined Locations
locations = school_locations + dorm_locations + home_locations + job_locations + dating_locations
...
personalities = {
    ...
    "strict": {"homework_mult": 1.2, "grade_mult": 0.9, "romance_compatibility": 0.6, "relationship_gain": 2, "relationship_loss": 4},
    "kind": {"homework_mult": 0.8, "grade_mult": 1.1, "romance_compatibility": 0.9, "relationship_gain": 4, "relationship_loss": 2},
    "serious": {"homework_mult": 1.1, "grade_mult": 1.0, "romance_compatibility": 0.7, "relationship_gain": 3, "relationship_loss": 3},
    "lazy": {"homework_mult": 0.7, "grade_mult": 1.2, "romance_compatibility": 0.7, "relationship_gain": 3, "relationship_loss": 2},
    
    # Anime-inspired personality types
    "tsundere": {"homework_mult": 1.0, "grade_mult": 1.0, "romance_compatibility": 0.8, "relationship_gain": 2, "relationship_loss": 3, 
        "special_trait": "Acts cold but actually cares deeply. Relationship grows slower at first, then faster with persistence."},
    
    "kuudere": {"homework_mult": 1.3, "grade_mult": 0.8, "romance_compatibility": 0.6, "relationship_gain": 1, "relationship_loss": 4, 
        "special_trait": "Emotionally distant but highly intelligent. Harder to build relationship, but very loyal once close."},
    
    "dandere": {"homework_mult": 0.9, "grade_mult": 1.1, "romance_compatibility": 0.7, "relationship_gain": 3, "relationship_loss": 5, 
        "special_trait": "Shy and quiet, but opens up when comfortable. Sensitive to negative interactions."},
    
    "deredere": {"homework_mult": 0.7, "grade_mult": 1.2, "romance_compatibility": 1.0, "relationship_gain": 5, "relationship_loss": 3, 
        "special_trait": "Always cheerful and affectionate. Relationships grow very quickly."},
    
    "yandere": {"homework_mult": 1.1, "grade_mult": 1.0, "romance_compatibility": 0.4, "relationship_gain": 4, "relationship_loss": 8, 
        "special_trait": "Obsessively devoted but potentially dangerous. Relationship points decrease when interacting with others."},
    
    "genki": {"homework_mult": 0.8, "grade_mult": 1.1, "romance_compatibility": 0.9, "relationship_gain": 4, "relationship_loss": 2, 
        "special_trait": "Energetic and optimistic. Great at social activities and clubs."},
    
    "himedere": {"homework_mult": 1.2, "grade_mult": 0.9, "romance_compatibility": 0.5, "relationship_gain": 2, "relationship_loss": 5, 
    ...
    "bokukko": {"homework_mult": 0.9, "grade_mult": 1.0, "romance_compatibility": 0.8, "relationship_gain": 3, "relationship_loss": 3, 
        "special_trait": "Tomboyish personality who uses masculine speech. Excels in sports and physical activities."},
    
    "chunibyo": {"homework_mult": 0.7, "grade_mult": 0.8, "romance_compatibility": 0.7, "relationship_gain": 2, "relationship_loss": 4, 
    ...
    "megane": {"homework_mult": 1.4, "grade_mult": 1.3, "romance_compatibility": 0.7, "relationship_gain": 2, "relationship_loss": 3, 
        "special_trait": "Intellectual with glasses. Exceptional at studying and academic pursuits, but can be socially awkward."},
    
    "ojou-sama": {"homework_mult": 1.0, "grade_mult": 1.0, "romance_compatibility": 0.6, "relationship_gain": 2, "relationship_loss": 4, 
        "special_trait": "High class and elegant. Has refined tastes and expectations, but can be surprisingly down-to-earth."},
    
    "otaku": {"homework_mult": 1.1, "grade_mult": 1.0, "romance_compatibility": 0.5, "relationship_gain": 2, "relationship_loss": 3, 
        "special_trait": "Obsessed with anime, games, or other specific interests. Social relationship depends on shared interests."},
    
    "sporty": {"homework_mult": 0.8, "grade_mult": 0.9, "romance_compatibility": 0.8, "relationship_gain": 3, "relationship_loss": 2, 
        "special_trait": "Athletic and competitive. Excels in physical activities and has strong determination."},
    
    "wise": {"homework_mult": 1.2, "grade_mult": 1.2, "romance_compatibility": 0.7, "relationship_gain": 3, "relationship_loss": 3, 
        "special_trait": "Mature beyond their years, offering sage advice. Thoughtful and observant."},
    
    "fashionista": {"homework_mult": 0.8, "grade_mult": 0.9, "romance_compatibility": 0.8, "relationship_gain": 4, "relationship_loss": 3, 
        "special_trait": "Trend-conscious and socially savvy. Excels in style and social situations."},
    
    "class-clown": {"homework_mult": 0.6, "grade_mult": 0.8, "romance_compatibility": 0.9, "relationship_gain": 5, "relationship_loss": 2, 
        "special_trait": "Always joking and making others laugh. Very popular but sometimes not taken seriously."},
    
    "edgy": {"homework_mult": 0.9, "grade_mult": 0.9, "romance_compatibility": 0.5, "relationship_gain": 2, "relationship_loss": 4, 
...
def slow_print(text, delay=None, color=Fore.WHITE, style=None, highlight=None):
    """
    Print text with a typing effect and customizable colors
    
    Arguments:
    text -- the text to print
    delay -- the delay between characters (overrides text_speed setting)
    color -- text color (from colorama.Fore)
    style -- text style (from colorama.Style, e.g., BRIGHT, DIM)
    ...
    filtered_text = text
    if "shit" in text.lower() or "fuck" in text.lower() or "ass" in text.lower() or "bitch" in text.lower():
        filtered_text = filter_text(text, ["cuss_words"])
    
    # Simplify color handling for better compatibility
    # Check if text already contains colorama codes
    if any(marker in filtered_text for marker in [Fore.RED, Fore.GREEN, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN, Fore.WHITE, Style.RESET_ALL]):
        # Just print with a delay
        print(filtered_text)
        time.sleep(len(filtered_text) * delay * 0.5)  # Apply a scaled delay based on text length
        return
    
    # If text doesn't have colorama codes, apply default coloring
    styled_text = f"{highlight or ''}{style or Style.NORMAL}{color}{filtered_text}{Style.RESET_ALL}"
    
    # Simple character-by-character printing approach for non-colorama texts
    if delay > 0.01:  # Only do character by character for slower speeds
        for char in styled_text:
            print(char, end='', flush=True)
            time.sleep(delay)
        print()
    else:
        # For fast speed, just print everything at once
        print(styled_text)
        time.sleep(len(filtered_text) * delay * 0.5)  # Apply a scaled delay based on text length
...
def check_weekend_activities():
    ...
    if current_date.weekday() == 5:  # Saturday
        slow_print(f"{Fore.YELLOW}It's Saturday! Special weekend locations are available.{Style.RESET_ALL}")
    elif current_date.weekday() == 6:  # Sunday
        slow_print(f"{Fore.YELLOW}It's Sunday! Last chance for weekend activities.{Style.RESET_ALL}")
...
def handle_weekend_location(location):
    ...
    if location == "Movie Theater":
        ...
        try:
            ...
            if 1 <= choice_num <= len(movies):
                ...
                if player["money"] < selected_movie["price"]:
                    slow_print(f"You don't have enough money for that. You need ¥{selected_movie['price']}.")
                    return
                
                # Watch movie
                player["money"] -= selected_movie["price"]
                player["stress"] = max(0, player["stress"] + selected_movie["stress"])  # Reduce stress
                
                slow_print(f"You watch {selected_movie['title']} and enjoy the experience.")
                slow_print(f"Stress {selected_movie['stress']}")
                slow_print(f"You spent ¥{selected_movie['price']}. Remaining money: ¥{player['money']}")
                
                # Random chance for date opportunity if player has a romantic interest
                if player.get("romantic_interest") and player["romance_stage"] >= 3 and random.random() < 0.4:
                    romantic_date_event()
        ...
        try:
            ...
            if 1 <= choice_num <= len(activities):
                ...
                if choice_num == 1:  # Clothing stores
                    ...
                    slow_print("You spend some time window shopping and enjoying the atmosphere.")
        ...
        try:
            ...
            if 1 <= choice_num <= len(activities):
                ...
                if choice_num == 1:  # Jogging
                    ...
                    if player["money"] < 800:
                        slow_print("You don't have enough money for picnic supplies (¥800).")
                        return
                    ...
                    slow_print(f"You spent ¥800 on supplies. Remaining money: ¥{player['money']}")
                    
                elif choice_num == 3:  # Reading
                    slow_print("You find a quiet spot under a tree and read for a while.")
                    ...
                    slow_print("You spend a relaxing time feeding the ducks at the pond.")
                    slow_print("Stress -30")
                    slow_print(f"You spent ¥300 on duck feed. Remaining money: ¥{player['money']}")
            ...
            if 1 <= choice_num <= len(activities):
                ...
                if choice_num == 1:  # Swimming
                    ...
                    if random.random() < 0.2:  # 20% chance
                        ...
                        slow_print(f"You found a rare shell that a collector buys from you for ¥{found_money}!")
        ...
        restaurants = [
            {"name": "Sushi Bar", "price": 1800, "hunger": 70, "stress": -15, "charm": 3, "description": "A traditional Japanese sushi restaurant with fresh fish and authentic atmosphere."},
            {"name": "Italian Bistro", "price": 2200, "hunger": 80, "stress": -20, "charm": 5, "description": "A cozy Italian restaurant with homemade pasta and wood-fired pizzas."},
            {"name": "Campus Diner", "price": 1200, "hunger": 75, "stress": -10, "energy": 10, "description": "A popular student hangout with affordable comfort food."},
            {"name": "Fine Dining", "price": 3500, "hunger": 65, "stress": -25, "charm": 8, "reputation": 5, "description": "An upscale restaurant perfect for special occasions or impressing someone."},
            {"name": "Ramen Shop", "price": 900, "hunger": 85, "stress": -15, "energy": 15, "description": "A small but authentic ramen shop with steaming bowls of noodles."},
            {"name": "Okonomiyaki House", "price": 1100, "hunger": 80, "stress": -18, "energy": 12, "description": "A restaurant specializing in savory Japanese pancakes cooked on a hot plate."},
            {"name": "Korean BBQ", "price": 2500, "hunger": 90, "stress": -20, "energy": 10, "description": "A place where you grill your own meat at the table in Korean style."},
            {"name": "Curry House", "price": 1300, "hunger": 85, "stress": -15, "energy": 18, "description": "A restaurant serving Japanese-style curry dishes that are popular with students."},
            {"name": "Healthy Cafe", "price": 1600, "hunger": 60, "stress": -25, "health": 10, "description": "A modern cafe focusing on nutritious, healthy meals and smoothies."},
            {"name": "Dessert Parlor", "price": 800, "hunger": 40, "stress": -30, "happiness": 15, "description": "A sweet shop specializing in various desserts and crepes."}
        ]
        ...
        if game_settings["text_speed"] == "fast":
            ...
            slow_print("You walk down the vibrant Restaurant District, with various eateries lining the street:")
            for i, restaurant in enumerate(restaurants, 1):
                print(f"{i}. {restaurant['name']} - ¥{restaurant['price']}")
        ...
        if (dinner_start <= current_hour <= dinner_end or is_weekend()) and random.random() < 0.3:
            student = random.choice(students)
            if relationship.get(student["name"], 0) >= 30:  # Only for students you know somewhat
                slow_print(f"\n{Fore.CYAN}You spot {student['name']} looking for a place to eat too!{Style.RESET_ALL}")
                invite = input("Would you like to invite them to join you for dinner? (y/n): ").lower()
        ...
        try:
            ...
            if 1 <= choice_num <= len(restaurants):
                ...
                if player["money"] < restaurant["price"]:
                    slow_print(f"You realize this place is too expensive for your budget. You need ¥{restaurant['price']}.")
                    return
                
                # Apply base effects
                player["money"] -= restaurant["price"]
                player["hunger"] = min(100, player["hunger"] + restaurant["hunger"])
                player["stress"] = max(0, player["stress"] + restaurant.get("stress", 0))
                ...
                if "reputation" in restaurant:
                    player["reputation"]["social"] = min(100, player["reputation"]["social"] + restaurant["reputation"])
                
                if dining_choice == "1":  # Alone
                    slow_print(f"You enjoy a nice meal by yourself at {restaurant['name']}.")
                    slow_print("The food is delicious and gives you time to reflect.")
                    slow_print(f"Hunger +{restaurant['hunger']}, Stress {restaurant['stress']}")
                    slow_print(f"You spent ¥{restaurant['price']}. Remaining money: ¥{player['money'] - restaurant['price']}")
                    ...
                    friends = [name for name, level in player["relationships"].items() if level >= 30]
                    if not friends:
                        slow_print("You realize you don't have any close enough friends to invite to dinner.")
                        ...
                        try:
                            ...
                            if 0 <= friend_idx < len(friends):
                                ...
                                if player["money"] < additional_cost:
                                    slow_print(f"You realize you can't afford to pay for your friend as well (¥{additional_cost}).")
                                    slow_print("You text them to cancel and eat alone instead.")
                                else:
                                    player["money"] -= additional_cost
                                    
                                    # Friend dialogue
                                    slow_print(f"You invite {friend_name} to join you for dinner at {restaurant['name']}.")
                                    slow_print("You have a wonderful time catching up and enjoying the food together.")
                                    
                                    # Improve relationship
                                    relationship_boost = random.randint(5, 15)
                                    player["relationships"][friend_name] = min(100, player["relationships"][friend_name] + relationship_boost)
                                    
                                    slow_print(f"Your friendship with {friend_name} has grown stronger!")
                                    slow_print(f"Hunger +{restaurant['hunger']}, Stress {restaurant['stress'] - 5}")  # Extra stress reduction
                                    slow_print(f"You spent ¥{restaurant['price'] * 2} total. Remaining money: ¥{player['money']}")
                    ...
                    if player["money"] < date_cost:
                        slow_print(f"You realize you can't afford to pay for a date (¥{date_cost}).")
                        slow_print("You decide to eat alone instead.")
                    else:
                        player["money"] -= date_cost
                        
                        slow_print(f"You invite {interest_name} for a dinner date at {restaurant['name']}.")
                        ...
                        if restaurant["name"] == "Fine Dining":
                            ...
                            slow_print("The elegant atmosphere and exquisite food create a perfect romantic setting.")
                            slow_print(f"{interest_name} seems very impressed by your choice of restaurant.")
                        else:
                            romance_boost = random.randint(5, 15)
                            slow_print("You enjoy a lovely meal together, talking and laughing as you eat.")
                        
                        # Advance romance stage potentially
                        if stage < 5 and random.random() < 0.4:
                            player["romance_stage"] += 1
                            slow_print(f"Your relationship with {interest_name} has advanced to the next level!")
                        
                        slow_print(f"Hunger +{restaurant['hunger']}, Stress {restaurant['stress'] - 10}")  # Extra stress reduction
                        slow_print(f"You spent ¥{restaurant['price'] * 2} total. Remaining money: ¥{player['money']}")
                else:
                    slow_print("You decide to eat alone.")
                    slow_print(f"Hunger +{restaurant['hunger']}, Stress {restaurant['stress']}")
                    slow_print(f"You spent ¥{restaurant['price']}. Remaining money: ¥{player['money']}")
                
                # Advance time
                ticks += 12  # About 1-1.5 hours
            else:
                slow_print("You decide to just walk around the Restaurant District without eating.")
        except ValueError:
            slow_print("You wander around the Restaurant District, enjoying the sights and smells.")
        ...
        try:
            ...
            if 1 <= choice_num <= len(options):
                ...
                if player["money"] < option["price"]:
                    slow_print(f"You don't have enough money for that. You need ¥{option['price']}.")
                    return
                ...
                stress_reduction = min(25, option["time"] * 2)  # More time = more stress reduction
                ...
                if group_choice == "1":  # Solo
                    ...
                    player["creativity"] = min(100, player["creativity"] + random.randint(5, 10))
                    slow_print(f"You belt out your favorite songs for {option['name'].lower()}.")
                    slow_print(f"Creativity +5-10, Stress -{stress_reduction}")
                    slow_print(f"You spent ¥{option['price']}. Remaining money: ¥{player['money']}")
                    
                elif group_choice == "2":  # With friends
                    # Select friends to invite
                    friends = [name for name, level in player["relationships"].items() if level >= 20]
                    if not friends:
                        slow_print("You realize you don't have any friends to invite to karaoke.")
                        ...
                        try:
                            ...
                            for i in range(num_friends):
                                ...
                                try:
                                    idx = int(choice) - 1
                                    if 0 <= idx < len(friends) and friends[idx] not in invited_friends:
                            ...
                            if invited_friends:
                                ...
                                slow_print(f"You and {', '.join(invited_friends)} have an amazing time singing together!")
                                
                                # Additional cost for drinks/snacks
                                additional_cost = 500 * len(invited_friends)
                                if player["money"] < additional_cost:
                                    slow_print(f"You don't have enough money for drinks and snacks (¥{additional_cost}).")
                                    slow_print("Your friends chip in to cover the difference.")
                                else:
                                    player["money"] -= additional_cost
                                    slow_print(f"You treat everyone to drinks and snacks for ¥{additional_cost}.")
                                    
                                # Improve relationships
                                for friend in invited_friends:
                                    relationship_boost = random.randint(5, 15)
                                    player["relationships"][friend] = min(100, player["relationships"].get(friend, 0) + relationship_boost)
                                    
                                # Extra stress reduction for social fun
                                extra_stress = min(20, 5 * len(invited_friends))
                                player["stress"] = max(0, player["stress"] - extra_stress)
                                
                                # Charm increase from social activity
                                player["charm"] = min(100, player["charm"] + random.randint(3, 8))
                                
                                slow_print(f"Stress -{stress_reduction + extra_stress}, Charm +3-8")
                                total_cost = option["price"] + additional_cost
                                slow_print(f"You spent ¥{total_cost} total. Remaining money: ¥{player['money']}")
                            else:
                                slow_print("You couldn't decide who to invite and end up singing alone.")
                        except ValueError:
                    ...
                    if player["money"] < date_cost:
                        slow_print(f"You don't have enough money for a proper date (¥{date_cost}).")
                        slow_print("You decide to sing alone instead.")
                    else:
                        player["money"] -= date_cost
                        
                        slow_print(f"You invite {interest_name} for a karaoke date.")
                        slow_print("You take turns singing your favorite songs, sometimes doing duets together.")
                        ...
                        if random.random() < 0.5:
                            slow_print(f"During a slow song, {interest_name} sits very close to you and your eyes meet...")
                        ...
                        player["charm"] = min(100, player["charm"] + random.randint(5, 10))
        ...
        arcade_games = [
            {"name": "Dancing Stage", "price": 300, "rounds": 1, "energy": -10, "stress": -12, "social": 2, "skill": "rhythm", "description": "A dance game with arrows to step on in time with music."},
            {"name": "Racing Simulator", "price": 500, "rounds": 1, "energy": -8, "stress": -10, "skill": "reflexes", "description": "A realistic racing game with full motion controls."},
            {"name": "Fighting Arena", "price": 400, "rounds": 1, "energy": -12, "stress": -15, "skill": "technique", "description": "A competitive fighting game requiring skill and timing."},
            {"name": "Rhythm Game", "price": 300, "rounds": 1, "energy": -8, "stress": -15, "skill": "rhythm", "description": "Tap buttons in time with falling notes and music."},
            {"name": "Crane Game", "price": 200, "rounds": 1, "luck": True, "stress": -5, "description": "Try to grab a prize with the mechanical claw."},
            {"name": "Photo Booth", "price": 600, "rounds": 1, "social": 5, "stress": -10, "description": "Take fun decorated photos with friends."},
            {"name": "Virtual Reality", "price": 1000, "rounds": 3, "energy": -20, "stress": -25, "description": "An immersive VR experience with various game options."},
            {"name": "Ticket Games", "price": 1500, "rounds": 5, "energy": -15, "stress": -20, "tickets": True, "description": "Various games that award tickets for prizes."}
        ]
        
        slow_print("Welcome to the Arcade Center! With flashing lights and exciting sounds, this is a great place to have fun.")
        
        if game_settings["text_speed"] != "fast":
            slow_print("Students gather here to play games, compete, and hang out with friends.")
            ...
            if friend_options:
                ...
                if interact == 'y':
                    ...
                    if player["money"] < friend_game["price"]:
                        slow_print(f"Unfortunately, you don't have enough money. You need ¥{friend_game['price']}.")
                        slow_print(f"You watch {friend} play instead.")
                    else:
                        player["money"] -= friend_game["price"]
                        
                        # Play together
                        slow_print(f"You and {friend} have a great time playing {friend_game['name']} together!")
                        
                        # Game effects
                        if "energy" in friend_game:
                            player["energy"] = max(0, player["energy"] + friend_game["energy"])
                        if "stress" in friend_game:
                            player["stress"] = max(0, player["stress"] + friend_game["stress"] - 5)  # Extra stress reduction with friend
                        
                        # Relationship boost
                        relationship_boost = random.randint(3, 8)
                        relationship[friend] = min(100, relationship[friend] + relationship_boost)
                        
                        slow_print(f"Your friendship with {friend} has grown stronger!")
                        slow_print(f"You spent ¥{friend_game['price']}. Remaining money: ¥{player['money']}")
                        ...
                        if random.random() < 0.3:
                            ...
                            relationship[friend] = min(100, relationship[friend] + 5)  # Extra boost
        ...
        try:
            ...
            if 1 <= choice_num <= len(arcade_games):
                ...
                if selected_game["name"] != "Crane Game" and selected_game["name"] != "Photo Booth":
                    ...
                    if play_style == "2":  # With friend
                        friends = [name for name, level in relationship.items() if level >= 30]
                        if not friends:
                            slow_print("You realize you don't have any close enough friends to invite.")
                    ...
                    if play_style == "1":  # Alone
                        ...
                        if "energy" in selected_game:
                            player["energy"] = max(0, player["energy"] + selected_game["energy"])
                        if "stress" in selected_game:
                            player["stress"] = max(0, player["stress"] + selected_game["stress"])
                        ...
                        if "skill" in selected_game:
                            ...
                            if skill_roll < 0.7:  # 70% chance of normal success
                                ...
                                slow_print("You're on fire! You set a new personal record!")
                                player["stress"] = max(0, player["stress"] - 5)  # Extra stress reduction
                            else:  # 5% chance of amazing success
                                slow_print("INCREDIBLE! You achieved the highest score ever seen on this machine!")
                                player["stress"] = max(0, player["stress"] - 10)  # Major stress reduction
                                
                                # Other players gather around
                                slow_print("Other arcade-goers gather around to watch your amazing performance!")
                                if "social" in selected_game:
                                    player["charisma"]["social"] = min(100, player["charisma"]["social"] + selected_game["social"] * 2)
                        ...
                        if "tickets" in selected_game:
                            ...
                            if prize_choice == "1" and tickets >= 50:
                                slow_print("You exchange your tickets for a cute small toy!")
                                ...
                                slow_print("You exchange your tickets for a cool keychain!")
                                player["items"].append("Arcade Keychain")
                            else:
                                slow_print("You decide to save your tickets for another time.")
                        
                    elif play_style == "2.1" and invited_friend:  # With specific friend
                        slow_print(f"You invite {invited_friend} to play {selected_game['name']} with you.")
                        ...
                        if "energy" in selected_game:
                            player["energy"] = max(0, player["energy"] + selected_game["energy"])
                        if "stress" in selected_game:
                            player["stress"] = max(0, player["stress"] + selected_game["stress"] - 5)  # Extra stress reduction with friend
                        
                        # Relationship boost
                        if invited_friend in relationship:
                            relationship_boost = random.randint(5, 10)
                            relationship[invited_friend] = min(100, relationship[invited_friend] + relationship_boost)
                            slow_print(f"Your friendship with {invited_friend} has grown stronger!")
                        else:
                            # Add friend to relationships if not already there
                            relationship_boost = random.randint(5, 10)
                            relationship[invited_friend] = relationship_boost
                            slow_print(f"You've started a new friendship with {invited_friend}!")
                        
                    elif play_style == "3":  # Challenge stranger
                        slow_print(f"You look around for someone to challenge at {selected_game['name']}.")
                        
                        # Meet a new potential friend or rival
                        stranger_names = ["Yusuke", "Keiko", "Takeshi", "Hana", "Ryota", "Sakura"]
                        ...
                        if outcome < 0.4:  # You win
                            slow_print(f"You win against {stranger}! They seem impressed by your skills.")
                            relationship[stranger] = min(100, relationship[stranger] + random.randint(5, 15))
                            slow_print(f"{stranger} asks for a rematch sometime. You've made a new friend!")
                        elif outcome < 0.8:  # Close match
                            slow_print(f"It's a close match with {stranger}! You both had fun competing.")
                            relationship[stranger] = min(100, relationship[stranger] + random.randint(3, 8))
                            slow_print(f"You and {stranger} exchange contact info to play again sometime.")
                        else:  # You lose
                            slow_print(f"You lose to {stranger}, who's clearly an expert at this game.")
                            slow_print(f"{stranger} gives you some tips on how to improve.")
                            relationship[stranger] = min(100, relationship[stranger] + random.randint(1, 5))
                        
                        if "stress" in selected_game:
                            player["stress"] = max(0, player["stress"] + selected_game["stress"])
                        if "energy" in selected_game:
                            player["energy"] = max(0, player["energy"] + selected_game["energy"] - 5)  # Extra energy used in competition
                
                else:  # Special games with different mechanics
                    if selected_game["name"] == "Crane Game":
                        slow_print("You try your luck at the crane machine, aiming for a prize...")
                        
                        # Crane game outcome based on luck
                        crane_luck = random.random()
                        if crane_luck < 0.15:  # 15% chance to win
                            prizes = ["Cute Plushie", "Character Keychain", "Small Figurine", "Lucky Charm"]
                            won_prize = random.choice(prizes)
                            slow_print(f"Success! You managed to grab a {won_prize}!")
                            player["items"].append(won_prize)
                            player["stress"] = max(0, player["stress"] - 15)  # Extra stress reduction from winning
                        elif crane_luck < 0.3:  # Almost wins
                            slow_print("So close! The prize slips out of the claw at the last moment.")
                            slow_print("A staff member notices and gives you a small consolation prize.")
                            player["items"].append("Consolation Prize")
                            player["stress"] = max(0, player["stress"] - 8)
                        else:
                            slow_print("The claw weakly grabs at the prize but can't pick it up.")
                        ...
                        if photo_choice == "1":  # Solo
                            slow_print("You take some fun solo photos with different filters and backgrounds.")
                            slow_print("The photos turn out great! You keep them as a memento.")
                            ...
                            if not friends:
                                ...
                                try:
                                    indices = [int(idx) - 1 for idx in friend_choices.split()]
                                    for idx in indices[:3]:
                                        if 0 <= idx < len(friends):
                                            photo_friends.append(friends[idx])
                                except ValueError:
                                    slow_print("Invalid selection. You'll take solo photos.")
                                
                                if photo_friends:
                                    friend_str = ", ".join(photo_friends)
                                    slow_print(f"You and {friend_str} squeeze into the photo booth together.")
                                    slow_print("You all make silly poses and try different backgrounds.")
                                    slow_print("The photos turn out hilarious and memorable!")
                                    
                                    player["items"].append("Friend Group Photos")
                                    player["stress"] = max(0, player["stress"] + selected_game["stress"] - 5)
                                    ...
                                    slow_print("Your friendships have grown stronger through this shared memory!")
                                else:
                                    slow_print("You take solo photos instead.")
                                    player["items"].append("Solo Photo Booth Pictures")
                        
                        elif photo_choice == "3" and player.get("romantic_interest"):  # With date
                            interest_name = player["romantic_interest"]
                            
                            slow_print(f"You invite {interest_name} to take cute photos together.")
                            slow_print("You both pose close together with various romantic backgrounds.")
                            
                            player["items"].append("Couple Photo Booth Pictures")
                            player["stress"] = max(0, player["stress"] + selected_game["stress"] - 10)
                            
                            # Romance boost
                            romance_boost = random.randint(5, 15)
                            player["romance_points"] = min(100, player["romance_points"] + romance_boost)
                            
                            slow_print(f"The photo session brings you and {interest_name} closer together!")
                            ...
                            if random.random() < 0.3 and player["romance_stage"] >= 3:
                                ...
                                player["romance_points"] = min(100, player["romance_points"] + 5)
                        else:
                            slow_print("You take solo photos instead.")
                            player["items"].append("Solo Photo Booth Pictures")
                
                # Common end for all arcade games
                slow_print(f"You spent ¥{total_cost}. Remaining money: ¥{player['money']}")
                
                # Advance time
                ticks += selected_game["rounds"] * 2  # Each round is about 10-15 minutes
                
            elif choice_num == len(arcade_games) + 1:  # Just watch
                slow_print("You wander around the arcade, watching other people play games.")
                slow_print("It's entertaining and relaxing just to observe the excitement around you.")
        ...
        if player["money"] < entrance_fee:
            slow_print(f"The entrance fee is ¥{entrance_fee}, but you only have ¥{player['money']}.")
            slow_print("You decide to come back another time when you have enough money.")
            return
        ...
        if max_rides == 0:
            slow_print("You're too tired to go on any rides. Maybe come back another time.")
            # Refund entrance fee
            player["money"] += entrance_fee
        ...
        while ride_count < max_rides:
            ...
            choice = input(f"Which ride would you like to go on? (1-{len(rides) + 1}): ")
            ...
            try:
                ...
                if 1 <= choice_num <= len(rides):
                    ...
                    player["stress"] = max(0, player["stress"] - selected_ride['thrill'])
                    player["energy"] = max(0, player["energy"] + selected_ride['energy'])
                    
                    slow_print(f"Stress -{selected_ride['thrill']}, Energy {selected_ride['energy']}")
                    
                    # Chance for special event on rides
                    if random.random() < 0.3:  # 30% chance
                        if player.get("romantic_interest") and player["romance_stage"] >= 2:
                            romantic_date_event()
                        else:
                            random_friend_encounter()
...
def weekend_study_session():
    ...
    try:
        ...
        if 1 <= choice_num <= len(current_subjects):
            ...
            player["charisma"]["academic"] = min(100, player["charisma"]["academic"] + random.randint(5, 10))
            player["energy"] = max(0, player["energy"] - 25)
            player["stress"] = max(0, player["stress"] - 5)  # Slight stress reduction from accomplishment
            ...
            slow_print("You spread your time across all subjects, making moderate progress in each.")
            ...
            player["charisma"]["academic"] = min(100, player["charisma"]["academic"] + random.randint(3, 7))
...
def improve_family_relationships():
    ...
    for member in family_members:
        ...
        player["family_relationship"][name] = min(100, current_relationship + improvement)
...
def romantic_date_event():
    ...
    if stage <= 2:
        ...
        if choice == "1":
            ...
            slow_print("You spend a fun time together and get to know each other better.")
        ...
        if choice == "1":
            slow_print(f"You suggest making this an official date with {interest_name}.")
            ...
            if personality == "tsundere":
                slow_print(f"At the end of the date, {interest_name} hesitantly says, \"I... I guess this wasn't terrible. We could... maybe do it again sometime.\"")
            elif personality == "kuudere":
                slow_print(f"{interest_name} looks at you with a slight softening in their usually stoic expression. \"This was... pleasant. I would not be opposed to repeating it.\"")
            elif personality == "dandere":
                slow_print(f"{interest_name} shyly takes your hand for a moment. \"Th-thank you for today. It was really special.\"")
            elif personality == "deredere":
                slow_print(f"{interest_name} hugs you enthusiastically. \"This was the BEST day ever! Let's do this again soon, okay?\"")
            elif personality == "yandere":
                slow_print(f"{interest_name} looks intensely into your eyes. \"No one else gets to see you like this. Only me. Remember that.\"")
            elif personality == "genki":
                slow_print(f"{interest_name} jumps excitedly. \"That was super fun! We make an awesome team! Same time next weekend?\"")
            elif personality == "himedere":
                slow_print(f"{interest_name} gives you a regal nod. \"I suppose I can pencil you in for another outing. Consider yourself fortunate.\"")
            else:
                slow_print(f"{interest_name} smiles warmly at you. \"Thank you for today. I really enjoyed spending time with you.\"")
                
        elif choice == "2":
            slow_print("You casually spend time together, enjoying each other's company.")
...
def random_friend_encounter():
    ...
    available_students = [s for s in students if s["name"] != player.get("romantic_interest")]
    ...
    if choice == "1":
        slow_print(f"You have a great conversation with {name} and spend some time together.")
        ...
        if personality == "tsundere":
            slow_print(f"{name} tries to act indifferent but clearly enjoys your company.")
        elif personality == "kuudere":
            slow_print(f"{name} remains calm, but you notice them relaxing slightly as you talk.")
        elif personality == "dandere":
            slow_print(f"{name} gradually becomes more talkative as they get comfortable with you.")
        elif personality == "deredere":
            slow_print(f"{name} enthusiastically shares stories and laughs at all your jokes.")
        elif personality == "yandere":
            slow_print(f"{name} seems very interested in your personal life and other friendships.")
        elif personality == "genki":
            slow_print(f"{name} bubbles with energy, making your time together action-packed and fun.")
        elif personality == "himedere":
        ...
        slow_print(f"You have a pleasant chat with {name} before continuing your activities.")
        relationship_boost = random.randint(5, 10)
        current = player["relationships"].get(name, 0)
        player["relationships"][name] = min(100, current + relationship_boost)
        ...
        slow_print(f"You exchange brief pleasantries with {name} and continue on your way.")
    ...
    if student.get("personality") in ["serious", "kuudere"] and choice in ["1", "2"]:
        ...
        if current_subjects:
            ...
            new_grade = min(100.0, current_grade + grade_boost)
            player["grades"][subject] = new_grade
            slow_print(f"{name} mentions something helpful about {subject} that improves your understanding!")
...
def find_study_partner():
    ...
    studious_students = [s for s in students if s.get("personality") in ["serious", "kuudere", "strict"]]
    ...
    if choice == "1":
        ...
        if random.random() < acceptance:
            ...
            if current_subjects:
                ...
                # Complete homework for this subject
                homework[subject] = True
                slow_print(f"You made progress on your {subject} homework with a grade boost from {current_grade:.1f} to {new_grade:.1f}.")
            ...
            if subject:
                slow_print(f"Studying with {name} helps you understand {subject} much better!")
            else:
                slow_print(f"Studying with {name} helps you understand your coursework better!")
            slow_print(f"Your friendship with {name} has improved!")
            
            # Intelligence boost
            player["intelligence"] = min(100, player["intelligence"] + random.randint(3, 8))
...
def clothes_shopping():
    ...
    for i, (name, details) in enumerate(available_clothing.items(), 1):
        ...
        if "charisma" in details:
            stat_bonuses.append(f"Charisma {'+'if details['charisma'] >= 0 else ''}{details['charisma']}")
    ...
    try:
        ...
        if 1 <= choice_num <= len(clothing_list):
            ...
            if player["money"] < clothing_details["price"]:
                slow_print(f"You don't have enough money for that. You need ¥{clothing_details['price']}.")
                return
            ...
            slow_print(f"You spent ¥{clothing_details['price']}. Remaining money: ¥{player['money']}")
            
            # Ask if player wants to wear the new clothing right away
            wear_now = input("Would you like to wear your new clothing now? (y/n): ").lower()
            if wear_now == 'y':
                ...
                if player["current_location"] in CHANGING_LOCATIONS:
                    ...
                    slow_print("You can't change clothes here. You need to be in your room or a changing room.")
            
            # Random compliment chance for stylish clothing
            if "charisma" in clothing_details and clothing_details["charisma"] > 5 and random.random() < 0.4:
    ...
    try:
        ...
        if 1 <= choice_num <= len(books):
            ...
            if player["money"] < book["price"]:
                slow_print(f"You don't have enough money for that. You need ¥{book['price']}.")
                return
            ...
            slow_print(f"You spent ¥{book['price']}. Remaining money: ¥{player['money']}")
            ...
            if random.random() < 0.3:
                ...
                slow_print("You learn something interesting from browsing books. Intelligence +2")
...
def electronics_shopping():
    ...
    items = [
        ...
        {"name": "Noise-Canceling Headphones", "price": 8000, "stress": -10, "productivity": 5},
    ...
    try:
        ...
        if 1 <= choice_num <= len(items):
            ...
            if player["money"] < item["price"]:
                slow_print(f"You don't have enough money for that. You need ¥{item['price']}.")
                return
            ...
            slow_print(f"You spent ¥{item['price']}. Remaining money: ¥{player['money']}")
...
def food_court_meal():
    ...
    food_options = [
        {"name": "Fast Food Combo", "price": 800, "hunger": 70, "energy": 10, "health": -5},
        {"name": "Healthy Salad", "price": 900, "hunger": 50, "health": 10},
        {"name": "Gourmet Burger", "price": 1200, "hunger": 80, "energy": 15},
        {"name": "International Cuisine", "price": 1500, "hunger": 75, "energy": 10, "stress": -10},
        {"name": "Sweet Dessert", "price": 600, "hunger": 30, "stress": -15, "health": -3}
    ]
    ...
    try:
        ...
        if 1 <= choice_num <= len(food_options):
            ...
            # Check if player has enough money
            if player["money"] < food["price"]:
                slow_print(f"You don't have enough money for that. You need ¥{food['price']}.")
                return
            ...
            slow_print(f"You spent ¥{food['price']}. Remaining money: ¥{player['money']}")
            ...
            slow_print("You decide not to eat anything and just look around the food court.")
...
def is_holiday():
    ...
    is_summer_holiday = (current_date.month == 7 and current_date.day >= 20) or (current_date.month == 8)
    
    # Winter holiday period (December 24 to January 7)
    is_winter_holiday = (current_date.month == 12 and current_date.day >= 24) or (current_date.month == 1 and current_date.day <= 7)
    
    # Spring break (March 15 to April 5)
    is_spring_break = (current_date.month == 3 and current_date.day >= 15) or (current_date.month == 4 and current_date.day <= 5)
    
    # Special holiday messages and events
    global holiday_events_shown
    
    # Lunar New Year Celebration
    if is_lunar_new_year and not holiday_events_shown["lunar_new_year"]:
        slow_print(f"\n{Fore.RED}======= LUNAR NEW YEAR FESTIVAL ======={Style.RESET_ALL}")
        slow_print(f"{Fore.RED}Happy Lunar New Year! The campus is decorated with red lanterns and traditional decorations.{Style.RESET_ALL}")
        slow_print("Many students have returned to their families, but special activities are available for those who stayed.")
        ...
        slow_print("Your participation in New Year festivities increased your reputation!")
        ...
        if choice == "1":
            slow_print("You enjoyed the exciting lion dance performance with drums and cymbals!")
            slow_print("The performers were impressed by your enthusiasm.")
            player["stress"] = max(0, player["stress"] - 15)
            player["charisma"]["social"] += 1
        elif choice == "2":
            slow_print("You spent time hanging red lanterns and spring couplets around campus.")
            slow_print("The teachers appreciated your help and the campus looks beautiful!")
            player["reputation"]["teachers"] += 5
            player["charisma"]["social"] += 1
        elif choice == "3":
            slow_print("You learned how to write New Year blessings in beautiful calligraphy.")
            player["charisma"]["academic"] += 2
            player["stress"] = max(0, player["stress"] - 10)
        elif choice == "4":
            slow_print("You spent time making dumplings and other traditional foods with fellow students.")
            slow_print("Everyone enjoyed the delicious meal and appreciated your cooking skills!")
            player["hunger"] = 100
    ...
    if is_summer_holiday and current_date.day == 20 and current_date.month == 7 and not holiday_events_shown["summer_holiday"]:
        slow_print(f"\n{Fore.YELLOW}======= SUMMER VACATION BEGINS ======={Style.RESET_ALL}")
        slow_print(f"{Fore.YELLOW}Summer vacation has started! You have free time until the end of August.{Style.RESET_ALL}")
        slow_print("The campus is much quieter now, with most students gone for the break.")
        ...
        if choice == "1":
            ...
            player["energy"] -= 30
            player["stress"] += 15
            slow_print(f"You worked hard all summer at a resort and earned ¥{earnings}!")
            slow_print("Your experience dealing with customers improved your social skills.")
            player["charisma"]["social"] += 2
            ...
            if "Summer Worker" not in player["achievements"]:
                ...
                slow_print(f"{Fore.GREEN}Achievement Unlocked: Summer Worker{Style.RESET_ALL}")
            ...
            for subject in list(player["grades"].keys()):
                if player["grades"][subject] != "A" and player["grades"][subject] != "A+":
            ...
            slow_print(f"You studied hard over the summer and improved in {improved} subjects!")
            slow_print("Your focus on academics has made you better prepared for the coming year.")
            player["reputation"]["teachers"] += 5
            ...
            if improved >= 3 and "Summer Scholar" not in player["achievements"]:
                ...
                slow_print(f"{Fore.GREEN}Achievement Unlocked: Summer Scholar{Style.RESET_ALL}")
            ...
            destinations = ["the beach", "the mountains", "a historic city", "a foreign country"]
            destination = random.choice(destinations)
            
            slow_print(f"You traveled to {destination} and had an amazing adventure!")
            slow_print("You met many interesting people and learned about different cultures.")
            slow_print("The experience broadened your horizons and reduced your stress.")
            ...
            if "World Traveler" not in player["achievements"]:
                ...
                slow_print(f"{Fore.GREEN}Achievement Unlocked: World Traveler{Style.RESET_ALL}")
            ...
            slow_print("You had a relaxing summer at home, catching up on sleep and hobbies.")
            ...
            internship_types = ["research assistant", "teaching assistant", "laboratory technician"]
            ...
            slow_print("You made valuable connections with faculty and gained practical experience.")
            ...
            if "Summer Intern" not in player["achievements"]:
                ...
                slow_print(f"{Fore.GREEN}Achievement Unlocked: Summer Intern{Style.RESET_ALL}")
    ...
    if is_winter_holiday and current_date.day == 24 and current_date.month == 12 and not holiday_events_shown["winter_holiday"]:
        slow_print(f"\n{Fore.CYAN}======= WINTER HOLIDAY BEGINS ======={Style.RESET_ALL}")
        slow_print(f"{Fore.CYAN}Winter break has started! The campus is decorated for the holidays.{Style.RESET_ALL}")
        slow_print("Many students have gone home to celebrate with family.")
        
        # Small gift and money from family
        player["money"] += 1000
        slow_print("Your family sent you ¥1000 and a small care package for the holidays!")
        player["hunger"] = 100
        player["energy"] += 20
    ...
    # Spring break events
    if is_spring_break and current_date.day == 15 and current_date.month == 3 and not holiday_events_shown["spring_break"]:
        slow_print(f"\n{Fore.GREEN}======= SPRING BREAK BEGINS ======={Style.RESET_ALL}")
        slow_print(f"{Fore.GREEN}Spring break has begun! A short rest before the end of the academic year.{Style.RESET_ALL}")
        slow_print("Many students are studying for final exams or enjoying a brief vacation.")
    ...
    return is_regular_holiday or is_lunar_new_year or is_summer_holiday or is_winter_holiday or is_spring_break
...
def handle_year_end():
    ...
    if player["school_year"] not in player["completed_years"]:
        ...
        slow_print(f"{Fore.YELLOW}CONGRATULATIONS! School Year {player['school_year']} has been completed!{Style.RESET_ALL}")
    ...
    for subject_name, grade in player["grades"].items():
        ...
        if subject_name in subjects:
            ...
            if isinstance(grade, (int, float)):
                ...
                points = grade_points.get(grade, grade_points.get(grade[0], 0))  # Handle grades like "A" or "A+"
...
def show_year_end_summary():
    ...
    if relationship:
        for name, points in sorted(relationship.items(), key=lambda x: x[1], reverse=True)[:5]:
    ...
    try:
        input(f"{Fore.CYAN}Press Enter to continue...{Style.RESET_ALL}")  # No need to store the input
    except EOFError:
...
def choose_new_electives():
    ...
    slow_print("As you advance to a new school year, you can choose new elective subjects.")
    ...
    while len(chosen_electives) < num_to_choose:
        try:
            choice = int(input(f"{Fore.CYAN}Select elective {len(chosen_electives)+1}/{num_to_choose} (enter number): {Style.RESET_ALL}"))
            ...
            slow_print(f"{Fore.RED}Input error encountered. Skipping selection.{Style.RESET_ALL}")
            ...
            if not chosen_electives and available_electives:
                ...
                slow_print(f"Automatically selected {random_elective} due to input error.")
            break
        except Exception as e:
            slow_print(f"{Fore.RED}Error: {str(e)}. Skipping selection.{Style.RESET_ALL}")
            break
...
def offer_internship_opportunities():
    ...
    slow_print("As a third-year student, you now have access to internship opportunities.")
    slow_print("Internships can provide valuable experience, connections, and future job prospects.")
    
    internships = [
        {"name": "Tech Company Internship", "requirements": {"charisma": {"academic": 8}, "grades": {"Computer Science": "B"}}, "benefits": {"money": 5000, "reputation": 15, "skill": "Programming"}},
        {"name": "Hospital Volunteer", "requirements": {"charisma": {"social": 7}}, "benefits": {"reputation": 20, "skill": "Medical Knowledge"}},
        {"name": "Research Assistant", "requirements": {"charisma": {"academic": 10}}, "benefits": {"reputation": 15, "skill": "Research Methodology"}},
        {"name": "Teaching Assistant", "requirements": {"grades": {"average": "B"}}, "benefits": {"money": 3000, "reputation": 10, "skill": "Teaching"}},
        {"name": "Business Intern", "requirements": {"charisma": {"social": 8, "academic": 5}}, "benefits": {"money": 8000, "skill": "Business Acumen"}}
    ]
        ...
        if "grades" in internship["requirements"]:
            for subject, min_grade in internship["requirements"]["grades"].items():
                if subject == "average":
                    ...
                    meets_requirements = player["grades"][subject] <= min_grade  # A is better than B
        
        if meets_requirements:
            available.append(internship)
            slow_print(f"\n{i+1}. {internship['name']}")
            slow_print(f"   Benefits: ¥{internship['benefits'].get('money', 0)}, Reputation +{internship['benefits'].get('reputation', 0)}")
            slow_print(f"   Skill gained: {internship['benefits']['skill']}")
    
    if not available:
        slow_print("\nUnfortunately, you don't qualify for any internships at this time.")
        slow_print("Focus on improving your grades and skills to qualify next semester.")
        return
    ...
    try:
        ...
        if 1 <= choice <= len(available):
            ...
            slow_print("This will provide valuable experience throughout your third year.")
        elif choice == 0:
        ...
        slow_print(f"{Fore.RED}Input error encountered. Skipping internship selection.{Style.RESET_ALL}")
    except Exception as e:
        slow_print(f"{Fore.RED}Error: {str(e)}. Skipping internship selection.{Style.RESET_ALL}")

def begin_career_planning():
    """Start career planning for fourth-year students"""
    slow_print(f"\n{Fore.YELLOW}=== Career Planning ==={Style.RESET_ALL}")
    slow_print("As a fourth-year student, it's time to start thinking about your future after graduation.")
    slow_print("Your academic performance, relationships, and experiences will influence your career options.")
    
    # Career paths based on player's strengths
    careers = [
        {"path": "Graduate School", "description": "Continue your academic journey with advanced studies", "min_gpa": 3.5},
        {"path": "Corporate Career", "description": "Join a company in your field of study", "min_charisma": {"social": 7}},
        {"path": "Entrepreneurship", "description": "Start your own business venture", "min_charisma": {"social": 8, "academic": 6}},
        {"path": "Teaching", "description": "Share your knowledge with the next generation", "min_reputation": {"teachers": 60}},
        {"path": "Creative Industry", "description": "Pursue a career in arts, design, or media", "requirements": {"clubs": ["Art Club", "Music Club", "Drama Club"]}},
        {"path": "Public Service", "description": "Work for the government or non-profit organizations", "min_reputation": {"students": 50, "teachers": 50}}
    ]
    ...
    for career in careers:
        ...
        if "requirements" in career and "clubs" in career["requirements"]:
            if not any(club in player["clubs"] for club in career["requirements"]["clubs"]):
                eligible = False
...
def handle_graduation():
    ...
    social_score = min(100, player["reputation"]["students"] + player["charisma"]["social"] * 5)
    teacher_score = min(100, player["reputation"]["teachers"] + player["charisma"]["academic"] * 5)
    achievement_score = min(100, len(player["achievements"]) * 10)
    
    # Calculate overall success score (0-100)
    overall_score = (academic_score + social_score + teacher_score + achievement_score) / 4
    ...
    if overall_score >= 90:
        ...
        outcome = "Successful"
        description = "You've done well and have good career opportunities available to you."
        ...
        description = "You graduated, but may need to work extra hard to establish your career."
    ...
    slow_print(f"Achievements Unlocked: {len(player['achievements'])}/{len(achievements)}")
    ...
    if player["money"] >= 10000:
        career_paths.append({
            ...
            "outcome": "You'll build your own company and create opportunities for yourself and others."
        })
    ...
    slow_print("Based on your accomplishments, these career paths are available to you:")
    ...
    while attempt_count < max_attempts:
        try:
            ...
            slow_print(f"{Fore.RED}Input error encountered. Selecting a random career path.{Style.RESET_ALL}")
            chosen_path = random.choice(career_paths)
            break
        except Exception as e:
            slow_print(f"{Fore.RED}Error: {str(e)}. Selecting a random career path.{Style.RESET_ALL}")
            chosen_path = random.choice(career_paths)
            break
            
    # If all attempts failed, choose a random path
    if attempt_count >= max_attempts:
        slow_print(f"{Fore.RED}Too many invalid choices. Selecting a random career path.{Style.RESET_ALL}")
        chosen_path = random.choice(career_paths)
    ...
    if "Graduate School" in chosen_path['name']:
        slow_print("You completed your advanced degree with distinction. Your research has been published in prestigious journals, and you've received offers from multiple institutions.")
        if player["charisma"]["social"] >= 5:
            slow_print("You've also maintained your social connections from college, creating a supportive network of fellow academics.")
    
    elif "Corporate" in chosen_path['name']:
        slow_print("You've been promoted twice in five years, managing your own team and impressing executives with your leadership skills.")
        if player["gpa"] >= 3.0:
            slow_print("Your solid academic foundation has helped you master complex business challenges with ease.")
    
    elif "Research" in chosen_path['name']:
        slow_print("Your research has contributed to important advancements in your field. Colleagues respect your innovative approach and dedication.")
        if "internships" in player:
            slow_print("The connections you made during your internships have opened doors to collaborative projects with industry leaders.")
    
    elif "Teaching" in chosen_path['name']:
        slow_print("Your students consistently rate you as one of their favorite teachers. Your unique teaching style has made complex subjects accessible to many.")
        if player["reputation"]["students"] >= 50:
            slow_print("The social skills you developed in college have made you particularly effective at connecting with and motivating your students.")
    
    elif "Community" in chosen_path['name']:
        slow_print("You've become a respected voice in your community, advocating for important changes and bringing people together around common goals.")
        if player["money"] >= 5000:
            slow_print("Your financial stability has allowed you to dedicate more time to volunteer work and community projects.")
    
    elif "Job Offer" in chosen_path['name']:
        slow_print("You've thrived in your role, earning the respect of colleagues and superiors alike. Your prior internship experience gave you a valuable head start.")
        if player["charisma"]["academic"] >= 6:
            slow_print("Your strong academic foundation has helped you approach problems with depth and nuance that impresses everyone around you.")
    
    elif "Entrepreneurship" in chosen_path['name']:
        slow_print("Your business has grown steadily under your leadership. The skills you developed in college have served you well as an entrepreneur.")
        if player["reputation"]["teachers"] >= 60:
            slow_print("You've maintained relationships with your former professors, who have provided valuable advice and connections for your business.")
    
    elif "Family" in chosen_path['name']:
        slow_print(f"Your relationship with {player['romantic_interest']} has flourished. You've created a supportive partnership that brings out the best in both of you.")
        if player["charisma"]["social"] >= 7:
            slow_print("Your social skills have helped you build a wonderful community of friends around your family.")
    
    else:
        slow_print("The past five years have been full of growth and learning. Your college experience provided a strong foundation for navigating adult life.")
        slow_print("While there have been challenges along the way, you've faced them with the resilience and skills you developed during your campus years.")
    ...
    slow_print(f"{Fore.YELLOW}The lessons learned, friendships formed, and challenges overcome during your campus life continue to influence your journey.{Style.RESET_ALL}")
    ...
    if "Entrepreneurship" in chosen_path['name'] and "Business Mogul" not in player["achievements"]:
    ...
    slow_print("Use this password in the main menu for special options in your next game!")
    
    # Final thank you message
    slow_print(f"\n{Fore.CYAN}Thank you for playing Campus Life RPG!{Style.RESET_ALL}")
    slow_print("Your journey through school has concluded, but your life's adventure is just beginning.")
...
def advance_day():
    global current_date, homework, ticks
    ...
    if not is_weekend() and not is_holiday():
        ...
        for subject in player["electives"]:
            if subject in subjects and random.random() < subjects[subject]["homework_freq"]:
    ...
    day_type = "Weekend" if is_weekend() else "Holiday" if is_holiday() else "School Day"
    ...
    if player["hunger"] < 30:
        slow_print(f"{Fore.RED}You're very hungry! Find something to eat soon.{Style.RESET_ALL}")
    if player["energy"] < 30:
        slow_print(f"{Fore.RED}You're exhausted! You should rest soon.{Style.RESET_ALL}")
    if player["stress"] > 70:
        slow_print(f"{Fore.RED}You're feeling very stressed! Take some time to relax.{Style.RESET_ALL}")
...
def random_school_event():
    ...
    bullying_events = [
        {"text": "Someone threw a paper airplane with a rude message at you.", "severity": 1},
        {"text": "A classmate kept kicking the back of your chair.", "severity": 1},
        {"text": "Someone put an eraser on your chair when you stood up.", "severity": 1},
        {"text": "A group of students was whispering and laughing when you walked past.", "severity": 2},
        {"text": "Someone hid your textbook as a prank.", "severity": 2},
        {"text": "You found mean comments about you written in the bathroom.", "severity": 3},
        {"text": "Someone spread a false rumor about you.", "severity": 4},
        {"text": "Your personal belongings were damaged.", "severity": 4},
        {"text": "A group of students cornered you and mocked you relentlessly.", "severity": 5}
    ]
    ...
    if game_settings["allow_bullying"] and random.random() < 0.15:
        ...
        slow_print("\n{0}Event:{1} {2}".format(Fore.YELLOW, Style.RESET_ALL, filter_text(event_text, ["bullying"])))
        ...
        if random.random() < 0.20:
            ...
            mental_health["support_network"] = min(100, mental_health["support_network"] + 5)
    else:
        # Regular event
        ...
        if event == "You forgot your homework!":
            ...
            if subject_list:
                ...
                if subject in homework and homework[subject]:
                    slow_print("But wait! You actually did complete your homework for {}!".format(subject))
            ...
            if random.random() < 0.3:
                ...
                slow_print("It was {} who shared with you. Your relationship improved!".format(name))
            ...
            if subject_list:
                ...
                if subject in homework and homework[subject]:
                    ...
                    improve_grade = random.random() < 0.7  # 70% chance of grade improvement
                    if improve_grade and subject in player["grades"]:
                        ...
                        if current_grade != "A":  # Can't improve beyond A
                            ...
                            new_value = max(0, current_value - 1)  # Improve by one level
                            new_grade = rev_grade_values[new_value]
                            player["grades"][subject] = new_grade
                            slow_print("Your grade in {} improved to {}!".format(subject, new_grade))
                else:
                    ...
                    if worsen_grade and subject in player["grades"]:
                        ...
                        if current_grade != "F":  # Can't get worse than F
                            ...
                            new_grade = rev_grade_values[new_value]
                            player["grades"][subject] = new_grade
                            slow_print("Your grade in {} dropped to {}!".format(subject, new_grade))
            ...
            decrease_stress(7)
            player["mental_health"]["anxiety"] = max(0, player["mental_health"]["anxiety"] - 3)
        elif event == "Your friend tells you a funny joke":
            slow_print("You laugh and feel your mood improving.")
            decrease_stress(5)
            player["mental_health"]["happiness"] = min(100, player["mental_health"]["happiness"] + 5)
            ...
            if player["rumors"]:
                ...
                if spread_choice == 'y':
                    ...
                    if random.random() < 0.2:  # 20% chance of negative consequence
                        slow_print(f"{Fore.RED}Someone overheard you spreading the rumor and seems upset.{Style.RESET_ALL}")
...
def show_me():
    ...
    print(f"Current Date: {current_date.strftime('%Y-%m-%d')} ({'Weekend' if is_weekend() else 'Holiday' if is_holiday() else 'Weekday'})")
    ...
    print(f"{Fore.MAGENTA}School Year: {year_name} (Year {player['school_year']}){Style.RESET_ALL}")
    ...
    if player["completed_years"]:
        completed_years = ", ".join(str(year) for year in player["completed_years"])
    ...
    print(f"  Among Students: {player['reputation']['students']} ({player['rank']['students']})")
    print(f"  Among Teachers: {player['reputation']['teachers']} ({player['rank']['teachers']})")
    ...
    for stat, value in player["pe_stats"].items():
    ...
    if player["romantic_interest"]:
        ...
        if player["romantic_interest"] in relationship:
            ...
            print(f"  Romance Points: {player['romance_points']}/{ROMANCE_STAGES[min(stage+1, max(ROMANCE_STAGES.keys()))]['req']}")
    ...
    if player["part_time_job"]:
        print(f"\n{Fore.YELLOW}Part-time Job:{Style.RESET_ALL} {player['part_time_job']}")
    ...
    if homework:
        ...
        for subject, done in homework.items():
            status = f"{Fore.GREEN}✓ Done{Style.RESET_ALL}" if done else f"{Fore.RED}✗ Not Done{Style.RESET_ALL}"
...
def generate_family_members():
    ...
    last_names = ["Tanaka", "Yamamoto", "Sato", "Suzuki", "Watanabe", "Ito", "Nakamura", "Kobayashi", "Takahashi", "Kato", 
                  "Yoshida", "Sasaki", "Yamaguchi", "Matsumoto", "Inoue"]
    ...
    parent_personalities = ["caring", "strict", "relaxed", "ambitious", "traditional", "modern", "protective", "wise"]
    parent_occupations = ["teacher", "doctor", "office worker", "business owner", "chef", "artist", "engineer", "lawyer", 
                          "police officer", "writer", "scientist", "store manager"]
    
    mother_first_names = ["Akiko", "Haruka", "Yui", "Emi", "Sakura", "Misaki", "Hana", "Aiko", "Yumiko", "Naomi"]
    father_first_names = ["Kenji", "Hiroshi", "Takashi", "Akira", "Yuki", "Daichi", "Kazuki", "Satoshi", "Ryo", "Taro"]
    ...
    sibling_first_names_male = ["Sota", "Hayato", "Ryota", "Haruto", "Yuto", "Kota", "Sora", "Minato", "Ren", "Kaito"]
    sibling_first_names_female = ["Yuna", "Hinata", "Hina", "Koharu", "Mei", "Rin", "Miyu", "Aoi", "Ichika", "Sana"]
    
    sibling_personalities = ["friendly", "competitive", "nerdy", "athletic", "artistic", "shy", "outgoing", "rebellious", "studious"]
    ...
    for _ in range(num_siblings):
        ...
        # Determine age (younger or older)
        age_diff = random.randint(-5, 5)
        if age_diff == 0:  # Avoid same age (no twins)
        ...
        if sibling_gender == "M":
            sibling_name = f"{random.choice(sibling_first_names_male)} {family_last_name}"
            relation = f"{age_status} brother"
        else:
            sibling_name = f"{random.choice(sibling_first_names_female)} {family_last_name}"
...
NEURODIVERSITY_TRAITS = {
    ...
    "adhd": {
        ...
        "accommodations": ["structured environment", "frequent breaks", "note-taking assistance"]
    },
    "autism": {
        ...
        "strengths": ["pattern recognition", "detailed memory", "specialized knowledge", "honesty"],
        "accommodations": ["sensory accommodations", "clear instructions", "social skills support"]
    },
    "anxiety_disorder": {
        ...
        "accommodations": ["quiet test environment", "alternative presentations", "counseling support"]
    },
    "depression": {
        ...
        "accommodations": ["flexible deadlines", "counseling support", "absence accommodations"]
    }
}
...
def setup_game():
    global player, subjects, homework
    
    slow_print(f"Welcome to {TITLE}!")
    ...
    if random.random() < 0.02:
        ...
        slow_print(f"This gives you strengths in: {', '.join(player['neurodiversity']['strengths'])}")
        ...
        if accommodations_choice == "Y":
            ...
            slow_print(f"You have been granted: {', '.join(player['neurodiversity']['accommodations'])}")
    ...
    # Initialize clothing
    player["clothing"] = {
        "owned": ["School Uniform", "Casual Black T-shirt", "Casual Black Pants", "Pajamas"],
    ...
    slow_print(f"{Fore.CYAN}Dormitory{Style.RESET_ALL}: Close to campus, less commute time, more freedom. No family interaction.")
    slow_print(f"{Fore.CYAN}Home{Style.RESET_ALL}: Further from campus, more commute time, family interactions, less freedom.")
    ...
    while True:
        ...
        if choice == "1":
            ...
            for parent in family["parents"]:
                slow_print(f"Your {parent['relation']}, {parent['name']}, is a {parent['personality']} {parent['occupation']}.")
            
            if family["siblings"]:
                for sibling in family["siblings"]:
                    slow_print(f"You have a {sibling['personality']} {sibling['relation']}, {sibling['name']}.")
            ...
            for parent in family["parents"]:
                slow_print(f"Your {parent['relation']}, {parent['name']}, is a {parent['personality']} {parent['occupation']}.")
                slow_print(f"Initial relationship: {family_relationship[parent['name']]}/100")
            
            if family["siblings"]:
                slow_print("\nYour siblings:")
                for sibling in family["siblings"]:
                    slow_print(f"- {sibling['name']}, your {sibling['personality']} {sibling['relation']}.")
                    slow_print(f"  Initial relationship: {family_relationship[sibling['name']]}/100")
            else:
    ...
    slow_print("You can choose two elective subjects to study alongside your core subjects.")
    ...
    # Add core quests
    quests.extend([
        {"id": 1, "description": "Find your way around campus", "objective": "Visit 5 different locations", "status": "active", "reward": 10, "completed": False},
        {"id": 2, "description": "Meet your roommate", "objective": "Go to Student Room 364", "status": "active", "reward": 5, "completed": False},
        {"id": 3, "description": "Attend orientation", "objective": "Go to Classroom", "status": "active", "reward": 15, "completed": False},
    ])
    ...
    slow_print("Your journey begins now. Good luck with your studies and new life on campus!")
...
def handle_holiday_accommodation():
    ...
    if player["accommodation_type"] == "dorm" and is_holiday():
        ...
        if not player.get("holiday_mode", False):
            ...
            slow_print("This is a great time to strengthen your family relationships and relax!")
            ...
            for parent in player["family"]["parents"]:
                ...
                current_relationship = player["family_relationship"].get(parent["name"], 50)
                improvement = random.randint(5, 10)
                player["family_relationship"][parent["name"]] = min(100, current_relationship + improvement)
            
            # Siblings also welcome you if any
            if player["family"]["siblings"]:
                for sibling in player["family"]["siblings"]:
                    slow_print(f"{sibling['name']} seems {random.choice(['happy', 'excited', 'pleased'])} to see you.")
                    
                    # Improve sibling relationship
                    current_relationship = player["family_relationship"].get(sibling["name"], 50)
                    improvement = random.randint(3, 8)
                    player["family_relationship"][sibling["name"]] = min(100, current_relationship + improvement)
...
def show_academic_year():
    ...
    print(f"{Fore.MAGENTA}Current Year: {year_name} (Year {player['school_year']}){Style.RESET_ALL}")
    print(f"Year Progress: {player['year_progress']}%")
    print(f"GPA: {player['gpa']}")
    
    if player["completed_years"]:
        completed_years = ", ".join(str(year) for year in sorted(player["completed_years"]))
    ...
    if player["school_year"] == 4:
        print(f"{Fore.MAGENTA}This is your final year! Prepare for graduation.{Style.RESET_ALL}")
...
def go_location(args):
    ...
    destination_match = next((loc for loc in locations if loc.lower() == destination.lower()), None)
    ...
    if destination_match:
        ...
        if not clothing_appropriate:
            if destination_match in school_locations and is_weekday and player["clothing"]["wearing"] != "School Uniform":
            ...
            elif player["clothing"]["wearing"] == "Pajamas" and destination_match not in ["Student Room 364", "Your Bedroom"]:
                ...
                if response != 'y':
                    ...
                    slow_print(f"{Fore.RED}People are staring at you in your pajamas! Your reputation decreases.{Style.RESET_ALL}")
                    player["reputation"]["students"] = max(0, player["reputation"]["students"] - 5)
                    player["stress"] += 10
            elif destination_match == "Beach" and player["clothing"]["wearing"] != "Swimwear":
                # Beach without swimwear
                slow_print(f"Warning: {reason} You'll need proper swimwear to enjoy the beach activities.")
            elif destination_match == "Gym" and player["clothing"]["wearing"] not in ["Sporty Outfit", "School Uniform"]:
                ...
                if response != 'y':
                    ...
                    player["reputation"]["teachers"] = max(0, player["reputation"]["teachers"] - 2)
...
def sleep_command(args=None):
    global current_date, ticks, homework, player
    ...
    if args:
        try:
            ...
            if args[0].lower() == "overnight":
                ...
                print("Invalid sleep command. Use '/sleep [hours]' or '/sleep overnight'.")
                return
    else:
    ...
    if current_location not in valid_sleep_locations:
        print(f"You can't sleep here. Go to your {'Student Room 364' if player['accommodation_type'] == 'dorm' else 'Your Bedroom'} first.")
        return
    ...
    if sleep_overnight and player["clothing"]["wearing"] != "Pajamas":
        ...
        if "Pajamas" in player["clothing"]["owned"]:
            change_prompt = input("You need to wear pajamas to sleep properly overnight. Change into pajamas? (y/n): ").lower()
            if change_prompt == 'y':
                # Change into pajamas
                if player["clothing"]["wearing"]:
                    apply_clothing_effects(player["clothing"]["wearing"], apply=False)
            ...
            slow_print("You don't own any pajamas. You need to buy them from the Shopping Mall first.")
            return
    ...
    # Handle overnight sleep
        ...
        if current_hour >= bedtime_start or current_hour < bedtime_end or player["energy"] <= 30:  # Allow sleeping at night or when tired
            ...
            slow_print(f"{Fore.CYAN}You sleep soundly through the night and wake up refreshed.{Style.RESET_ALL}")
            ...
            if player["stress"] < 30:
                sleep_quality += 0.1
                slow_print("Your low stress levels contributed to a good night's sleep.")
            ...
            slow_print("It's too early to sleep for the night. You can take a nap instead using '/sleep [hours]'.")
            show_tutorial("sleep")
            return
        ...
        if player["accommodation_type"] == "home" and random.random() < 0.7:  # 70% chance
            have_family_breakfast()
    else:
        print(f"It's only {current_hour}:00. It's too early to sleep. Do something else until night time (after {bedtime_start}:00).")
...
def have_dinner():
    ...
    if current_location not in valid_dinner_locations:
        print(f"You can't have dinner here. Go to the {'Cafeteria' if player['accommodation_type'] == 'dorm' else 'Kitchen'} first.")
        return
    ...
    if not (dinner_start <= current_hour < dinner_end):
        print(f"It's not dinner time yet. Dinner is served between {dinner_start}:00 and {dinner_end}:00.")
        return
    ...
    if current_location == "Cafeteria":
        ...
        dinner_options = {
            "1": {"name": "Standard Meal Set", "price": 500, "hunger": 60, "energy": 20, "stress": -10},
            "2": {"name": "Deluxe Curry", "price": 800, "hunger": 80, "energy": 30, "stress": -15},
            "3": {"name": "Healthy Salad Bowl", "price": 600, "hunger": 50, "energy": 25, "stress": -20},
            "4": {"name": "Ramen Special", "price": 700, "hunger": 75, "energy": 25, "stress": -12}
        }
        ...
        if choice in dinner_options:
            ...
            if player["money"] < meal["price"]:
                print(f"You don't have enough money for that. You need ¥{meal['price']}.")
                return
            ...
            slow_print(f"Hunger +{meal['hunger']}, Energy +{meal['energy']}, Stress {meal['stress']}")
            slow_print(f"You spent ¥{meal['price']}. Remaining money: ¥{player['money']}")
        ...
        present_members = random.sample(family_members, min(len(family_members), random.randint(1, len(family_members))))
        
        slow_print(f"You join your family for dinner. Present: {', '.join([m['name'] for m in present_members])}")
        ...
        if choice == "1":
            ...
            for member in present_members:
                ...
                player["family_relationship"][name] = max(0, min(100, current_relationship + change))
            
            if random.random() < 0.3:  # 30% chance of comment
                commenter = random.choice(present_members)
                slow_print(f"{commenter['name']} asks if you're feeling alright since you're so quiet.")
...
def have_family_breakfast():
    ...
    # Update relationship
    current_relationship = player["family_relationship"].get(name, 50)
    player["family_relationship"][name] = max(0, min(100, current_relationship + relationship_change))
...
def analyze_student_response(student, target=None):
    ...
    response_types = {
        ...
        "strict": ["I only talk to serious students.", "Studies come first!", "What's your grade in Math?"],
        "kind": ["Nice to meet you!", "How are you doing?", "Want to study together?"],
        "serious": ["Have you done your homework?", "The exams are coming up...", "Let's focus on studies.", "Do you need help with anything?"],
        "lazy": ["Hey, what's up?", "Want to skip class?", "This is boring...", "Heya, let's chill"],
        
        # Anime-inspired personality types
        "tsundere": ["Don't get me wrong, I'm not talking to you because I like you or anything!", 
        ...
        "kuudere": ["Hello.", "I see you are here again.", 
                   "Your academic performance is satisfactory.", 
                   "I have noticed your efforts."],
        
        "dandere": ["Oh... h-hello there...", "I was just... um... reading this book...", 
                   "You want to t-talk to me...?", 
                   "*nods silently with a small smile*"],
        
        "deredere": ["Hi there! I'm so happy to see you!", 
    ...
    if target == "romance" and player["romantic_interest"] == student["name"]:
        ...
        default_romantic_responses = {
            0: ["I don't really know you that well...", "Oh, hi there.", "Did you need something?"],
            1: ["Nice to see you again!", "I was hoping to run into you.", "What's up?"],
            2: ["Hey, I was just thinking about you!", "I'm glad you came to talk to me.", "What have you been up to?"],
            3: ["I've been looking forward to seeing you!", "You always brighten my day.", "I was hoping we could spend some time together."],
            4: ["You know, I really enjoy spending time with you...", "There's something I've been wanting to tell you...", "Do you want to go somewhere together after school?"],
            5: ["Hey there, sweetie!", "I've missed you!", "Want to go on a date this weekend?"]
        }
        
        # Personality-specific romantic responses
        personality_romantic_responses = {
            "tsundere": {
                0: ["Why are you talking to me? I-it's not like I care!", "Don't think you're special or anything!", "I just happened to be here, okay?"],
                1: ["I noticed you around... not that I was looking!", "I guess talking to you isn't the worst...", "Don't get any weird ideas about us!"],
                2: ["I didn't save a seat for you or anything... it was just empty!", "I made extra lunch... you can have it if you want...", "You're... not completely terrible to be around."],
                3: ["I wasn't waiting for you! I just got here early!", "It's not like I made this specially for you... I just had extra!", "You're... kind of important to me... maybe."],
                4: ["Don't get me wrong, I don't like-like you or anything!", "I just happened to have two tickets... it would be a waste not to invite you.", "I... fine! I like spending time with you! Happy now?"],
                5: ["B-baka! Of course I like you! Why else would I be here?", "It's not like I love you or anything... I just... okay maybe I do.", "Don't make me say embarrassing things!"]
            },
            "kuudere": {
                0: ["Hello.", "I acknowledge your presence.", "You wish to speak with me?"],
                1: ["Your company is acceptable.", "I find your conversation... adequate.", "I would not object to further interaction."],
                2: ["I anticipated our meeting today.", "Your academic performance is impressive.", "I have prepared notes for our shared class."],
                3: ["Your absence was... noticeable.", "I have calculated a 73% compatibility between us.", "Your presence causes an unusual physiological response in me."],
                4: ["I have analyzed my feelings. The conclusion is... affection.", "My efficiency decreases by 23% when thinking of you. This is... troubling.", "I would like to establish a formal relationship."],
                5: ["I experience happiness in your presence.", "I have strong emotional attachments to you.", "My feelings for you are... significant."]
            },
            "dandere": {
                0: ["Oh... um... h-hi...", "*looks down shyly*", "*smiles nervously*"],
                1: ["I... um... it's nice to see you...", "*quietly offers you a small gift*", "*whispers* Thank you for talking to me..."],
                2: ["I was... hoping you might come by today...", "*writes you a note instead of speaking*", "*finally makes eye contact and smiles*"],
                3: ["I've been... thinking about you...", "*speaks more than usual when you're alone together*", "I... made something for you..."],
                4: ["Being with you makes me... happy...", "*manages a full conversation without looking away*", "I... really care about you..."],
                5: ["You make me feel safe enough to be myself...", "*initiates holding hands*", "*whispers* I love you..."]
            },
            "deredere": {
                0: ["Hi there! Nice to meet you!", "Wow, I'm so happy you came to talk to me!", "You seem really interesting!"],
                1: ["I was just thinking about you! How's your day?", "You always have the best energy! I love that!", "I've been hoping to run into you!"],
                2: ["You're amazing! I love spending time with you!", "Every day is better when I get to see you!", "You always make me smile so much!"],
                3: ["I've been thinking about you all day!", "You're the best thing in my life right now!", "I get so excited whenever I see you!"],
                4: ["My heart beats faster whenever you're near!", "I've never felt this way about anyone before!", "You mean everything to me!"],
                5: ["I love you so much! You're my everything!", "Every moment with you is magical!", "You make me the happiest person in the world!"]
            },
            "yandere": {
                0: ["Oh, you noticed me...", "I've been watching you for a while...", "You're talking to me? How wonderful..."],
                1: ["I saw you yesterday... you looked nice.", "I know your schedule by heart already.", "You don't need other friends, you know..."],
                2: ["I don't like when you talk to others...", "You belong with me, don't you think?", "I've been keeping track of everywhere you go..."],
                3: ["I made sure no one else will bother us today...", "I've cleared my schedule - and yours - so we can be together.", "No one will ever love you like I do. No one."],
                4: ["I've eliminated all distractions from your life.", "We're meant to be together forever.", "I can't live without you. And I won't let you live without me."],
                5: ["You're mine. Forever and ever and ever...", "I've made sure we'll never be apart again.", "I love you so much it hurts... Do you understand?"]
            },
            "genki": {
                0: ["Hey hey! Nice to meet you!", "Wow, this is exciting! Let's be friends!", "I love meeting new people! Especially ones as cool as you!"],
                1: ["Hey there! Ready for another awesome day?", "I have so much energy today! Let's do something fun!", "Life is an adventure, right? Let's enjoy it together!"],
                2: ["I was just about to go do something fun! Wanna come?", "You make every day more exciting!", "We make such a great team, don't we?"],
                3: ["I can't stop thinking about all the fun things we could do together!", "You're the highlight of my day, every day!", "Being with you makes everything twice as fun!"],
                4: ["My heart does a little dance whenever I see you!", "I've never met anyone who gets me like you do!", "Let's make every moment count together!"],
                5: ["You're my favorite person in the whole world!", "Life with you is like one big exciting adventure!", "I want to share all my happiness with you forever!"]
            },
            "himedere": {
                0: ["You may address me if you wish.", "I suppose I could spare you a moment of my time.", "Oh? You're approaching me?"],
                1: ["I acknowledge your persistent attempts to gain my favor.", "Your dedication to earning my attention is... noted.", "I shall permit you to accompany me occasionally."],
                2: ["You have shown yourself worthy of my continued acknowledgment.", "I find your company less tiresome than most.", "I have decided to include you in my inner circle."],
                3: ["Your absence is... noticeable to me.", "I have come to expect your presence.", "I shall allow you the honor of escorting me."],
                4: ["I find myself... thinking of you in your absence.", "You have somehow become important to me. How curious.", "I shall grant you the exclusive privilege of my affection."],
                5: ["You alone are worthy of my love.", "I have chosen you above all others.", "Consider yourself honored - you have captured the heart of royalty."]
            }
        }
        
        # Use personality-specific responses if available, otherwise default
        if personality in personality_romantic_responses and stage in personality_romantic_responses[personality]:
...
def interact_student(args):
    ...
    ex_partner = next((ex for ex in player["ex_partners"] if ex["name"].lower() == name.lower()), None)
    if ex_partner:
    ...
    if found:
        ...
        print("4. Give gift")
        if relationship[name] >= 40:  # Friend level required
            print("5. Ask to hang out")
            print("7. Ask for friendship")
        if relationship[name] >= 50:
            print("8. Ask for money")
        if relationship[name] >= 70 and not student_status.get(name, "").startswith(("Best Friend", "Dating")):
            print("6. Confess feelings")
        if relationship[name] >= 60:
            print("9. Ask for help with homework")
        if relationship[name] >= 80:
            print("10. Request a favor")
        
        # If they're your partner and NSFW is allowed, show intimate options
        if student_status.get(name, "") == "Dating" and game_settings.get("allow_nsfw", False):
        ...
        subject = "General Studies"  # Default value in case there are no current subjects
        current_grade = 60.0
        new_grade = 60.0
        
        # Get personality-specific relationship modifiers
        relationship_gain_mult = personalities[personality].get("relationship_gain", 3) / 3.0
        relationship_loss_mult = personalities[personality].get("relationship_loss", 3) / 3.0
        ...
        if choice == "1":  # Chat
            ...
            if personality == "tsundere" and relationship[name] < 30:
                # Tsundere initially gives fewer points but more later
                points_gain = int(base_points * 0.7)
                slow_print(f"{Fore.CYAN}(They seem reluctant to chat, but you sense they actually enjoy talking to you){Style.RESET_ALL}")
            elif personality == "dandere" and relationship[name] < 20:
                # Dandere takes time to open up
                points_gain = int(base_points * 0.6)
                slow_print(f"{Fore.CYAN}(They seem very shy, but appreciate your patience){Style.RESET_ALL}")
            elif personality == "yandere" and relationship[name] > 50:
                # Yandere becomes more intense at higher relationship levels
                points_gain = int(base_points * 1.5)
                slow_print(f"{Fore.RED}(They seem intensely focused on you...){Style.RESET_ALL}")
            else:
            ...
            if academic_focused:
                ...
                if personality == "lazy":
                    ...
                    slow_print("I-it's not like I'm helping you because I care! I just don't want you to fail...")
            ...
            if personality == "yandere":
                slow_print(f"Why do you care about {other_student['name']}? You should only be interested in me...")
                points_gain = -int(random.randint(3, 6) * relationship_loss_mult)
            # Himedere is dismissive of others
            elif personality == "himedere":
                slow_print(f"{other_student['name']}? A commoner not worth my time. Unlike you, who has better taste.")
                points_gain = int(random.randint(1, 3) * relationship_gain_mult)
            else:
            ...
            if player["money"] >= 500:
                ...
                if personality == "tsundere":
                    slow_print("W-why would you give me this? It's not like I wanted a gift... but... thank you.")
                    ...
                    slow_print("*looks surprised and blushes* F-for me? Thank you so much...")
                    points_gain = int(random.randint(7, 12) * relationship_gain_mult)
                elif personality == "deredere":
                    slow_print("Oh my gosh! A gift! I love it! You're the sweetest person ever!")
                    points_gain = int(random.randint(8, 15) * relationship_gain_mult)
                elif personality == "yandere":
                    slow_print("A gift from you? I'll keep it forever. I'll never let it go. Never.")
                    points_gain = int(random.randint(10, 20) * relationship_gain_mult)
                elif personality == "genki":
                    slow_print("Wow! This is so exciting! Thank you, thank you, thank you!")
                    points_gain = int(random.randint(7, 12) * relationship_gain_mult)
                elif personality == "himedere":
                    slow_print("Hmm, acceptable. I shall add it to my collection of tributes from admirers.")
                    points_gain = int(random.randint(5, 8) * relationship_gain_mult)
                else:
        ...
        elif choice == "5" and relationship[name] >= 40:  # Hang out
            if personality == "dandere":
                slow_print("*nods shyly* I'd... like that...")
                points_gain = int(random.randint(6, 10) * relationship_gain_mult)
            elif personality == "kuudere":
                slow_print("I find the prospect of spending time with you... satisfactory.")
                points_gain = int(random.randint(4, 7) * relationship_gain_mult)
            elif personality == "deredere" or personality == "genki":
                slow_print("Yes! I'd love to hang out with you! I know the perfect place!")
        ...
        elif choice == "6" and relationship[name] >= 70:  # Confess feelings
            # Romance compatibility affects success chance
            romance_compatibility = personalities[personality].get("romance_compatibility", 0.7)
            success_chance = (player["charisma"]["social"] / 100) * romance_compatibility
            
            if random.random() < success_chance:
                student_status[name] = "Dating"
                
                if personality == "tsundere":
                    slow_print(f"{Fore.GREEN}I... fine! I like you too! Are you happy now? Don't make me say it again!{Style.RESET_ALL}")
                elif personality == "kuudere":
                    slow_print(f"{Fore.GREEN}I have also developed romantic feelings for you. This is acceptable.{Style.RESET_ALL}")
                elif personality == "dandere":
                    slow_print(f"{Fore.GREEN}*blushes deeply* I... I've liked you for a long time... *smiles*{Style.RESET_ALL}")
                elif personality == "deredere":
                    slow_print(f"{Fore.GREEN}Yes! Yes! I love you too! I'm so happy right now!{Style.RESET_ALL}")
                elif personality == "yandere":
                    slow_print(f"{Fore.GREEN}Finally. You're mine now. Forever and ever...{Style.RESET_ALL}")
                elif personality == "genki":
                    slow_print(f"{Fore.GREEN}Wow! This is the best day ever! Of course I feel the same way!{Style.RESET_ALL}")
                elif personality == "himedere":
                    slow_print(f"{Fore.GREEN}I suppose I shall accept your feelings. Consider yourself fortunate.{Style.RESET_ALL}")
                else:
                    slow_print(f"{Fore.GREEN}They accepted your feelings!{Style.RESET_ALL}")
                ...
                if personality == "tsundere":
                    slow_print(f"{Fore.RED}W-what? Don't say such embarrassing things! I don't... I'm not ready...{Style.RESET_ALL}")
                elif personality == "kuudere":
                    slow_print(f"{Fore.RED}Your feelings are noted, but I cannot reciprocate at this time.{Style.RESET_ALL}")
                elif personality == "dandere":
                    slow_print(f"{Fore.RED}*looks down* I'm sorry... I'm not... I can't...{Style.RESET_ALL}")
                elif personality == "deredere":
                    slow_print(f"{Fore.RED}Oh! I'm so flattered, but I think we should stay friends for now.{Style.RESET_ALL}")
                elif personality == "yandere":
                    slow_print(f"{Fore.RED}Not yet. You need to prove yourself to me first. Show me how much you care...{Style.RESET_ALL}")
                elif personality == "himedere":
                    slow_print(f"{Fore.RED}You presume too much. One does not simply confess to royalty.{Style.RESET_ALL}")
                else:
                    slow_print(f"{Fore.RED}They want to remain friends...{Style.RESET_ALL}")
                    
                points_gain = -int(random.randint(5, 10) * relationship_loss_mult)
                
        elif choice == "7" and relationship[name] >= 40:  # Ask for friendship
            current_level = get_relationship_status(relationship[name])
            next_level = None
            ...
            if next_level:
                ...
                success_chance = min(0.85, relationship[name] / 100)
                ...
                if personality == "deredere" or personality == "genki":
                    ...
                    success_chance += 0.2 if player.get("romantic_interest") == name else -0.2
                
                if random.random() < success_chance:
                    if personality == "tsundere":
                        slow_print(f"{Fore.GREEN}Friends? I guess... if it's that important to you... Not that I care!{Style.RESET_ALL}")
                    elif personality == "kuudere":
                        slow_print(f"{Fore.GREEN}I acknowledge our bond has grown. Your friendship is acceptable.{Style.RESET_ALL}")
                    elif personality == "dandere":
                        slow_print(f"{Fore.GREEN}*smiles softly* I'd... really like that...{Style.RESET_ALL}")
                    elif personality == "deredere" or personality == "genki":
                        slow_print(f"{Fore.GREEN}Yes! We're totally {next_level.lower()}s now! This is amazing!{Style.RESET_ALL}")
                    elif personality == "yandere":
                        slow_print(f"{Fore.GREEN}Closer friends? Good. I want to be the closest person to you...{Style.RESET_ALL}")
                    elif personality == "himedere":
                        slow_print(f"{Fore.GREEN}I shall grant you the status of royal {next_level.lower()}. Consider it an honor.{Style.RESET_ALL}")
                    else:
                        slow_print(f"{Fore.GREEN}Of course! I'd love to be {next_level.lower()}s with you!{Style.RESET_ALL}")
                    ...
                    if next_level == "Best Friend":
                        ...
                        slow_print(f"Your relationship with {name} has advanced to {next_level}!")
                    
                    # Significant relationship boost
                    points_gain = int(random.randint(10, 20) * relationship_gain_mult)
                else:
                    slow_print(f"{Fore.RED}I don't think we're quite there yet. Let's spend more time together first.{Style.RESET_ALL}")
                    points_gain = -int(random.randint(3, 8) * relationship_loss_mult)
            else:
        ...
        elif choice == "8" and relationship[name] >= 50:  # Ask for money
            # Base chance depends on relationship and personality
            base_chance = min(0.6, relationship[name] / 120)
            ...
            min_amount = 500
            max_amount = int(min(5000, relationship[name] * 50))
            ...
            if random.random() < base_chance:
                ...
                # Track this request
                player["money_requests"][name] = player["money_requests"].get(name, 0) + 1
                
                if personality == "tsundere":
                    slow_print(f"{Fore.GREEN}Fine, take it! But don't think I'm doing this because I like you or anything!{Style.RESET_ALL}")
                elif personality == "kuudere":
                    slow_print(f"{Fore.GREEN}I see. Your financial situation is suboptimal. I can provide assistance this once.{Style.RESET_ALL}")
                elif personality == "dandere":
                    slow_print(f"{Fore.GREEN}*quietly hands over money* H-here... please use it well...{Style.RESET_ALL}")
                elif personality == "deredere":
                    slow_print(f"{Fore.GREEN}Of course I'll help you! What are friends for?{Style.RESET_ALL}")
                elif personality == "yandere":
                    slow_print(f"{Fore.GREEN}You need money? I'll give you whatever you need. Just stay with me.{Style.RESET_ALL}")
                elif personality == "himedere":
                    slow_print(f"{Fore.GREEN}A royal donation to help one of my subjects. Use it wisely.{Style.RESET_ALL}")
                else:
                    slow_print(f"{Fore.GREEN}Sure, I can help you out this time.{Style.RESET_ALL}")
                    
                slow_print(f"You received ¥{amount}!")
                points_gain = int(random.randint(2, 5) * relationship_gain_mult)
            else:
                if player.get("money_requests", {}).get(name, 0) > 0:
                    slow_print(f"{Fore.RED}You've already asked me for money before... I can't keep giving you money.{Style.RESET_ALL}")
                else:
                    slow_print(f"{Fore.RED}I'm sorry, I can't spare any money right now.{Style.RESET_ALL}")
                points_gain = -int(random.randint(5, 10) * relationship_loss_mult)

        elif choice == "9" and relationship[name] >= 60:  # Ask for help with homework
            # Get current subjects and homework
            current_subjects = get_current_subjects()
            incomplete_homework = [subject for subject in current_subjects if not homework.get(subject, False)]
            ...
            if not incomplete_homework:
                ...
                help_chance = min(0.9, relationship[name] / 100)
                ...
                if random.random() < help_chance:
                    ...
                    # Grade boost
                    grade_boost = random.uniform(1.0, 3.0)  # Bigger boost than studying alone
                    current_grade = player["grades"].get(subject, 60.0)
                    new_grade = min(100.0, current_grade + grade_boost)
                    player["grades"][subject] = new_grade
                    
                    if personality == "tsundere":
                        slow_print(f"{Fore.GREEN}Fine, I'll help you. But only because you'd obviously fail without me!{Style.RESET_ALL}")
                    elif personality == "kuudere":
                        slow_print(f"{Fore.GREEN}Your academic progress is important. I will assist you with this assignment.{Style.RESET_ALL}")
                    elif personality == "dandere":
                        slow_print(f"{Fore.GREEN}I'd be happy to help... I actually really enjoy {subject}...{Style.RESET_ALL}")
                    elif personality == "deredere":
                        slow_print(f"{Fore.GREEN}Homework together? That's so fun! Let's make it a study date!{Style.RESET_ALL}")
                    elif personality == "yandere":
                        slow_print(f"{Fore.GREEN}I'll help you with anything you need. Anything at all...{Style.RESET_ALL}")
                    elif personality == "genki":
                        slow_print(f"{Fore.GREEN}Let's tackle this homework together! We'll finish it in no time!{Style.RESET_ALL}")
                    else:
                        slow_print(f"{Fore.GREEN}Sure, I can help you with your {subject} homework.{Style.RESET_ALL}")
                    
                    slow_print(f"With {name}'s help, you completed your {subject} homework!")
                    slow_print(f"Your grade in {subject} has improved from {current_grade:.1f} to {new_grade:.1f}!")
                    points_gain = int(random.randint(5, 10) * relationship_gain_mult)
                else:
                    if personality == "kuudere" or personality == "serious":
                        slow_print(f"{Fore.RED}I believe in self-sufficiency. You should attempt this assignment on your own.{Style.RESET_ALL}")
                    elif personality == "lazy":
                        slow_print(f"{Fore.RED}Ugh, homework? I haven't even done mine. Sorry, can't help.{Style.RESET_ALL}")
                    else:
                        slow_print(f"{Fore.RED}I'd like to help, but I'm not very good at {subject} myself.{Style.RESET_ALL}")
                    points_gain = -int(random.randint(2, 5) * relationship_loss_mult)
        
        elif choice == "10" and relationship[name] >= 80:  # Request a favor
            ...
            if favor_choice == "5":
                ...
                base_chance = min(0.8, relationship[name] / 100)
                ...
                if favor_choice == "1":  # Reputation boost
                    if random.random() < base_chance:
                        ...
                        slow_print(f"{Fore.GREEN}{name} speaks highly of you to other students, boosting your reputation!{Style.RESET_ALL}")
                        slow_print(f"Student Reputation +{rep_increase}")
                        
                        # Track favor
                        player["favors_asked"][name] = player["favors_asked"].get(name, 0) + 1
                        points_gain = int(random.randint(3, 7) * relationship_gain_mult)
                    else:
                        slow_print(f"{Fore.RED}I'm not sure I have that kind of influence with other students...{Style.RESET_ALL}")
                        points_gain = -int(random.randint(3, 6) * relationship_loss_mult)
                    ...
                    for student in students:
                        ...
                        if other_name != name and other_name != player["name"]:
                            if other_name not in player["relationships"] or player["relationships"][other_name] < 20:
                                potential_introductions.append(student)
                    
                    if not potential_introductions or random.random() >= base_chance:
                        slow_print(f"{Fore.RED}I don't really know anyone you haven't met already...{Style.RESET_ALL}")
                        points_gain = -int(random.randint(2, 5) * relationship_loss_mult)
                    else:
                        ...
                        slow_print(f"{Fore.GREEN}{name} introduces you to {intro_name}!{Style.RESET_ALL}")
                        slow_print(f"You have a pleasant first conversation with {intro_name}.")
                    ...
                    if random.random() < base_chance:
                        ...
                        player["intelligence"] = min(100, player["intelligence"] + int_boost)
                        ...
                        if current_subjects:
                            ...
                            slow_print(f"{Fore.GREEN}{name} agrees to help with your project! Their assistance is invaluable.{Style.RESET_ALL}")
                            slow_print(f"Intelligence +{int_boost}")
                            slow_print(f"Your grade in {subject} improved from {current_grade:.1f} to {new_grade:.1f}!")
                        else:
                            slow_print(f"{Fore.GREEN}{name} agrees to help with your project! Their assistance is invaluable.{Style.RESET_ALL}")
                            slow_print(f"Intelligence +{int_boost}")
                        ...
                        slow_print(f"{Fore.RED}I'm really swamped with my own projects right now. Maybe next time?{Style.RESET_ALL}")
                        points_gain = -int(random.randint(3, 7) * relationship_loss_mult)
                    ...
                    if not player["bully_encounter"]:
                        slow_print("There isn't currently anyone you need protection from.")
                        points_gain = 0
                    elif random.random() < base_chance:
                        slow_print(f"{Fore.GREEN}{name} agrees to stand up for you the next time someone gives you trouble!{Style.RESET_ALL}")
                        slow_print("You feel more confident knowing you have their support.")
                        ...
                        slow_print(f"{Fore.RED}I... I'm not sure I can help with that. Confrontation isn't really my thing...{Style.RESET_ALL}")
                        points_gain = -int(random.randint(4, 8) * relationship_loss_mult)
        
        # Rumor/gossip interaction options
        elif choice == "11" and student_status.get(name, "") == "Dating" and game_settings.get("allow_nsfw", False):
            # Process NSFW content based on relationship stage
            if relationship[name] >= 90:  # Close bond required
                slow_print(f"{Fore.MAGENTA}You and {name} spend some private time together...{Style.RESET_ALL}")
                
                if personality == "shy" or personality == "dandere":
                    slow_print(f"{name} blushes deeply but doesn't resist as you move closer...")
                elif personality == "genki" or personality == "deredere":
                    slow_print(f"{name} enthusiastically embraces you, pulling you closer...")
                elif personality == "yandere":
                    slow_print(f"{name}'s eyes gleam with intense passion as they pull you into their arms...")
                else:
        ...
        elif choice in ["12", "13", "14"] and relationship[name] >= 50:
            ...
            points_gain = handle_rumor_interaction(choice, name, personality, relationship_gain_mult, relationship_loss_mult)
        ...
        student_status[name] = new_status if not student_status[name].startswith("Dating") else student_status[name]
...
def show_students():
    print("\n--- Classmates ---")
    for student in students:
        print(f"{student['name']} ({student['personality']})")
...
def complete_quest(args):
    ...
    if quest and not quest['completed']:
        ...
        slow_print(f"Quest '{quest['description']}' completed! You earned {quest['reward']} relationship points.")
        for student in students:
            if student['name'] == quest['description'].split()[1]:  # E.g., Haruki
                if student['name'] not in relationship:
                    relationship[student['name']] = 0
                relationship[student['name']] += quest['reward']
    ...
    if club_name in player["clubs"]:
        ...
        for subject, bonus in benefits.items():
            if subject in subjects:
                if not homework.get(subject, False):
                    homework[subject] = True
                    slow_print(f"You worked on your {subject} homework with club members!")
        ...
        if "charisma" in benefits:
            for skill, value in benefits["charisma"].items():
                ...
                slow_print(f"Your {skill} skills improved by being at the club! (+{value})")
        
        # Reduce stress
        if "stress_reduction" in benefits:
            reduction = benefits["stress_reduction"]
            player["stress"] = max(0, player["stress"] - reduction)
            slow_print(f"Spending time at the club helped you relax! (-{reduction} stress)")
        
        # Consume energy
        if "energy_cost" in benefits:
            player["energy"] = max(0, player["energy"] - benefits["energy_cost"])
            slow_print(f"Club activities made you tired. (-{benefits['energy_cost']} energy)")
        
        # Chance to become president if not already and relationship with current president is high
        if position != "President" and president in relationship and relationship[president] >= 80:
            if random.random() < 0.1:  # 10% chance
                clubs[club_name]["president"] = player["name"]
                player["club_positions"][club_name] = "President"
                slow_print(f"{Fore.GREEN}Congratulations! You've been elected as the new President of {club_name}!{Style.RESET_ALL}")
                
                # Unlock achievement
                if "Club Leader" not in player["achievements"]:
                    player["achievements"].append("Club Leader")
                    slow_print(f"{Fore.YELLOW}Achievement unlocked: Club Leader{Style.RESET_ALL}")
    else:
        # Not a member yet
        slow_print(f"{Fore.CYAN}You visit the {club_name}.{Style.RESET_ALL}")
        slow_print(f"The club is currently in session. {clubs[club_name]['president']} is leading the activities.")
        ...
        if join == 'y':
            if len(player["clubs"]) < 2:  # Limit to 2 clubs
                ...
                slow_print(f"{Fore.GREEN}You are now a member of the {club_name}!{Style.RESET_ALL}")
                player["reputation"]["students"] += 5
                update_ranks()
                ...
                if len(player["clubs"]) == 1 and "Club Member" not in player["achievements"]:
                    ...
                    slow_print(f"{Fore.YELLOW}Achievement unlocked: Club Member{Style.RESET_ALL}")
            else:
                slow_print(f"{Fore.RED}You can only join a maximum of 2 clubs.{Style.RESET_ALL}")
                leave_club = input("Would you like to leave one of your current clubs? (y/n): ").lower()
                if leave_club == 'y':
                    print("Your current clubs:")
                    for i, club in enumerate(player["clubs"], 1):
                        print(f"{i}. {club}")
                    choice = input("Enter the number of the club you want to leave (or 0 to cancel): ")
...
def pe_class_challenge():
    ...
    slow_print("{0}Description: {1}{2}".format(
    ...
    # Challenge success is based on player's energy, PE stats, social charisma, and some randomness
    pe_skill_factor = (player["pe_stats"]["technique"] / 10) * 0.2 + (player["pe_stats"]["stamina"] / 10) * 0.2
    success_chance = (player["energy"] / 100) * 0.4 + pe_skill_factor + (player["charisma"]["social"] / 10) * 0.1 + random.random() * 0.1
    ...
    if random.random() < 0.3:  # 30% chance
        slow_print("\n{0}=== After the Challenge ==={1}".format(
            Fore.MAGENTA, Style.RESET_ALL))
        ...
        if result == "great_success":
            ...
            slow_print("\"That was amazing! Do you play {0} outside of school too?\"".format(
            ...
            slow_print("You notice {0} giving you an approving nod after the challenge.".format(student_name))
            ...
            slow_print("\"Don't worry about it. We all have off days. You'll do better next time!\"")
...
def romance_opportunity():
    ...
    if player["romantic_interest"] and player["romantic_interest"] in relationship and relationship[player["romantic_interest"]] > 75:
    ...
    for student in students:
        ...
        if student["gender"] != player["gender"]:
            ...
            if student["name"] in relationship and relationship[student["name"]] >= 30:
                eligible_students.append(student)
            elif student["personality"] in ["kind", "serious"] and random.random() < 0.3:
                eligible_students.append(student)
    ...
    if rel_points < 40:  # Early stage - initial interest
        ...
        if choice == "1":
            ...
            slow_print(f"You look away. When you glance back, {name} seems disappointed.")
        ...
        choice = input("Choose an option (1-4): ")
        if choice == "1":
            slow_print(f"You enthusiastically accept {name}'s gesture. They seem very happy.")
            ...
            slow_print(f"You politely decline. {name} tries to hide their disappointment.")
        ...
        if not player["romantic_interest"]:
            ...
            if choice == "1":
                ...
                if "Romance Blooms" not in player["achievements"]:
                    ...
                    slow_print(f"{Fore.YELLOW}Achievement unlocked: Romance Blooms{Style.RESET_ALL}")
            
            elif choice == "2":
                slow_print(f"You decide to wait. {name} seems a bit uncertain about your feelings.")
                relationship[name] += random.randint(2, 5)
            
            elif choice == "3":
                slow_print(f"You keep things friendly. {name} seems slightly disappointed.")
                ...
                success_chance = (player["charisma"]["social"] / 50) + random.random() * 0.5
                
                if success_chance > 0.6:
                    slow_print(f"{Fore.GREEN}{name} feels the same way! You're now dating!{Style.RESET_ALL}")
                    ...
                    slow_print(f"{Fore.RED}{name} appreciates your feelings but wants to remain friends for now.{Style.RESET_ALL}")
        ...
        relationship[name] += random.randint(5, 10)  # Always gain points when in a relationship
        player["stress"] = max(0, player["stress"] - random.randint(5, 10))  # Reduce stress
...
def play_festival_minigame(minigame_type):
    ...
    if minigame_type == "poetry_contest":
        ...
        slow_print("Create a poem by selecting words that fit the theme of spring and cherry blossoms.")
        ...
        word_sets = {
            "nouns": ["blossoms", "spring", "breeze", "petals", "sunlight", "branches", "dawn", "dreams", "path", "garden"],
            "verbs": ["floating", "dancing", "falling", "blooming", "whispering", "awakening", "drifting", "embracing", "shimmering"],
            "adjectives": ["gentle", "pink", "delicate", "fleeting", "serene", "peaceful", "radiant", "ephemeral", "tender"]
        }
        ...
        final_poem = poem_template.format(
            adj1=poem_words[4], adj2=poem_words[5], 
            noun1=poem_words[0], noun2=poem_words[1],
            verb1=poem_words[2], verb2=poem_words[3]
        )
        ...
        theme_words = {
            "beauty": ["delicate", "radiant", "pink", "gentle", "petals", "blossoms", "shimmering"],
            "nature": ["garden", "branches", "breeze", "spring", "blooming", "petals", "sunlight"],
            "transience": ["fleeting", "ephemeral", "falling", "drifting", "dreams", "tender"],
            "renewal": ["awakening", "blooming", "dawn", "spring", "path", "embracing", "new"],
            "pink petals": ["pink", "petals", "blossoms", "floating", "falling", "delicate", "dancing"]
        }
        ...
        if score >= 70:
            slow_print(f"{Fore.GREEN}Congratulations! Your poem won first place!{Style.RESET_ALL}")
            reward_money = 1000
            stress_reduction = 20
            charisma_gain = 2
        elif score >= 50:
            slow_print(f"{Fore.CYAN}Well done! Your poem won second place!{Style.RESET_ALL}")
            reward_money = 500
            stress_reduction = 15
            charisma_gain = 1
        else:
        ...
        slow_print(f"You earned ¥{reward_money}, reduced stress by {stress_reduction}, and gained {charisma_gain} academic charisma!")
    ...
    # Goldfish Scooping Minigame (Summer Cultural Festival)
        ...
        slow_print("You have a paper scoop (poi) to catch as many goldfish as you can before it breaks.")
        ...
        slow_print(f"Your paper scoop has {scoop_health}% durability. Catch as many goldfish as you can!")
        ...
        while scoop_health > 0 and total_attempts < max_attempts:
            ...
            if random.random() < success_chance:
                ...
                slow_print(f"{Fore.GREEN}Success! You caught {fish_caught} goldfish!{Style.RESET_ALL}")
            else:
        ...
        slow_print(f"You earned ¥{reward_money}, reduced stress by {stress_reduction}, and gained {social_charisma} social charisma!")
        
        # Special prize for high performance
        if caught_fish >= 8:
            slow_print(f"{Fore.YELLOW}The stall owner gives you a special prize for your excellent performance!{Style.RESET_ALL}")
    ...
    # Tanzaku Wish Minigame (Tanabata Festival)
        ...
        wish_elements = {
            "Academic Success": ["studying", "learning", "tests", "homework", "grades", "knowledge", "wisdom"],
            "Romantic Love": ["romance", "love", "dating", "partner", "relationship", "heart", "feelings"],
            "Friendship": ["friends", "bonds", "trust", "memories", "support", "fun", "laughter"],
            "Health": ["body", "mind", "spirit", "strength", "energy", "healing", "balance"],
            "Wealth": ["money", "fortune", "abundance", "success", "prosperity", "career", "opportunity"],
            "Personal Growth": ["growth", "skills", "talent", "confidence", "courage", "creativity", "passion"]
        }
        ...
        slow_print(f"{Fore.MAGENTA}The night sky seems to twinkle in response to your wish!{Style.RESET_ALL}")
        ...
        if chosen_category == "Academic Success":
            ...
            slow_print("Your heart feels lighter! (+5 points with all romantic interests)")
        elif chosen_category == "Friendship":
            player["charisma"]["social"] += 2
            player["reputation"]["students"] += 10
            slow_print("You feel more connected to your friends! (+2 Social Charisma, +10 Student Reputation)")
            ...
            player["charisma"]["social"] += 1
            player["pe_stats"]["technique"] += 1
            slow_print("You feel a surge of personal growth! (+1 to Academic, Social, and Technique)")
    ...
    # Bean Throwing Minigame (Setsubun Festival)
        ...
        while beans_left > 0:
            ...
            while demon_health > 0 and beans_left > 0:
                ...
                if random.random() < hit_chance:
                    demon_health -= damage
                    if choice == "3":
                        slow_print(f"{Fore.YELLOW}'Oni wa soto, fuku wa uchi!' (Demons out, luck in!){Style.RESET_ALL}")
                    slow_print(f"{Fore.GREEN}Hit! You dealt {damage} damage to the {current_demon}!{Style.RESET_ALL}")
                else:
                    slow_print(f"{Fore.RED}Miss! The {current_demon} dodged your beans!{Style.RESET_ALL}")
                
                if demon_health <= 0:
                    slow_print(f"{Fore.GREEN}You defeated the {current_demon}!{Style.RESET_ALL}")
                    oni_defeated += 1
                    break
                
                if beans_left <= 0:
                    slow_print(f"{Fore.RED}You're out of beans! The {current_demon} escapes!{Style.RESET_ALL}")
                    break
            ...
            if beans_left > 0:
                continue_choice = input(f"You have {beans_left} beans left. Continue throwing? (y/n): ")
        ...
        if oni_defeated >= 5:
            slow_print(f"{Fore.YELLOW}Achievement Unlocked: Oni Vanquisher{Style.RESET_ALL}")
            player["festival_achievements"].append("Oni Vanquisher")
            player["reputation"]["students"] += 10
            slow_print("Your victory against the oni has impressed your fellow students! (+10 Student Reputation)")
    
    # Bon Odori Dance Minigame (Obon Festival)
    elif minigame_type == "bon_odori":
        slow_print("Welcome to the Bon Odori Dance Circle!")
        slow_print("Follow the rhythm and dance patterns to honor the spirits of ancestors.")
        
        # Game parameters
        dance_moves = ["Step Right", "Step Left", "Turn", "Clap", "Bow", "Raise Arms", "Wave Fan"]
        dance_sequence = []
        player_sequence = []
        sequence_length = 3
        max_rounds = 5
        ...
        while current_round <= max_rounds:
            ...
            dance_sequence = [random.choice(dance_moves) for _ in range(sequence_length)]
            ...
            # Check accuracy
            correct_moves = sum(1 for p, d in zip(player_sequence, dance_sequence) if p == d)
            round_score = (correct_moves / sequence_length) * 100
            score += round_score
            
            # Feedback
            slow_print(f"\n{Fore.YELLOW}Original sequence: {', '.join(dance_sequence)}{Style.RESET_ALL}")
            slow_print(f"{Fore.CYAN}Your sequence: {', '.join(player_sequence)}{Style.RESET_ALL}")
            slow_print(f"You got {correct_moves}/{sequence_length} moves correct ({round_score:.0f}% accuracy)!")
            
            if correct_moves == sequence_length:
                slow_print(f"{Fore.GREEN}Perfect! The spirits are pleased with your dance!{Style.RESET_ALL}")
            elif correct_moves >= sequence_length / 2:
                slow_print(f"{Fore.YELLOW}Good effort! The spirits appreciate your dance.{Style.RESET_ALL}")
            else:
                slow_print(f"{Fore.RED}The spirits seem a bit confused by your dance...{Style.RESET_ALL}")
        ...
        if final_score >= 80:
            ...
            stress_reduction = 25
            social_charisma = 3
            slow_print(f"{Fore.GREEN}The spirits were deeply moved by your dance!{Style.RESET_ALL}")
        elif final_score >= 60:
            energy_bonus = 20
            stress_reduction = 15
            social_charisma = 2
            slow_print(f"{Fore.YELLOW}Your dance honored the ancestors well.{Style.RESET_ALL}")
        else:
            energy_bonus = 10
            stress_reduction = 10
            social_charisma = 1
        ...
        slow_print(f"Dancing gave you energy (+{energy_bonus}) and reduced your stress (-{stress_reduction})!")
        slow_print(f"Your dancing improved your social charisma (+{social_charisma})!")
        
        # Special achievement for high performance
        if final_score >= 90:
            slow_print(f"{Fore.YELLOW}Achievement Unlocked: Spirit Dancer{Style.RESET_ALL}")
            player["festival_achievements"].append("Spirit Dancer")
    
    # Implement other minigames as needed
    else:
        slow_print(f"The {minigame_type.replace('_', ' ').title()} minigame is still being prepared for the festival!")
...
def interact_ex_partner(ex_partner):
    ...
    if choice == "1":  # Chat about feelings
        ...
        if status == "moved_on":
            slow_print(f"{name}: I appreciate your maturity in talking about this. I've moved on, and I hope you're doing well too.")
            # Small chance of status change
            if random.random() < 0.1:
                ex_partner["status"] = "still_interested"
                slow_print(f"{Fore.CYAN}(You notice a hint of lingering feelings in their eyes){Style.RESET_ALL}")
                
        elif status == "still_interested":
            slow_print(f"{name}: It's... nice to talk about what we had. I still think about us sometimes.")
            
            # Chance they might make a move
            if random.random() < 0.3:
                slow_print(f"{name} moves closer to you, hesitating as if wanting to say more.")
                respond = input("Encourage them to speak? (Y/N): ").upper()
                
                if respond == "Y":
                    slow_print(f"{name}: I still have feelings for you. I've been hoping we might try again someday.")
                    reconcile = input("Suggest reconciliation? (Y/N): ").upper()
                    
                    if reconcile == "Y":
                        # Similar logic to reconciliation option
                        if player["romantic_interest"]:
                            slow_print(f"You're currently in a relationship with {player['romantic_interest']}.")
                            break_up = input(f"End your relationship with {player['romantic_interest']}? (Y/N): ").upper()
                            ...
                            if break_up == "Y":
                                ...
                                slow_print(f"{Fore.GREEN}You and {name} decide to give your relationship another chance.{Style.RESET_ALL}")
                                slow_print(f"You've broken up with {current} to be with {name} again.")
                            else:
                                slow_print(f"You tell {name} that you're committed to your current relationship.")
                                slow_print(f"{name} looks disappointed but nods in understanding.")
                            ...
                            slow_print(f"{Fore.GREEN}You and {name} are back together!{Style.RESET_ALL}")
                    else:
                        slow_print(f"You gently tell {name} that you value them but aren't looking to get back together.")
                    ...
                    slow_print(f"You keep the conversation casual. {name} seems slightly disappointed.")
            
        elif status == "angry":
            slow_print(f"{name}: *coldly* I don't really want to talk about our past. It's better left behind us.")
            
            # Chance of status improvement
            if random.random() < 0.2:
                ex_partner["status"] = "moved_on"
                slow_print(f"{Fore.CYAN}(Despite their words, they seem less angry than before){Style.RESET_ALL}")
                
        elif status == "yandere":
            slow_print(f"{name}: *intense stare* Our relationship isn't in the past. It's just on pause. We'll be together again... one way or another.")
            slow_print(f"{Fore.RED}Their intensity makes you uncomfortable.{Style.RESET_ALL}")
            
            # Increase mental health stress
            player["mental_health"]["anxiety"] = min(100, player["mental_health"]["anxiety"] + 5)
            increase_stress(5)
            
    elif choice == "2":  # Apologize
        slow_print(f"You sincerely apologize to {name} for any hurt you caused in your relationship.")
        
        if status == "moved_on":
            slow_print(f"{name}: I appreciate that, thank you. It's all water under the bridge now.")
            ...
            slow_print(f"{name}: Your apology means a lot to me. I've never stopped caring about you.")
                ...
                if respond == "Y":
                    ...
                    if player["romantic_interest"]:
                        ...
                        if break_up == "Y":
                            ...
                            slow_print(f"You tell {name} that while you care about them, you're committed to your current relationship.")
                            slow_print(f"{name} looks heartbroken but nods in understanding.")
                    ...
                    slow_print(f"You tell {name} that you value their forgiveness but think it's best to move forward separately.")
            ...
            if random.random() < 0.6:
                slow_print(f"{name}: *sighs* I guess I appreciate the apology. It's been a while anyway.")
                ex_partner["status"] = "moved_on"
                slow_print(f"{Fore.GREEN}{name} seems to have let go of some anger toward you.{Style.RESET_ALL}")
            else:
                slow_print(f"{name}: An apology doesn't change what happened. I need more time.")
                slow_print(f"{Fore.YELLOW}{name} is still upset, but your apology didn't make things worse.{Style.RESET_ALL}")
                
        elif status == "yandere":
            slow_print(f"{name}: *smiles unsettlingly* There's nothing to apologize for. We'll be together again soon anyway.")
            slow_print(f"{Fore.RED}Their response makes you deeply uncomfortable.{Style.RESET_ALL}")
            player["mental_health"]["anxiety"] = min(100, player["mental_health"]["anxiety"] + 7)
            increase_stress(8)
            
    elif choice == "3":  # Suggest friendship
        slow_print(f"You suggest to {name} that you could try to be friends despite your history.")
        ...
        if status == "moved_on":
            ...
            slow_print(f"{Fore.GREEN}You and {name} have established a friendship.{Style.RESET_ALL}")
            
        elif status == "still_interested":
            slow_print(f"{name}: *hesitates* I can try, but it might be difficult for me to see you just as a friend.")
            ...
            slow_print(f"{Fore.YELLOW}You and {name} are working on building a friendship, though it may be complicated.{Style.RESET_ALL}")
            
        elif status == "angry":
            # Chance of accepting friendship
            if random.random() < 0.4:
                slow_print(f"{name}: *considers* I suppose we could try being civil. Baby steps.")
                ...
                slow_print(f"{Fore.YELLOW}{name} is cautiously open to rebuilding a connection.{Style.RESET_ALL}")
            else:
                slow_print(f"{name}: I don't think that's a good idea right now. I need more space.")
                slow_print(f"{Fore.RED}{name} isn't ready for friendship yet.{Style.RESET_ALL}")
                
        elif status == "yandere":
            slow_print(f"{name}: *laughs inappropriately* Friends? No, no. We're meant to be so much more than friends.")
            slow_print(f"{Fore.RED}They step uncomfortably close to you.{Style.RESET_ALL}")
            slow_print("\"Friends is such a weak word for what we are to each other.\"")
            
            player["mental_health"]["anxiety"] = min(100, player["mental_health"]["anxiety"] + 10)
            increase_stress(10)
            
    elif choice == "4" and status in ["moved_on", "still_interested"]:  # Reconciliation
        slow_print(f"You suggest to {name} that you might want to try your relationship again.")
        ...
        if random.random() < reconcile_chance:
            ...
            if status == "moved_on":
                slow_print(f"{name}: *surprised* Really? I... I've thought about it too, actually.")
            ...
            if player["romantic_interest"]:
                ...
                if break_up == "Y":
                    ...
                    slow_print(f"You tell {name} that you can't end your current relationship.")
                    slow_print(f"{name} looks deeply hurt. \"So why did you bring this up?\"")
                ...
                slow_print(f"{Fore.GREEN}You and {name} are back together!{Style.RESET_ALL}")
                decrease_stress(15)
                player["mental_health"]["happiness"] = min(100, player["mental_health"]["happiness"] + 15)
        else:
            # They reject reconciliation
            if status == "moved_on":
                slow_print(f"{name}: I'm sorry, but I've moved on. I think it's best if we both keep moving forward separately.")
            else:  # still_interested
                slow_print(f"{name}: I still care about you, but I'm not sure going back is the right choice. I need more time.")
            ...
            slow_print(f"{Fore.RED}Your reconciliation attempt was unsuccessful.{Style.RESET_ALL}")
            increase_stress(10)
            player["mental_health"]["self_esteem"] = max(0, player["mental_health"]["self_esteem"] - 5)
            
    elif choice == "5" and status == "yandere":  # Therapy option for yandere
        slow_print(f"{Fore.CYAN}You carefully suggest to {name} that their feelings seem very intense, and that talking to someone might help.{Style.RESET_ALL}")
        
        # Success chance starts low and improves with repeated attempts
        therapy_attempts = ex_partner.get("therapy_attempts", 0)
        base_success = 0.1 + (therapy_attempts * 0.1)  # Each attempt improves chance by 10%
        ...
        if random.random() < success_chance:
            ...
            slow_print(f"{Fore.GREEN}{name} appears vulnerable for a moment. \"Maybe you're right. I haven't been feeling like myself.\"")
            slow_print(f"They agree to speak with the campus counselor.{Style.RESET_ALL}")
            ...
            if random.random() < 0.7:
                ...
                slow_print(f"{Fore.YELLOW}After their first session, {name} seems slightly less obsessive.{Style.RESET_ALL}")
                
            # Reduce player anxiety
            player["mental_health"]["anxiety"] = max(0, player["mental_health"]["anxiety"] - 10)
            decrease_stress(15)
            ...
            if therapy_attempts == 0:
                # First rejection
                slow_print(f"{Fore.RED}{name} becomes defensive. \"There's nothing wrong with me! I just love you too much!\"{Style.RESET_ALL}")
                slow_print("They storm off, clearly upset by your suggestion.")
            elif therapy_attempts < 3:
                # Subsequent rejections
                slow_print(f"{Fore.RED}{name}: \"You're trying to change me, aren't you? Don't you see how perfect we are together?\"{Style.RESET_ALL}")
                slow_print("They refuse again, but seem slightly less defensive than before.")
            else:
                # After multiple attempts
                slow_print(f"{Fore.YELLOW}{name} pauses. \"You keep bringing this up... do you really think I need help?\"{Style.RESET_ALL}")
                slow_print("They don't agree, but they're starting to question their behavior.")
    ...
    if ex_partner.get("in_therapy", False):
        ...
        if therapy_sessions % 3 == 0:
            ...
            if random.random() < improvement_chance:
                if status == "yandere":
                    ex_partner["status"] = "angry"
                    slow_print(f"{Fore.GREEN}Their obsessive behaviors have decreased significantly.{Style.RESET_ALL}")
                elif status == "angry":
                    ex_partner["status"] = "moved_on"
                    slow_print(f"{Fore.GREEN}They seem much more at peace with the past now.{Style.RESET_ALL}")
                    
                    # Chance to become friends
                    if random.random() < 0.6:
                        if name not in player["relationships"]:
                            player["relationships"][name] = 35
                        slow_print(f"{name} mentions they'd like to try being friends again.")
...
def generate_random_rumor():
    ...
    if rumor_type == "romantic":
        ...
        if len(students) >= 2:
            ...
            content = "There's a new couple on campus, but no one knows who they are yet."
...
def update_rumors():
    ...
    for rumor in player["rumors"]:
        ...
        if "target" in rumor and rumor["target"] == player["name"] and rumor["spread_level"] >= 7:
            rumor_type = rumor.get("type", "")
            
            if rumor_type == "romantic":
                # Romance rumors are mild
                if random.random() < 0.1:  # Small chance
                    increase_reputation("students", random.randint(-3, 5))  # Could be positive or negative
        ...
        days_old = (current_date - datetime.strptime(rumor["date_created"], "%Y-%m-%d")).days
        if days_old > 14 and random.random() < 0.2:  # After 2 weeks, 20% daily chance to fade
            rumor["spread_level"] = max(1, rumor["spread_level"] - 1)
        
        # Very old rumors may be forgotten
        if days_old > 30 and random.random() < 0.1:  # After a month, 10% chance to be forgotten
    ...
    if status in EX_PARTNER_STATUSES and "events" in EX_PARTNER_STATUSES[status]:
        ...
        if status == "yandere":
            ...
            if random.random() < EX_PARTNER_STATUSES[status]["stalking_chance"]:
                slow_print(f"{Fore.RED}You notice {ex_name} has been following you around campus.{Style.RESET_ALL}")
                increase_stress(10)
                player["mental_health"]["anxiety"] = min(100, player["mental_health"]["anxiety"] + 8)
                
                # Check for dangerous escalation
                if random.random() < EX_PARTNER_STATUSES[status]["dangerous_chance"]:
                    slow_print(f"{Fore.RED}They've been leaving increasingly concerning messages and gifts.{Style.RESET_ALL}")
                    slow_print("You should consider reporting this behavior to campus security.")
                    increase_stress(15)
                    player["mental_health"]["anxiety"] = min(100, player["mental_health"]["anxiety"] + 15)
                    
                    # Option to report
                    if random.random() < 0.7:  # 70% chance to offer reporting option
                        report_choice = input("Report this behavior to campus security? (Y/N): ").upper()
                        if report_choice == "Y":
                            slow_print(f"You report {ex_name}'s behavior to campus security.")
                            slow_print("They tell you they'll investigate and keep an eye out.")
                            slow_print(f"{Fore.GREEN}You feel safer knowing authorities are aware of the situation.{Style.RESET_ALL}")
                            decrease_stress(10)
                            player["mental_health"]["anxiety"] = max(0, player["mental_health"]["anxiety"] - 12)
                            
                            # Chance to affect ex's status
                            if random.random() < 0.6:  # 60% chance of intervention working
                                ex_partner_data["status"] = "angry"  # Change from yandere to angry
                                slow_print(f"Campus security has spoken to {ex_name}. They seem to be keeping their distance now.")
        ...
        if random.random() < reconciliation_chance:
            ...
            # Player decision
            reconcile_choice = input(f"Try to reconcile with {ex_name}? (Y/N): ").upper()
            if reconcile_choice == "Y":
                # Reconciliation effects
                slow_print(f"You agree to give your relationship with {ex_name} another chance.")
                ...
                if "harem_enabled" in player and player["harem_enabled"]:
                    ...
                    slow_print(f"{ex_name} is now part of your romantic partners again!")
                    # Remove from ex-partners list
                    player["ex_partners"] = [ex for ex in player["ex_partners"] if ex["name"] != ex_name]
                else:
                    # Standard relationship - check if we're already in another relationship
                    if player["romantic_interest"] and player["romantic_interest"] != ex_name:
                        current_partner = player["romantic_interest"]
                        choice = input(f"You're currently dating {current_partner}. End that relationship to be with {ex_name}? (Y/N): ").upper()
                        
                        if choice == "Y":
                            # Break up with current partner
                            slow_print(f"You break things off with {current_partner} to get back with {ex_name}.")
                            ...
                            player["romance_points"] = ROMANCE_STAGES[4]["req"]  # Min req for Crushing
                            
                            # Remove from ex-partners list
                            player["ex_partners"] = [ex for ex in player["ex_partners"] if ex["name"] != ex_name]
                        else:
                            slow_print(f"You decide to stay with {current_partner} and turn down {ex_name}'s offer.")
                ...
                slow_print(f"You tell {ex_name} that you don't think getting back together is a good idea.")
                
                # Random reaction to rejection
                reactions = [
                    {"text": "seems to take it well, nodding sadly.", "new_status": "moved_on"},
                    {"text": "looks disappointed but tries to smile.", "new_status": "still_interested"},
                    {"text": "gets upset and walks away quickly.", "new_status": "angry"},
                    {"text": "stares at you intensely before leaving without a word.", "new_status": "yandere"}
                ]
                ...
                chosen_reaction = random.choices(reactions, weights=reaction_probabilities, k=1)[0]
        ...
        if random.random() < friendship_chance and status != "yandere":
            ...
            choice = input(f"Try to maintain a friendship with {ex_name}? (Y/N): ").upper()
            if choice == "Y":
                slow_print(f"You agree to work on having a friendly relationship with {ex_name}.")
                ...
                if ex_name not in player["relationships"]:
                    ...
                    player["relationships"][ex_name] = max(40, player["relationships"][ex_name])
                
                # If they were "angry" status, improve that
                if ex_partner_data["status"] == "angry":
                    ex_partner_data["status"] = "moved_on"
                    slow_print(f"{ex_name} seems relieved that you can still be friends.")
            else:
                slow_print(f"You tell {ex_name} that you'd prefer to keep your distance.")
...
def check_for_special_events():
    """Check if today is a special event or festival day"""
    current_month = current_date.month
    current_day = current_date.day
    ...
    for event_name, event_data in special_events.items():
        if event_data["month"] == current_month and event_data["day"] == current_day:
            ...
            if "reputation" in rewards:
                ...
                slow_print(f"Your reputation among students increased by {rewards['reputation']}!")
            ...
            if "stress_reduction" in rewards:
                player["stress"] = max(0, player["stress"] - rewards["stress_reduction"])
                slow_print(f"The festival helped you relax! (Stress -{ rewards['stress_reduction']})")
    ...
    for festival_name, festival_data in FESTIVALS.items():
        ...
        if current_month == festival_month and current_day == festival_day:
            ...
            slow_print(f"{Fore.MAGENTA}======================================{Style.RESET_ALL}")
            ...
            if choice == "1":
                ...
                if "minigame" in festival_data and festival_data["minigame"]:
                    minigame_choice = input("Would you like to participate in the special festival minigame? (y/n): ")
                    if minigame_choice.lower() == "y":
                        play_festival_minigame(festival_data["minigame"])
                    else:
                        slow_print("You enjoy the festival without participating in the special game.")
                ...
                slow_print(f"You enjoyed the festival and reduced your stress by {stress_reduction}!")
                slow_print(f"You earned ¥{money_reward} and gained {reputation_gain} student reputation!")
                
                # Chance for special rewards
                if random.random() < 0.3:  # 30% chance
                    for achievement, reward_desc in festival_data["special_rewards"].items():
            ...
            if "Festival Participant" not in player["achievements"]:
                ...
                slow_print(f"{Fore.YELLOW}Achievement unlocked: Festival Participant{Style.RESET_ALL}")
...
def check_club_meetings():
    ...
    for club_name in player["clubs"]:
        if club_name in clubs and current_weekday in clubs[club_name]["meeting_days"]:
            ...
            if "charisma" in benefits:
                for skill, value in benefits["charisma"].items():
                    player["charisma"][skill] += value // 2  # Half benefit for automated meetings
            ...
            if "energy_cost" in benefits:
                ...
                slow_print(f"The club activities made you a bit tired. (Energy -{energy_cost})")
...
def check_achievements():
    ...
    if "Social Butterfly" not in player["achievements"] and achievements["Social Butterfly"]["condition"]():
        player["achievements"].append("Social Butterfly")
        slow_print(f"{Fore.YELLOW}Achievement unlocked: Social Butterfly{Style.RESET_ALL}")
        player["charisma"]["social"] += achievements["Social Butterfly"]["reward"]["charisma"]["social"]
    
    # Academic Excellence (All A's)
    if "Academic Excellence" not in player["achievements"] and achievements["Academic Excellence"]["condition"]():
        player["achievements"].append("Academic Excellence")
        slow_print(f"{Fore.YELLOW}Achievement unlocked: Academic Excellence{Style.RESET_ALL}")
        player["charisma"]["academic"] += achievements["Academic Excellence"]["reward"]["charisma"]["academic"]
...
def check_quest_objectives(location):
    ...
    for quest in quests:
        if not quest["completed"] and quest["objective"].startswith("Go to") and location in quest["objective"]:
            slow_print(f"{Fore.GREEN}You completed the objective for quest: {quest['description']}{Style.RESET_ALL}")
            quest["completed"] = True
            ...
            if "Quest Completer" not in player["achievements"]:
                ...
                slow_print(f"{Fore.YELLOW}Achievement unlocked: Quest Completer{Style.RESET_ALL}")
...
def show_clubs():
    ...
    for club_name, club_info in clubs.items():
        ...
        status = f"{Fore.GREEN}Member" if club_name in player["clubs"] else f"{Fore.RED}Not a member"
        ...
        print(f"Meeting days: {', '.join(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][day] for day in club_info['meeting_days'])}")
        print("Benefits:")
...
def join_club(args):
    ...
    club_match = next((name for name in club_names if name.lower() == club_name.lower()), None)
    ...
    if len(player["clubs"]) >= 2:
        print(f"{Fore.RED}You can only join up to 2 clubs. Leave a club first.{Style.RESET_ALL}")
        print("Your current clubs:")
        for i, club in enumerate(player["clubs"], 1):
            print(f"{i}. {club}")
        return
    ...
    print(f"{Fore.GREEN}You have successfully joined the {club_match}!{Style.RESET_ALL}")
    print(f"Visit the {clubs[club_match]['location']} during club hours to participate in club activities.")
...
def switch_active_partner(args):
    ...
    if "harem_enabled" not in player or not player["harem_enabled"]:
        slow_print("{0}You don't have multiple romantic partners in this playthrough.{1}".format(
            Fore.RED, Style.RESET_ALL))
        return
    
    if "romantic_partners" not in player or not player["romantic_partners"]:
        slow_print("{0}You don't have any romantic partners yet.{1}".format(
            Fore.RED, Style.RESET_ALL))
        return
    ...
    if not args:
        ...
        for i, partner_name in enumerate(player["romantic_partners"], 1):
            active = " (Active)" if player.get("romantic_interest") == partner_name else ""
    ...
    if partner_name in player["romantic_partners"]:
        ...
        slow_print(f"{Fore.RED}{partner_name} is not one of your romantic partners.{Style.RESET_ALL}")
...
def show_romance():
    ...
    if "harem_enabled" in player and player["harem_enabled"] and "romantic_partners" in player and player["romantic_partners"]:
        ...
        stage = player["romance_stage"]
        next_stage_req = ROMANCE_STAGES[min(stage+1, max(ROMANCE_STAGES.keys()))]["req"]
        ...
        if stage >= 2:
            ...
            print("You can go on dates using the '/date' command when in appropriate locations:")
            
            available_date_types = []
            for date_name, date_info in DATE_TYPES.items():
                if date_info["min_stage"] <= stage:
                    available_date_types.append(f"{date_name} (Stage {date_info['min_stage']}+)")
    ...
    # Standard single relationship
        ...
        print("3. You can also encounter potential romantic interests during special events")
...
def go_on_date(args):
    ...
    if not player.get("romantic_interest"):
        slow_print(f"{Fore.RED}You don't have a romantic partner to go on a date with.{Style.RESET_ALL}")
        return
    
    # Check romance stage
    if player["romance_stage"] < 2:
        slow_print(f"{Fore.RED}Your relationship with {player['romantic_interest']} isn't close enough for dating yet.{Style.RESET_ALL}")
    ...
    valid_date_locations = ["Restaurant District", "Movie Theater", "Shopping Mall", 
                           "City Park", "Beach", "Amusement Park", "Karaoke Bar", "Arcade Center"]
    
    if current_location not in valid_date_locations:
        slow_print(f"{Fore.RED}You need to be in a suitable location for a date.{Style.RESET_ALL}")
    ...
    available_dates = []
    for date_name, date_info in DATE_TYPES.items():
        if current_location in date_info["locations"] and player["romance_stage"] >= date_info["min_stage"]:
            available_dates.append((date_name, date_info))
    
    if not available_dates:
        slow_print(f"{Fore.RED}There are no suitable date activities for your relationship stage at this location.{Style.RESET_ALL}")
        return
    ...
    if not args:
        ...
        for i, (date_name, date_info) in enumerate(available_dates, 1):
            ...
            slow_print(f"{i}. {date_name}: {date_info['description']} - ¥{date_info['cost']} {stage_req}")
    ...
    if date_match:
        ...
        slow_print(f"{Fore.RED}'{date_type}' is not an available date option at this location.{Style.RESET_ALL}")
...
def process_date(date_name, date_info, partner_name):
    ...
    if player["money"] < date_info["cost"]:
        slow_print(f"{Fore.RED}You don't have enough money for this date. You need ¥{date_info['cost']}.{Style.RESET_ALL}")
        return
    
    # Check if player has enough energy
    if player["energy"] < date_info["energy_cost"]:
        slow_print(f"{Fore.RED}You don't have enough energy for this date. You need at least {date_info['energy_cost']} energy.{Style.RESET_ALL}")
        return
    ...
    if date_info["special_attribute"] == "fun":
        bonus = 5
        slow_print(f"Having fun together increases your bond significantly! (+{bonus} romance points)")
        player["romance_points"] += bonus
    elif date_info["special_attribute"] == "relax":
        reduction = 10
        slow_print(f"The relaxing atmosphere improves your mood and reduces stress. (-{reduction} stress)")
        player["stress"] = max(0, player["stress"] - reduction)
    elif date_info["special_attribute"] == "charisma":
        bonus = 1
        slow_print(f"This social activity enhances your charisma! (+{bonus} social charisma)")
        player["charisma"]["social"] += bonus
    elif date_info["special_attribute"] == "milestone":
        bonus = 10
        slow_print(f"{Fore.MAGENTA}This special date marks an important milestone in your relationship! (+{bonus} romance points){Style.RESET_ALL}")
        player["romance_points"] += bonus
...
def date_narratives(date_type, partner_name, personality):
    ...
    if personality in personality_reactions:
        slow_print("{0}● {1}'s Reactions:{2}".format(
    ...
    if date_type in ["Fancy Date", "Stargazing"]:
        ...
        if date_type == "Fancy Date":
            slow_print("{0}As the evening comes to a close, you and {1} share a meaningful glance across the table.{2}".format(
                Fore.YELLOW, partner_name, Style.RESET_ALL))
            slow_print("{0}The connection between you feels stronger than ever.{1}".format(
                Fore.YELLOW, Style.RESET_ALL))
        else:  # Stargazing
            slow_print("{0}A shooting star streaks across the sky, and you both make silent wishes.{1}".format(
                Fore.YELLOW, Style.RESET_ALL))
            slow_print("{0}In that perfect moment, you feel truly connected to {1}.{2}".format(
                Fore.YELLOW, partner_name, Style.RESET_ALL))
...
def check_romance_stage_advancement(partner_name):
    ...
    if next_stage in ROMANCE_STAGES and player["romance_points"] >= ROMANCE_STAGES[next_stage]["req"]:
        ...
        if next_stage == 3:
            slow_print("You and {0} have become close friends. You can feel a special connection developing.".format(partner_name))
        elif next_stage == 4:
            slow_print("You realize you have deeper feelings for {0}. The way they look at you has changed too.".format(partner_name))
        elif next_stage == 5:
            # Check for harem option (15% chance)
            if "harem_enabled" not in player and random.random() < 0.15:
                player["harem_enabled"] = True
                player["romantic_partners"] = [partner_name]  # Initialize with first partner
                slow_print("Your relationship with {0} is now official!".format(partner_name))
                slow_print("{0}You sense that you might be able to maintain multiple romantic relationships in this playthrough...{1}".format(
                    Fore.YELLOW, Style.RESET_ALL))
                ...
                slow_print("Your relationship with {0} is now official!".format(partner_name))
                slow_print("{0}You now have {1} romantic partners.{2}".format(
                    Fore.MAGENTA, len(player["romantic_partners"]), Style.RESET_ALL))
            else:
                # Standard exclusive relationship
                if player["romantic_interest"] and player["romantic_interest"] != partner_name:
                    slow_print("{0}You've ended your relationship with {1} to be with {2}.{3}".format(
                        Fore.RED, player["romantic_interest"], partner_name, Style.RESET_ALL))
                
                player["romantic_interest"] = partner_name
                slow_print("Your relationship with {0} is now official! You're dating exclusively.".format(partner_name))
            
            # Unlock romance achievement if reached Dating stage
            if "Romance Expert" not in player["achievements"]:
                player["achievements"].append("Romance Expert")
                slow_print("{0}Achievement unlocked: Romance Expert{1}".format(
                    Fore.YELLOW, Style.RESET_ALL))
                slow_print("Your social charisma has increased!")
                player["charisma"]["social"] += 4
            
            # Special harem achievement
            if "harem_enabled" in player and player["harem_enabled"] and "romantic_partners" in player and len(player["romantic_partners"]) >= 3:
                if "Harem Master" not in player["achievements"]:
                    player["achievements"].append("Harem Master")
                    slow_print("{0}Achievement unlocked: Harem Master{1}".format(
                        Fore.YELLOW, Style.RESET_ALL))
...
def relax_command():
    ...
    relaxation_options = {
        ...
        "Library": ["Read a novel", "Watch educational videos", "Study at your own pace"],
        "Courtyard": ["Sit under a tree", "People watch", "Exercise lightly"],
        "Rooftop": ["Enjoy the view", "Feel the breeze", "Have a quiet moment"],
        "School Garden": ["Smell the flowers", "Practice meditation", "Sketch the scenery"]
    }
    
    # Default options if location not in the dictionary
    if current_location not in relaxation_options:
        relaxation_options[current_location] = ["Take a moment to breathe", "Close your eyes and rest", "Stretch your muscles"]
    ...
    if random.random() < 0.2:  # 20% chance
        if current_location in ["Courtyard", "Rooftop", "School Garden"] and player["romantic_interest"]:
            ...
            player["stress"] = max(0, player["stress"] - random.randint(5, 10))  # Additional stress reduction
        else:
...
def show_rumors():
    ...
    sorted_rumors = sorted(player["rumors"], key=lambda r: r.get("spread_level", 0), reverse=True)
    ...
    for i, rumor in enumerate(sorted_rumors, 1):
        ...
        if spread_level <= 2:
            ...
            status = f"{Fore.YELLOW}(Spreading - becoming common knowledge){Style.RESET_ALL}"
        else:
    ...
    if any(r.get("target") == player["name"] for r in player["rumors"]):
        print(f"{Fore.YELLOW}Tip: Rumors about you can affect your reputation. You might want to address them.{Style.RESET_ALL}")
        
    if any(r.get("originator") == player["name"] for r in player["rumors"]):
        print(f"{Fore.YELLOW}Tip: Starting rumors may have consequences if discovered.{Style.RESET_ALL}")
...
def change_clothes_command(args=None):
    ...
    if current_clothing:
        clothing_type = CLOTHING_ITEMS[current_clothing]["type"].capitalize()
        slow_print(f"{Fore.CYAN}Currently Wearing:{Style.RESET_ALL} {current_clothing} ({clothing_type})")
    else:
        slow_print(f"{Fore.RED}Warning: You aren't wearing any clothes!{Style.RESET_ALL}")
    ...
    if player["clothing"]["owned"]:
        ...
        for i, clothing in enumerate(player["clothing"]["owned"], 1):
            clothing_type = CLOTHING_ITEMS[clothing]["type"].capitalize()
            currently_wearing = " (Currently wearing)" if clothing == current_clothing else ""
...
def random_location_event(location):
    ...
    generic_events = [
        {"description": "You find a small amount of money on the ground!", "effect": lambda: increase_money(random.randint(50, 200))},
        {"description": "You feel a bit tired from walking around.", "effect": lambda: decrease_energy(random.randint(3, 7))},
        {"description": "A cool breeze makes you feel refreshed.", "effect": lambda: decrease_stress(random.randint(3, 7))}
    ]
    
    # Location-specific events
    location_events = {
        "Classroom": [
            {"description": "The teacher asks you a question and you answer correctly!", "effect": lambda: increase_reputation("teachers", random.randint(1, 3))},
            {"description": "You help a classmate understand a difficult concept.", "effect": lambda: increase_reputation("students", random.randint(2, 4))},
            {"description": "A pop quiz is announced! You feel stressed.", "effect": lambda: increase_stress(random.randint(5, 10))}
        ],
        "Library": [
            {"description": "You find a helpful book that makes studying easier.", "effect": lambda: player["charisma"].update({"academic": player["charisma"]["academic"] + 1})},
            {"description": "The quiet atmosphere helps you concentrate.", "effect": lambda: decrease_stress(random.randint(5, 10))}
        ],
        "Cafeteria": [
            {"description": "Someone shares their food with you.", "effect": lambda: increase_hunger(random.randint(10, 20))},
            {"description": "You join a lively table conversation.", "effect": lambda: player["charisma"].update({"social": player["charisma"]["social"] + 1})}
        ],
        "Gym": [
            {"description": "An impromptu sports game starts and you join in.", "effect": lambda: decrease_energy(random.randint(10, 15))},
            {"description": "Watching others exercise motivates you.", "effect": lambda: player["charisma"].update({"social": player["charisma"]["social"] + 1})}
        ],
        "Student Room 364": [
            {"description": "Your roommate left a snack for you.", "effect": lambda: increase_hunger(random.randint(5, 15))},
            {"description": "The comfort of your room helps you relax.", "effect": lambda: decrease_stress(random.randint(8, 15))}
        ]
    }
...
def random_event(location):
    event_chance = random.randint(1, 6)  # Increased possibilities with ex-partner events
    ...
    if player["neurodiversity"]["traits"] and random.random() < 0.15:
        ...
        if trait == "dyslexia":
            ...
            if "extended_time" in player["neurodiversity"]["accommodations"]:
                slow_print("Your extended time accommodation helps you work through it at your own pace.")
                slow_print("You feel relieved that the pressure is reduced.")
                decrease_stress(5)
            else:
                slow_print("The pressure makes you feel anxious about finishing on time.")
            ...
            if "sensory_accommodations" in player["neurodiversity"]["accommodations"]:
                slow_print("You use your noise-cancelling headphones from your accommodation plan.")
            ...
            if "alternative_assessments" in player["neurodiversity"]["accommodations"]:
                slow_print("You remember you can use your alternative assessment accommodation.")
    ...
    if event_chance == 1:
        ...
        if event == "pop quiz":
            ...
            if attend == "Y":
                slow_print("You attend the workshop and learn valuable coping strategies.")
                player["mental_health"]["coping_skills"] = min(100, player["mental_health"]["coping_skills"] + 15)
                
                # Reduce anxiety and depression slightly
                player["mental_health"]["anxiety"] = max(0, player["mental_health"]["anxiety"] - 10)
                player["mental_health"]["depression"] = max(0, player["mental_health"]["depression"] - 8)
                
                slow_print(f"{Fore.GREEN}Your mental well-being has improved from what you learned!{Style.RESET_ALL}")
                decrease_stress(12)
            else:
            ...
            if attend == "Y":
                ...
                player["mental_health"]["support_network"] = min(100, player["mental_health"]["support_network"] + 5)
                decrease_stress(8)
                ...
                if not player["neurodiversity"]["traits"] and random.random() < 0.05:
                    ...
                    slow_print("They recommend you get assessed at the student health center.")
                    ...
                    if choice == "Y":
                        ...
                        slow_print(f"After assessment, you learn that you have {trait.replace('_', ' ')}.")
                        player["neurodiversity"]["traits"].append(trait)
                        player["neurodiversity"]["severity"][trait] = severity
                        player["neurodiversity"]["diagnosis_date"] = current_date.strftime("%Y-%m-%d")
                        
                        # Ask about accommodations
                        slow_print(f"The health center discusses possible accommodations for your {trait.replace('_', ' ')}.")
                        accommodations_choice = input("Request academic accommodations? (Y/N): ").upper()
                        if accommodations_choice == "Y":
                            player["neurodiversity"]["accommodations"] = random.sample(
                                NEURODIVERSITY_TRAITS[trait]["accommodations"], 
                                min(2, len(NEURODIVERSITY_TRAITS[trait]["accommodations"]))
                            )
                            slow_print(f"You are granted: {', '.join(player['neurodiversity']['accommodations'])}")
            else:
            ...
            slow_print("You spend some time petting the dogs and chatting with their handlers.")
            
            # Mental health benefits
            decrease_stress(15)
            player["mental_health"]["happiness"] = min(100, player["mental_health"]["happiness"] + 10)
            player["mental_health"]["anxiety"] = max(0, player["mental_health"]["anxiety"] - 8)
            
            slow_print(f"{Fore.GREEN}The friendly animals really brightened your day!{Style.RESET_ALL}")
    
    elif event_chance == 2 and player["romance_stage"] >= 3 and player["romantic_interest"]:
        ...
        decrease_stress(10)
        player["mental_health"]["happiness"] = min(100, player["mental_health"]["happiness"] + 8)
...
def load_game(slot_name):
    ...
    if not os.path.exists(filename):
        print(f"No save file found for {slot_name}.")
        return
...
def display_save_slots():
        ...
        if i < len(saves):
            ...
            try:
                with open(save_file, 'r') as f:
                    ...
                    timestamp = datetime.fromtimestamp(save_time).strftime('%d/%m/%Y')
                print(f"{Fore.YELLOW}Saving slot {i+1}{Style.RESET_ALL}")
                print(f"name: {player_name}")
                print(f"timestamp: {timestamp}")
            except Exception as e:
                print(f"Saving slot {i+1} (Error: {str(e)})")
                print("name: corrupted save")
    ...
    if slot_num.isdigit():
        ...
        if 1 <= slot_num <= 5 and slot_num <= len(saves):
            load_game(saves[slot_num-1].replace('.json', ''))
            return True
    return False
...
student_templates = {
    ...
    "Senior Class President": {
        ...
        "stats": {
            ...
            "achievements": ["Club Leader", "Social Butterfly", "Teacher's Pet", "Perfect Attendance"],
    ...
    "Exchange Student": {
        ...
        "stats": {
            ...
            "gpa": 3.7,
            "completed_years": [1, 2],
            "relationships": {"Yuki Tanaka": 20, "Akira Suzuki": 15, "Haruki Takahashi": 25}
        }
    }
}
...
def apply_template(template_name):
    ...
    if "electives" in stats and all(elective in available_electives for elective in stats["electives"]):
    ...
    homework = {}
    for subject_name, subject_data in subjects_data.items():
        if subject_data.get("core", False) or subject_name in player.get("electives", []):
            homework[subject_name] = False
...
def show_templates():
    ...
    for name, template in student_templates.items():
        ...
        print("Stats:")
        stats = template["stats"]
        print(f"  Social: {stats['charisma']['social']}, Academic: {stats['charisma']['academic']}")
        print(f"  Starting Money: ¥{stats['money']}")
        print(f"  Electives: {', '.join(stats['electives'])}")
        print(f"  Reputation: Students {stats['reputation']['students']}, Teachers {stats['reputation']['teachers']}")
...
def test_color_display():
    ...
    # Mixed color test
    print("\nMixed color test:")
    print("Normal {}Red{} Normal {}Green{} Normal".format(
        Fore.RED, Style.RESET_ALL, Fore.GREEN, Style.RESET_ALL))
    
    # Background color test
    print("\nBackground color test:")
    print("{}{}White text on red background{}".format(
        Back.RED, Fore.WHITE, Style.RESET_ALL))
    print("{}{}White text on blue background{}".format(
        Back.BLUE, Fore.WHITE, Style.RESET_ALL))
    print("{}{}Black text on green background{}".format(
        Back.GREEN, Fore.BLACK, Style.RESET_ALL))
    
    # Style test
    print("\nStyle test:")
    print("{}{}This should be BRIGHT yellow text{}".format(
        Style.BRIGHT, Fore.YELLOW, Style.RESET_ALL))
    print("{}{}This should be DIM blue text{}".format(
    ...
    slow_print("This should be yellow bright text", color=Fore.YELLOW, style=Style.BRIGHT, delay=0.01)
    slow_print("{}This has embedded red{} in the text".format(
        Fore.RED, Style.RESET_ALL), delay=0.01)
    
    # Color code embedding tests
    print("\nColor code embedding tests:")
    slow_print("{0}Red text with {1}embedded cyan{0} and back to red{2}".format(
        Fore.RED, Fore.CYAN, Style.RESET_ALL), delay=0.01)
    slow_print("{0}{1}Bright{2} and {3}Dim{2} in the same line{4}".format(
        Fore.YELLOW, Style.BRIGHT, Style.NORMAL, Style.DIM, Style.RESET_ALL), delay=0.01)
...
def show_main_menu():
    ...
    print("{0}4. Student Templates (Password Required){1}".format(Fore.MAGENTA, Style.RESET_ALL))
    ...
    try:
        ...
        if choice == "1":
            ...
            password = input("{0}Enter password (hint: complete a school year or graduate):{1} ".format(
                Fore.YELLOW, Style.RESET_ALL))
            if password.upper() == "SCHOOLYEAREND":
                ...
                print("{0}Incorrect password. Access denied.{1}".format(Fore.RED, Style.RESET_ALL))
                print("Hint: Complete a school year or graduate to obtain special passwords.")
                return show_main_menu()
        elif choice == "5":
            return test_color_display()
        else:
            print("{0}Invalid choice. Please select an option between 1 and 5.{1}".format(
                Fore.RED, Style.RESET_ALL))
            return show_main_menu()
    except EOFError:
        print("{0}Input error encountered. Please try again.{1}".format(Fore.RED, Style.RESET_ALL))
        return False
    except Exception as e:
...
def main():
    # Print the ASCII art and welcome message with proper color formatting
    print(ASCII_ART)
    print("\n{0}Welcome to a new school year!{1}".format(Fore.MAGENTA, Style.RESET_ALL))
    print("{0}Your journey through campus life begins...{1}\n".format(Fore.CYAN, Style.RESET_ALL))

    if not show_main_menu():
        setup_game()
        print("\n{0}Hello {1}{2}{0}! Type {3}/help{0} to see available commands.{4}".format(
            Fore.YELLOW, Fore.GREEN, player['name'], Fore.CYAN, Style.RESET_ALL))

    while True:
        if current_date >= SCHOOL_YEAR_END:
            slow_print("{0}The school year has ended! Thanks for playing!{1}".format(
                Fore.YELLOW, Style.RESET_ALL))
            print("\n{0}Congratulations on completing a full school year!{1}".format(
                Fore.CYAN, Style.RESET_ALL))
            print("{0}You've unlocked the Student Templates feature.{1}".format(
                Fore.GREEN, Style.RESET_ALL))
            print("{0}Secret password: SCHOOLYEAREND{1}".format(
                Fore.YELLOW, Style.RESET_ALL))
            print("Use this password in the main menu to access special student templates for your next playthrough!")
            break
        ...
        try:
            ...
            print("{0}Input error encountered. Please try again.{1}".format(
                Fore.RED, Style.RESET_ALL))
            continue
        except Exception as e:
            print("{0}Error: {1}{2}".format(Fore.RED, str(e), Style.RESET_ALL))
        ...
        if cmd == "help":
            ...
            print("Current date: {0}".format(current_date.strftime('%Y-%m-%d')))
            if is_weekend():
...
last_names = [
    "Takahashi", "Yamamoto", "Kobayashi", "Tanaka", "Sasaki", "Fujimoto", "Hoshino", "Shimizu", "Nakamura", "Suzuki", 
    "Sato", "Kato", "Ito", "Watanabe", "Yamada", "Matsumoto", "Inoue", "Kimura", "Hayashi", "Saito",
    "Yamaguchi", "Nakajima", "Matsui", "Ikeda", "Yoshida", "Yamashita", "Sasaki", "Yamazaki", "Ogawa", "Ishikawa",
    "Maeda", "Hasegawa", "Murata", "Kobayashi", "Kojima", "Kondo", "Ishii", "Abe", "Harada", "Fujita",
    "Aoki", "Hashimoto", "Okada", "Sakamoto", "Mori", "Endo", "Fukuda", "Goto", "Nishimura", "Takagi"
]
...
exam_questions = {
    "Math": [
        {"question": "What is the derivative of x²?", "answer": "2x", "options": ["x", "2x", "x²", "2"]},
        {"question": "What is the integral of 1/x?", "answer": "ln|x|", "options": ["x", "ln|x| + C", "1/x", "e^x"]},
        {"question": "Solve for x: 2x + 5 = 13", "answer": "4", "options": ["3", "4", "5", "6"]},
        {"question": "What is 15% of 200?", "answer": "30", "options": ["25", "30", "35", "40"]},
        {"question": "If a triangle has angles 30° and 60°, what is the third angle?", "answer": "90", "options": ["60", "75", "90", "120"]},
        {"question": "What is the square root of 144?", "answer": "12", "options": ["10", "11", "12", "14"]}
    ],
    "Science": [
        {"question": "What is the chemical symbol for Gold?", "answer": "Au", "options": ["Ag", "Au", "Fe", "Cu"]},
        {"question": "What is the nearest planet to the Sun?", "answer": "Mercury", "options": ["Venus", "Mars", "Mercury", "Earth"]},
        {"question": "What is the human body's largest organ?", "answer": "Skin", "options": ["Heart", "Liver", "Brain", "Skin"]},
        {"question": "What is the speed of light?", "answer": "300,000 km/s", "options": ["200,000 km/s", "250,000 km/s", "300,000 km/s", "350,000 km/s"]},
        {"question": "What is the process of plants making food called?", "answer": "Photosynthesis", "options": ["Respiration", "Photosynthesis", "Digestion", "Absorption"]}
    ],
    "Literature": [
        {"question": "Who wrote 'Romeo and Juliet'?", "answer": "Shakespeare", "options": ["Dickens", "Shakespeare", "Hemingway", "Twain"]},
        {"question": "What is the main theme of 'The Great Gatsby'?", "answer": "American Dream", "options": ["Love", "War", "American Dream", "Family"]},
        {"question": "What is a metaphor?", "answer": "Comparison without like/as", "options": ["Direct comparison", "Comparison without like/as", "Sound repetition", "Word play"]},
        {"question": "What is an autobiography?", "answer": "Self-written life story", "options": ["Fiction", "Biography", "Self-written life story", "Poetry"]},
        {"question": "What is alliteration?", "answer": "Same sound at start", "options": ["Rhyming", "Same sound at start", "Opposites", "Comparison"]}
    ],
    "History": [
        {"question": "When did World War II end?", "answer": "1945", "options": ["1943", "1944", "1945", "1946"]},
        {"question": "Who was the first President of the United States?", "answer": "George Washington", "options": ["John Adams", "Thomas Jefferson", "George Washington", "Benjamin Franklin"]},
        {"question": "What ancient civilization built the pyramids?", "answer": "Egyptians", "options": ["Romans", "Greeks", "Egyptians", "Mayans"]},
        {"question": "In what year did the Berlin Wall fall?", "answer": "1989", "options": ["1985", "1987", "1989", "1991"]},
        {"question": "Who was the first Emperor of Japan?", "answer": "Emperor Jimmu", "options": ["Emperor Jimmu", "Emperor Meiji", "Emperor Hirohito", "Emperor Akihito"]}
    ],
    "Art": [
        {"question": "Who painted the Mona Lisa?", "answer": "Leonardo da Vinci", "options": ["Van Gogh", "Leonardo da Vinci", "Picasso", "Monet"]},
        {"question": "What are the primary colors?", "answer": "Red, Blue, Yellow", "options": ["Red, Green, Blue", "Red, Blue, Yellow", "Red, Orange, Yellow", "Blue, Green, Yellow"]},
        {"question": "What style is 'The Starry Night'?", "answer": "Post-Impressionism", "options": ["Realism", "Cubism", "Post-Impressionism", "Abstract"]},
        {"question": "What is chiaroscuro?", "answer": "Light and shadow contrast", "options": ["Color theory", "Brush technique", "Light and shadow contrast", "Perspective"]},
        {"question": "What medium is used in fresco painting?", "answer": "Wet plaster", "options": ["Canvas", "Wood", "Wet plaster", "Paper"]}
    ]
}
...
def apply_sanction(reason):
    if reason == "missed_class":
        slow_print("{0}You missed a class! Your teacher is disappointed.{1}".format(
            Fore.RED, Style.RESET_ALL))
        player["reputation"]["teachers"] -= 10
        player["charisma"]["academic"] = max(player["charisma"]["academic"] - 1, 0)
    elif reason == "late_to_class":
        slow_print("{0}You were late to class! You lose some reputation.{1}".format(
            Fore.RED, Style.RESET_ALL))
        player["reputation"]["teachers"] -= 5
    elif reason == "missed_homework":
        slow_print("{0}You didn't complete your homework! Your grade might suffer.{1}".format(
            Fore.RED, Style.RESET_ALL))
        ...
        if player.get("uniform_violations", 0) == 0:
            slow_print("{0}The teacher scolds you for not wearing the proper school uniform!{1}".format(
                Fore.RED, Style.RESET_ALL))
        ...
        # Second violation is detention
            ...
            slow_print("{0}This is your second uniform violation!{1}".format(
                Fore.RED, Style.RESET_ALL))
            slow_print("You're given detention after school, which takes up your free time.")
        ...
        # Third violation is a meeting with parents/principal
            ...
            slow_print("{0}This is your third uniform violation! The principal is very disappointed.{1}".format(
                Fore.RED, Style.RESET_ALL))
            slow_print("You're scheduled for a disciplinary meeting with the principal.")
            slow_print("Your reputation with teachers suffers significantly.")
            player["reputation"]["teachers"] = max(0, player["reputation"]["teachers"] - 20)
            player["stress"] += 25
            # Reset counter for next set of violations
            player["uniform_violations"] = 0
    else:
        slow_print("{0}You received a sanction: {1}{2}".format(
...
def take_exam(subject):
    ...
    total_questions = random.randint(6, 10)  # Random number of questions between 6 and 10
    questions = random.sample(exam_questions[subject], min(total_questions, len(exam_questions[subject])))
    score = 0

    for i, q in enumerate(questions, 1):
        print("\n{0}Question {1}:{2} {3}".format(Fore.CYAN, i, Style.RESET_ALL, q['question']))
        ...
        if answer.isdigit() and 1 <= int(answer) <= len(q['options']):
            if q['options'][int(answer)-1] == q['answer']:
                score += 10 / total_questions  # Score scaled by number of questions
                print("{0}Correct!{1}".format(Fore.GREEN, Style.RESET_ALL))
            else:
                print("{0}Incorrect! The answer was: {1}{2}".format(
...
def check_monthly_exam():
    ...
    if current_date.day == 28:  # End of month exam
        ...
        for subject in subjects:
            ...
            player['grades'][subject] = new_grade
            print("\n{0} exam score: {1}/10 (Grade: {2})".format(subject, score, new_grade))

        # Update teacher reputation based on grades and homework
        completed_homework = sum(1 for done in homework.values() if done)
        good_grades = sum(1 for grade in player['grades'].values() if grade in ['A++', 'A+', 'A', 'B'])
...
# Random Events
random_events = [
    {"type": "positive", "description": "Found some money in the courtyard! (+500 yen)", "effect": {"money": 500}},
    {"type": "negative", "description": "Lost your textbook! (-1000 yen)", "effect": {"money": -1000}},
    {"type": "positive", "description": "Teacher praised your work! (+5 teacher reputation)", "effect": {"teacher_rep": 5}},
    {"type": "negative", "description": "Arrived late to class! (-3 teacher reputation)", "effect": {"teacher_rep": -3}},
    {"type": "positive", "description": "Helped a classmate! (+5 student reputation)", "effect": {"student_rep": 5}},
    {"type": "neutral", "description": "Someone left a note in your locker...", "effect": {"quest": True}},
    {"type": "neutral", "description": "Saw a cute cat on the way to school.", "effect": {}},
    {"type": "positive", "description": "Won a small prize in the school lottery! (+100 yen)", "effect": {"money": 100}},
    {"type": "negative", "description": "Got caught in a rainstorm without an umbrella! (-50 energy)", "effect": {"energy": -50}},
    {"type": "positive", "description": "Helped a classmate with homework! (+5 student reputation)", "effect": {"student_rep": 5}},
    {"type": "negative", "description": "Had a disagreement with a classmate. (-5 student reputation)", "effect": {"student_rep": -5}},
    {"type": "neutral", "description": "Saw a beautiful sunset on the way home.", "effect": {}},
    {"type": "positive", "description": "Helped a teacher with a project! (+10 teacher reputation)", "effect": {"teacher_rep": 10}},
    {"type": "negative", "description": "Forgot your lunch at home! (-20 energy)", "effect": {"energy": -20}}
]
...
graduate_templates = {
    "Academic Prodigy": {
        ...
        "stats": {
            ...
            "achievements": ["Academic Excellence", "Scholarship Recipient", "Science Olympiad"]
        }
    },
    "Social Influencer": {
        ...
        "stats": {
            ...
            "club_positions": {"Drama Club": "Member", "Student Council": "Public Relations"},
    ...
    "Exchange Student": {
        ...
        "stats": {
            ...
            "club_positions": {"International Club": "Vice President", "Language Exchange": "Coordinator"},
    ...
    "Campus Entrepreneur": {
        ...
        "stats": {
            ...
            "club_positions": {"Business Club": "President", "Entrepreneurship Society": "Founder"},
    ...
    "Double Major": {
        ...
        "stats": {
            ...
            "club_positions": {"Literature Club": "Member", "Philosophy Club": "Member"},
...
def show_graduate_templates():
    ...
    print("\n{0}=== Graduate Templates (Special) ==={1}".format(Fore.MAGENTA, Style.RESET_ALL))
    print("{0}These enhanced templates are unlocked by completing the game.{1}".format(
        Fore.CYAN, Style.RESET_ALL))
    
    for idx, (name, template) in enumerate(graduate_templates.items(), 1):
        year_names = {1: "Freshman", 2: "Sophomore", 3: "Junior", 4: "Senior"}
        year_name = year_names.get(template["stats"]["school_year"], 
        ...
        print("   Year: {0} (Year {1})".format(year_name, template['stats']['school_year']))
        print("   Academic Charisma: {0}".format(template['stats']['charisma']['academic']))
        ...
        if "achievements" in template["stats"]:
            print("   Starting Achievements: {0}".format(', '.join(template['stats']['achievements'])))
        if "clubs" in template["stats"]:
            print("   Starting Clubs: {0}".format(', '.join(template['stats']['clubs'])))
...
def apply_graduate_template(template_name):
    ...
    for subject_name in current_subjects:
        ...
        if "grades" in stats and subject_name in stats["grades"]:
            ...
            player["grades"][subject_name] = "B"  # Start with better grades in graduate templates
    ...
    slow_print("\n{0}As a special graduate template, you start with enhanced abilities and opportunities!{1}".format(
        Fore.CYAN, Style.RESET_ALL))
    slow_print("{0}Make the most of your campus life experience!{1}".format(
        Fore.CYAN, Style.RESET_ALL))
