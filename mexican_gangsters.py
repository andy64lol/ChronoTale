...
class Player:
    def __init__(self):
        ...
        self.fbi_attention = 0  # FBI attention level (0-500+)
        self.military_attention = 0  # Military intervention level (0-1000+)
        self.active_federal_operations = []  # Ongoing federal operations
        self.federal_evidence = 0  # Evidence collected against player
        self.federal_wanted_level = 0  # Federal wanted level (0-10)
        self.witness_protection_threats = []  # People in witness protection against player
    ...
    def increase_fbi_attention(self, amount: int):
        """Increase FBI attention level"""
        self.fbi_attention += amount
        if self.fbi_attention >= 100 and self.federal_wanted_level == 0:
            self.federal_wanted_level = 1
        elif self.fbi_attention >= 200 and self.federal_wanted_level < 2:
            self.federal_wanted_level = 2
        elif self.fbi_attention >= 350 and self.federal_wanted_level < 3:
            self.federal_wanted_level = 3
    
    def increase_military_attention(self, amount: int):
        """Increase military attention level"""
        self.military_attention += amount
        if self.military_attention >= 300:
            self.increase_fbi_attention(50)  # Military attention also increases FBI attention
    
    def get_fbi_threat_level(self) -> str:
        """Get current FBI threat level"""
        current_level = "low"
        for level_id, level_data in FBI_THREAT_LEVELS.items():
            if self.fbi_attention >= level_data["threshold"]:
                current_level = level_id
        return current_level
    
    def get_military_intervention_level(self) -> str:
        """Get current military intervention level"""
        current_level = "none"
        for level_id, level_data in MILITARY_INTERVENTION_LEVELS.items():
            if self.military_attention >= level_data["threshold"]:
                current_level = level_id
        return current_level
    
    def trigger_federal_response(self):
        """Check and trigger federal law enforcement response"""
        fbi_level = self.get_fbi_threat_level()
        military_level = self.get_military_intervention_level()
        
        responses = []
        
        # FBI Operations
        if fbi_level == "medium" and random.random() < 0.1:
            responses.append("FBI surveillance initiated")
            if "surveillance" not in self.active_federal_operations:
                self.active_federal_operations.append("surveillance")
        
        elif fbi_level == "high" and random.random() < 0.15:
            responses.append("FBI investigation launched")
            self.federal_evidence += random.randint(10, 25)
        
        elif fbi_level == "critical" and random.random() < 0.2:
            responses.append("FBI task force deployed")
            if random.random() < 0.3:
                responses.append("Federal raid imminent")
        
        # Military Intervention
        if military_level == "national_guard" and random.random() < 0.05:
            responses.append("National Guard activated")
        
        elif military_level == "federal_task_force" and random.random() < 0.08:
            responses.append("Federal task force coordinating response")
        
        elif military_level == "military_support" and random.random() < 0.1:
            responses.append("Military support authorized")
        
        elif military_level == "full_intervention" and random.random() < 0.15:
            responses.append("CRITICAL: Full military intervention authorized")
...
class GameEngine:
    ...
    def display_header(self):
        ...
        # Federal attention display
        fbi_level = self.player.get_fbi_threat_level()
        military_level = self.player.get_military_intervention_level()
        
        fbi_color = Fore.WHITE
        if fbi_level == "medium":
            fbi_color = Fore.YELLOW
        elif fbi_level == "high":
            fbi_color = Fore.LIGHTRED_EX
        elif fbi_level in ["critical", "maximum"]:
            fbi_color = Fore.RED
        
        military_color = Fore.WHITE
        if military_level == "national_guard":
            military_color = Fore.YELLOW
        elif military_level == "federal_task_force":
            military_color = Fore.LIGHTRED_EX
        elif military_level in ["military_support", "full_intervention"]:
            military_color = Fore.RED

        print(f"{fbi_color}FBI Alert: {FBI_THREAT_LEVELS[fbi_level]['spanish']}{Style.RESET_ALL} | " +
              f"{military_color}Military: {MILITARY_INTERVENTION_LEVELS[military_level]['spanish']}{Style.RESET_ALL}")
        ...
        # Active federal operations warning
        if self.player.active_federal_operations:
            print(f"{Fore.RED}⚠️  ACTIVE FEDERAL OPERATIONS: {', '.join(self.player.active_federal_operations)}{Style.RESET_ALL}")
    ...
    def main_menu(self):
        ...
        while self.running:
            ...
            print(f"29. {Fore.RED}Estado federal / Federal status{Style.RESET_ALL}")
            print(f"30. {Fore.LIGHTRED_EX}Gestión FBI / FBI management{Style.RESET_ALL}")
            print(f"31. {Fore.RED}Intervención militar / Military intervention{Style.RESET_ALL}")
            print(f"21. {Fore.RED}Salir del juego / Quit game{Style.RESET_ALL}")
            print()

            choice = input(f"{Fore.CYAN}Elige tu opción / Enter your choice (1-31): {Style.RESET_ALL}").strip()
            ...
            if choice == "1":
                ...
                self.federal_status_menu()
            elif choice == "30":
                self.fbi_management_menu()
            elif choice == "31":
                self.military_intervention_menu()
            else:
    ...
    def bank_heist(self):
        ...
        if input("\nProceed with bank heist? (y/n): ").lower() == 'y':
            ...
            if random.randint(1, 100) <= success_chance:
                ...
                # Bank heist triggers FBI attention
                self.player.increase_fbi_attention(30)
                if money_stolen > 30000:
                    self.player.increase_military_attention(15)
                ...
                print(f"{Fore.RED}⚠️  FBI attention increased significantly!{Style.RESET_ALL}")
                
                # Check for federal response
                federal_responses = self.player.trigger_federal_response()
                for response in federal_responses:
                    print(f"{Fore.LIGHTRED_EX}FEDERAL ALERT: {response}{Style.RESET_ALL}")
            else:
                ...
                # Failed bank heist still triggers some FBI attention
                self.player.increase_fbi_attention(20)
                ...
                print(f"{Fore.RED}⚠️  FBI is now investigating the attempted heist!{Style.RESET_ALL}")
    ...
    def execute_heist(self, heist_id, heist):
        ...
        if random.randint(1, 100) <= success_chance:
            ...
            # Complex heists trigger federal attention
            fbi_attention = heist["risk"] * 8
            self.player.increase_fbi_attention(fbi_attention)
            
            # High-value heists trigger military attention
            if reward > 100000:
                self.player.increase_military_attention(heist["risk"] * 3)
                print(f"{Fore.RED}⚠️  Military attention increased - high-value target!{Style.RESET_ALL}")
            ...
            print(f"{Fore.RED}⚠️  FBI attention +{fbi_attention}{Style.RESET_ALL}")
    ...
    def save_to_slot(self, slot):
        ...
        try:
            save_data = {
                ...
                "fbi_attention": self.player.fbi_attention,
                "military_attention": self.player.military_attention,
                "federal_wanted_level": self.player.federal_wanted_level,
                "federal_evidence": self.player.federal_evidence,
                "active_federal_operations": self.player.active_federal_operations,
                "save_timestamp": datetime.now().isoformat(),
                "game_version": "2.1"
            }
    ...
    def load_from_slot(self, slot):
        ...
        try:
            ...
            # Load new federal attributes with backwards compatibility
            self.player.fbi_attention = save_data.get("fbi_attention", 0)
            self.player.military_attention = save_data.get("military_attention", 0)
            self.player.federal_wanted_level = save_data.get("federal_wanted_level", 0)
            self.player.federal_evidence = save_data.get("federal_evidence", 0)
            self.player.active_federal_operations = save_data.get("active_federal_operations", [])
    ...
    def federal_status_menu(self):
        """Display federal law enforcement status and statistics"""
        self.display_header()
        print(f"{Fore.RED}=== ESTADO FEDERAL / FEDERAL STATUS ==={Style.RESET_ALL}")
        print()
        
        # FBI Status
        fbi_level = self.player.get_fbi_threat_level()
        fbi_data = FBI_THREAT_LEVELS[fbi_level]
        print(f"{Fore.YELLOW}FBI STATUS:{Style.RESET_ALL}")
        print(f"  Nivel de atención: {fbi_data['spanish']} / {fbi_data['english']}")
        print(f"  Puntos de atención: {self.player.fbi_attention}/500")
        print(f"  Nivel de amenaza federal: {self.player.federal_wanted_level}/3")
        print(f"  Evidencia federal: {self.player.federal_evidence}/100")
        print()
        
        # Military Status  
        military_level = self.player.get_military_intervention_level()
        military_data = MILITARY_INTERVENTION_LEVELS[military_level]
        print(f"{Fore.RED}MILITARY STATUS:{Style.RESET_ALL}")
        print(f"  Nivel de intervención: {military_data['spanish']} / {military_data['english']}")
        print(f"  Puntos de atención militar: {self.player.military_attention}/1000")
        print()
        
        # Active Operations
        if self.player.active_federal_operations:
            print(f"{Fore.LIGHTRED_EX}OPERACIONES ACTIVAS / ACTIVE OPERATIONS:{Style.RESET_ALL}")
            for op in self.player.active_federal_operations:
                print(f"  • {op}")
        else:
            print(f"{Fore.GREEN}No hay operaciones federales activas / No active federal operations{Style.RESET_ALL}")
        
        print()
        print(f"{Fore.YELLOW}Consejos / Tips:{Style.RESET_ALL}")
        print("• Mantén tu nivel de atención FBI bajo para evitar investigaciones")
        print("• La atención militar se activa con crímenes extremos")
        print("• Las operaciones federales pueden llevar a redadas")
        print("• Huye de la ciudad si la situación se vuelve demasiado peligrosa")
        
        input(f"\n{Fore.CYAN}Presiona Enter para continuar...{Style.RESET_ALL}")

    def fbi_management_menu(self):
        """FBI management and operations menu"""
        self.display_header()
        print(f"{Fore.RED}=== GESTIÓN FBI / FBI MANAGEMENT ==={Style.RESET_ALL}")
        print()
        
        fbi_level = self.player.get_fbi_threat_level()
        fbi_data = FBI_THREAT_LEVELS[fbi_level]
        
        print(f"Estado actual FBI: {fbi_data['spanish']}")
        print(f"Atención FBI: {self.player.fbi_attention}/500")
        print(f"Evidencia federal: {self.player.federal_evidence}/100")
        print()
        
        print(f"{Fore.YELLOW}OPCIONES DISPONIBLES:{Style.RESET_ALL}")
        print(f"1. {Fore.LIGHTCYAN_EX}Intentar reducir atención FBI{Style.RESET_ALL} (Costo: $50,000)")
        print(f"2. {Fore.LIGHTMAGENTA_EX}Destruir evidencia federal{Style.RESET_ALL} (Costo: $100,000)")
        print(f"3. {Fore.LIGHTGREEN_EX}Sobornar informantes{Style.RESET_ALL} (Costo: $25,000)")
        print(f"4. {Fore.LIGHTRED_EX}Cambiar identidad{Style.RESET_ALL} (Costo: $200,000)")
        print(f"5. {Fore.WHITE}Información de operaciones FBI{Style.RESET_ALL}")
        print(f"0. {Fore.YELLOW}Volver / Back{Style.RESET_ALL}")
        
        try:
            choice = int(input(f"\n{Fore.CYAN}Selecciona una opción / Select option (0-5): {Style.RESET_ALL}"))
            
            if choice == 0:
                return
            elif choice == 1:  # Reduce FBI attention
                if self.player.money >= 50000:
                    self.player.remove_money(50000)
                    reduction = random.randint(20, 40)
                    self.player.fbi_attention = max(0, self.player.fbi_attention - reduction)
                    print(f"{Fore.GREEN}Atención FBI reducida en {reduction} puntos{Style.RESET_ALL}")
                    if random.random() < 0.3:  # 30% chance of failure
                        self.player.increase_fbi_attention(10)
                        print(f"{Fore.RED}¡Pero el FBI sospecha de tus actividades!{Style.RESET_ALL}")
                else:
                    print(f"{Fore.RED}No tienes suficiente dinero{Style.RESET_ALL}")
                    
            elif choice == 2:  # Destroy federal evidence
                if self.player.money >= 100000:
                    self.player.remove_money(100000)
                    reduction = random.randint(30, 60)
                    self.player.federal_evidence = max(0, self.player.federal_evidence - reduction)
                    print(f"{Fore.GREEN}Evidencia federal destruida: -{reduction} puntos{Style.RESET_ALL}")
                    if random.random() < 0.4:  # 40% chance of being caught
                        self.player.increase_fbi_attention(25)
                        print(f"{Fore.RED}¡Fuiste detectado destruyendo evidencia!{Style.RESET_ALL}")
                else:
                    print(f"{Fore.RED}No tienes suficiente dinero{Style.RESET_ALL}")
                    
            elif choice == 3:  # Bribe informants
                if self.player.money >= 25000:
                    self.player.remove_money(25000)
                    if random.random() < 0.7:  # 70% success rate
                        self.player.fbi_attention = max(0, self.player.fbi_attention - 15)
                        print(f"{Fore.GREEN}Informantes sobornados exitosamente{Style.RESET_ALL}")
                        if "surveillance" in self.player.active_federal_operations:
                            self.player.active_federal_operations.remove("surveillance")
                            print(f"{Fore.GREEN}Vigilancia FBI suspendida{Style.RESET_ALL}")
                    else:
                        self.player.increase_fbi_attention(20)
                        print(f"{Fore.RED}El soborno fue reportado al FBI{Style.RESET_ALL}")
                else:
                    print(f"{Fore.RED}No tienes suficiente dinero{Style.RESET_ALL}")
                    
            elif choice == 4:  # Change identity
                if self.player.money >= 200000:
                    self.player.remove_money(200000)
                    self.player.fbi_attention = max(0, self.player.fbi_attention - 100)
                    self.player.federal_evidence = max(0, self.player.federal_evidence - 50)
                    self.player.federal_wanted_level = max(0, self.player.federal_wanted_level - 1)
                    self.player.active_federal_operations.clear()
                    print(f"{Fore.GREEN}Identidad cambiada exitosamente{Style.RESET_ALL}")
                    print(f"{Fore.GREEN}Todas las operaciones federales canceladas{Style.RESET_ALL}")
                else:
                    print(f"{Fore.RED}No tienes suficiente dinero{Style.RESET_ALL}")
                    
            elif choice == 5:  # FBI operations info
                print(f"{Fore.LIGHTCYAN_EX}INFORMACIÓN DE OPERACIONES FBI:{Style.RESET_ALL}")
                for level_id, level_data in FBI_THREAT_LEVELS.items():
                    print(f"• {level_data['spanish']}: {level_data['description']}")
                print()
    ...
    def military_intervention_menu(self):
        """Military intervention management menu"""
        self.display_header()
        print(f"{Fore.RED}=== INTERVENCIÓN MILITAR / MILITARY INTERVENTION ==={Style.RESET_ALL}")
        print()
        
        military_level = self.player.get_military_intervention_level()
        military_data = MILITARY_INTERVENTION_LEVELS[military_level]
        
        print(f"Estado militar: {military_data['spanish']}")
        print(f"Atención militar: {self.player.military_attention}/1000")
        print()
        
        print(f"{Fore.YELLOW}INFORMACIÓN DEL SISTEMA MILITAR:{Style.RESET_ALL}")
        print("• La intervención militar ocurre cuando tus crímenes son demasiado extremos")
        print("• Una vez activada, es muy difícil de detener")
        print("• La Guardia Nacional puede ser desplegada en tu contra")
        print("• En casos extremos, el ejército puede intervenir directamente")
        print()
        
        print(f"{Fore.LIGHTRED_EX}NIVELES DE INTERVENCIÓN MILITAR:{Style.RESET_ALL}")
        for level_id, level_data in MILITARY_INTERVENTION_LEVELS.items():
            color = Fore.GREEN if level_id == "none" else Fore.YELLOW if level_id == "national_guard" else Fore.RED
            print(f"• {color}{level_data['spanish']}: {level_data['description']}{Style.RESET_ALL}")
        
        print()
        print(f"{Fore.YELLOW}OPCIONES DISPONIBLES:{Style.RESET_ALL}")
        
        if military_level == "none":
            print(f"{Fore.GREEN}No hay intervención militar activa{Style.RESET_ALL}")
            print(f"1. {Fore.LIGHTCYAN_EX}Información sobre cómo evitar la intervención{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}¡INTERVENCIÓN MILITAR ACTIVA!{Style.RESET_ALL}")
            print(f"1. {Fore.LIGHTRED_EX}Intentar negociar con autoridades{Style.RESET_ALL} (Costo: $500,000)")
            print(f"2. {Fore.LIGHTMAGENTA_EX}Abandonar el estado temporalmente{Style.RESET_ALL} (Costo: $100,000)")
            print(f"3. {Fore.LIGHTGREEN_EX}Buscar refugio en territorio neutral{Style.RESET_ALL} (Costo: $250,000)")
            
        print(f"0. {Fore.YELLOW}Volver / Back{Style.RESET_ALL}")
        
        try:
            choice = int(input(f"\n{Fore.CYAN}Selecciona una opción / Select option: {Style.RESET_ALL}"))
            
            if choice == 0:
                return
            elif choice == 1:
                if military_level == "none":
                    print(f"{Fore.LIGHTCYAN_EX}CONSEJOS PARA EVITAR INTERVENCIÓN MILITAR:{Style.RESET_ALL}")
                    print("• Evita ataques masivos a instalaciones gubernamentales")
                    print("• No uses armas militares contra civiles")
                    print("• Mantén tu nivel de respeto criminal bajo control")
                    print("• No ataques bases militares o instalaciones federales")
                    print("• Evita causar disturbios masivos o terrorismo")
                else:
                    # Negotiate with authorities
                    if self.player.money >= 500000:
                        self.player.remove_money(500000)
                        if random.random() < 0.3:  # 30% success rate
                            self.player.military_attention = max(0, self.player.military_attention - 200)
                            self.player.increase_fbi_attention(-50)
                            print(f"{Fore.GREEN}Negociación exitosa - Intervención militar reducida{Style.RESET_ALL}")
                        else:
                            print(f"{Fore.RED}Las negociaciones fallaron - No confían en ti{Style.RESET_ALL}")
                            self.player.increase_military_attention(50)
                    else:
                        print(f"{Fore.RED}No tienes suficiente dinero para negociar{Style.RESET_ALL}")
                        
            elif choice == 2 and military_level != "none":
                # Leave state temporarily
                if self.player.money >= 100000:
                    self.player.remove_money(100000)
                    self.player.military_attention = max(0, self.player.military_attention - 150)
                    self.player.location = "Mexico"  # Temporary exile
                    print(f"{Fore.GREEN}Has abandonado New Mexico temporalmente{Style.RESET_ALL}")
                    print(f"{Fore.YELLOW}Ubicación actual: México (Exilio temporal){Style.RESET_ALL}")
                else:
                    print(f"{Fore.RED}No tienes suficiente dinero{Style.RESET_ALL}")
                    
            elif choice == 3 and military_level != "none":
                # Seek neutral territory refuge
                if self.player.money >= 250000:
                    self.player.remove_money(250000)
                    self.player.military_attention = max(0, self.player.military_attention - 100)
                    self.player.fbi_attention = max(0, self.player.fbi_attention - 30)
                    print(f"{Fore.GREEN}Has encontrado refugio en territorio neutral{Style.RESET_ALL}")
                    print(f"{Fore.YELLOW}Atención militar y FBI reducidas{Style.RESET_ALL}")
                else:
                    print(f"{Fore.RED}No tienes suficiente dinero{Style.RESET_ALL}")
